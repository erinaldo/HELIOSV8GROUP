...
Public Class frmRecepcionHospedadosXCliente
    Inherits frmMaster
    ...
    Public Property tipoIngreso As Integer = 0
    Public Property listaDoc As New List(Of entidad)
    Public Sub New()
...
#Region "Metodo"
    ...
    Private Sub DUPLICADOS(DNI As String)
        'For Each ITEM In FormPurchase.ListaPersonasHospedadas
        '    If (ITEM.nroDocumento = DNI) Then
        '        MessageBox.Show("ya Existe un cliente registrado con el documento")
        '        Exit Sub
        '    End If
        'Next
    End Sub
    ...
    Public Sub AgregarEntidad()
        Try
            ...
            For Each ITEM In dgvCompras.Table.Records
                If (ITEM.GetValue("dni") = TextNumIdentrazon.Text) Then
                    MessageBox.Show("ya Existe un cliente registrado con el documento")
                    Exit Sub
                End If
            Next
            ...
            listaProductosNuevo.Add(obj)
            AgregarCanasta(listaProductosNuevo)
    ...
    Private Sub AgregarCanastaPrincipal(listaProductos As List(Of personaBeneficio), IDPadr As Integer)
        If (listaProductos.Count > 0) Then
            Dim ListaNuevo As New List(Of personaBeneficio)
            ListaNuevo = listaProductos.Where(Function(O) O.distribucionID = IDPadr).ToList
...
#End Region
    ...
    Public Sub cargarHospedadosBD(IdDistribucion As Integer, ID As Integer)
        Try
            Dim personaSA As New personaBeneficioSA
            Dim personaBE As New personaBeneficio
            Dim ListaPersonaBenef As New List(Of personaBeneficio)

            personaBE = New personaBeneficio
            personaBE.idDistribucion = IdDistribucion
            personaBE.secuencia = ID
            personaBE.estado = "A"
            personaBE.idEmpresa = Gempresas.IdEmpresaRuc

            ListaPersonaBenef = personaSA.ListarPersonaXHabXCliente(personaBE)
            ...
            For Each i In ListaPersonaBenef
                dt.Rows.Add(i.idPersonaBeneficio,
                        conteo,
                        i.nroDocumento,
                        i.nombrePersona,
    ...
    Private Sub BunifuFlatButton2_Click_1(sender As Object, e As EventArgs) Handles BunifuFlatButton2.Click
        Try
            Dim entidad As New personaBeneficio
            Dim ENTIDADSA As New personaBeneficioSA
            ...
            If (tipoIngreso = 1) Then
                If (dgvCompras.Table.Records.Count > 0) Then
                    For Each item In dgvCompras.Table.Records
                        ...
                        Dim consulta = (listaDoc.Where(Function(o) o.listaDistribucion = idPadreDoc)).FirstOrDefault

                        entidad = New personaBeneficio
                        entidad.idEmpresa = Gempresas.IdEmpresaRuc
                        entidad.idEstablecimiento = GEstableciento.IdEstablecimiento
                        ...
                        entidad.distribucionID = idPadreDoc
                        entidad.idDistribucion = idPadreDoc
                        entidad.estado = "A"
                        entidad.idDocumento = consulta.idOrganizacion
                        entidad.secuencia = CInt(consulta.idEmpresa)
                        ListaEntidad.Add(entidad)

                    Next
                    ENTIDADSA.SavePersonaBeneficio(ListaEntidad, entidad.idDocumento)
                    Hide()
                End If
            Else
                If (dgvCompras.Table.Records.Count > 0) Then
        ...
        If Not IsNothing(cc) Then
            Select Case cc.ColIndex
                ...
                Case 3
                    ...
                    listaProductosNuevo = New List(Of personaBeneficio)
                    ...
                    If (dgvCompras.Table.Records.Count > 0) Then
                        For Each item In dgvCompras.Table.Records
                            ...
                            listaProductosNuevo.Add(entidadBE)
                        Next
                    End If
                Case 2
                    Dim entidadBE As personaBeneficio
    ...
    Private Sub BunifuFlatButton4_Click_1(sender As Object, e As EventArgs) Handles BunifuFlatButton4.Click
        Try
            ...
            If (Not IsNothing(dgvCompras.Table.CurrentRecord)) Then
                ...
                Dim item = listaProductosNuevo.Where(Function(o) o.nroDocumento = codigo).SingleOrDefault
                If item IsNot Nothing Then
                    listaProductosNuevo.Remove(item)
