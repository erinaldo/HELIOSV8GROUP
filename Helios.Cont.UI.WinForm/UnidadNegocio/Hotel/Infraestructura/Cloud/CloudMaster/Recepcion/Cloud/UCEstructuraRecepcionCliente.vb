...
Public Class UCEstructuraRecepcionCliente
...
#Region "Attributes"
    ...
    Public Property FormPurchase As FormVentaRecepcionCliente
    ...
    Public Property listaDatosComplementarios As List(Of ocupacionInfraestructura)
...
#Region "Constructors"
    Public Sub New(formventaNueva As FormVentaRecepcionCliente)
        ...
        txtCheckIn.MinDate = DateTime.Today
        txtCheckOn.Value = DateAdd(DateInterval.Day, 1, txtCheckIn.Value)
        Me.txtCheckOn.MinDate = DateAdd(DateInterval.Day, 1, txtCheckIn.Value)
        txtdias.Text = 1
...
#End Region
    ...
    Private Sub ActualizarDatos(dias As Integer)
        Try
            Dim listaProductos = New List(Of documentoventaAbarrotesDet)
            Dim obj As documentoventaAbarrotesDet

            If (Not IsNothing(ListaproductosVendidos)) Then

                listaProductos = ListaproductosVendidos
                ListaproductosVendidos = New List(Of documentoventaAbarrotesDet)
                For Each ITEM In listaProductos
                    ...
                    Dim precioVentaValue As Decimal = ITEM.PrecioUnitarioVentaMN.GetValueOrDefault
                    Dim canti As Decimal = dias
                    Dim baseImponible As Decimal = 0
                    Dim Iva As Decimal = 0
                    Dim total As Decimal = canti * ITEM.PrecioUnitarioVentaMN.GetValueOrDefault
                    ...
                    obj.CustomProducto.descripcionItem = ITEM.CustomProducto.descripcionItem
                    obj.CustomProducto.unidad1 = "NIU"
                    obj.CustomProducto.codigodetalle = ITEM.CustomProducto.codigodetalle
                    ...
                    obj.DetalleItem = ITEM.DetalleItem
                    obj.catalogo_id = 0
                    ...
                    obj.fechaIngreso = txtCheckIn.Value
                    obj.fechaFin = txtCheckOn.Value
                    obj.CustomListaInventarioMovimiento = New List(Of InventarioMovimiento)

                    'obj.CustomDocumentoCaja = New List(Of documentoCaja)

                    obj.conteoHospedados = (ListaPersonasHospedadas.Where(Function(o) o.distribucionID = ITEM.idDistribucion)).Count
    ...
    Private Sub ActualizarDatosxItem(dias As Integer, DistribucionID As Integer, fechaIngreso As DateTime, fechaFin As DateTime)
        Try
            Dim listaProductos = New documentoventaAbarrotesDet

            If (Not IsNothing(ListaproductosVendidos)) Then

                listaProductos = ListaproductosVendidos.Where(Function(o) o.idDistribucion = DistribucionID).FirstOrDefault

                If (Not IsNothing(listaProductos)) Then

                    Dim precioVentaValue As Decimal = listaProductos.PrecioUnitarioVentaMN.GetValueOrDefault
                    Dim canti As Decimal = dias
                    Dim baseImponible As Decimal = 0
                    Dim Iva As Decimal = 0
                    Dim total As Decimal = canti * listaProductos.PrecioUnitarioVentaMN.GetValueOrDefault
                    baseImponible = Math.Round(CDec(CalculoBaseImponible(total, 1.18)), 2)
                    Iva = Math.Round(total - baseImponible, 2)


                    listaProductos.monto1 = CDec(txtdias.Text)
                    listaProductos.montokardex = CDec(baseImponible)
                    listaProductos.montoIgv = Iva
                    listaProductos.importeMN = total
                    listaProductos.PrecioUnitarioVentaMN = precioVentaValue
                    listaProductos.precioUnitario = precioVentaValue
                    listaProductos.fechaIngreso = fechaIngreso
                    listaProductos.fechaFin = fechaFin
...
#Region "Events"
    ...
    Public Sub LoadCanastaVentas(listaProductos As List(Of documentoventaAbarrotesDet))
        ...
        dt.Columns.Add("diasHospedado")
        dt.Columns.Add("nroHospedados")
        dt.Columns.Add("agregarHospedados")
        dt.Columns.Add("eliminarHospedados")
        dt.Columns.Add("distribucionID")
        ...
        For Each i In listaProductos
            ...
            dt.Rows.Add(i.CodigoCosto,
                      ...
                      "", If(i.FlagBonif = "True", True, False), "", i.descuentoMN.GetValueOrDefault,
                        i.tipoExistencia, txtCheckIn.Value, txtCheckOn.Value, txtdias.Text, ListaPersonasHospedadas.Where(Function(O) O.distribucionID = i.idDistribucion).Count, 0, 0, i.idDistribucion)

            If (Not IsNothing(ListaPersonasHospedadas)) Then

                Dim consultaHopedados = (ListaPersonasHospedadas.Where(Function(o) o.distribucionID = i.idDistribucion)).ToList

                If (consultaHopedados.Count > 0) Then
                    For Each hosp In consultaHopedados
                        hosp.distribucionID = i.idDistribucion
                    Next
                End If
            End If

            'If (Not IsNothing(listaDatosComplementarios)) Then

            '    Dim consultaFechas = (listaDatosComplementarios.Where(Function(o) o.idDistribucion = i.ID)).ToList
            '    If (consultaFechas.Count > 0) Then
            '        For Each hosp In consultaFechas
            '            hosp.idDistribucion = i.idDistribucion
            '        Next
            '    End If
            'End If
    ...
    Private Sub TextBoxExt1_LostFocus(sender As Object, e As EventArgs)
        ...
        If cc.Renderer IsNot Nothing Then
            ...
            If cc.ColIndex > -1 Then
                ...
                ElseIf style.TableCellIdentity.Column.Name = "precioventa" Then
                    ...
                    '    '    End If
                    '    'End If
                ElseIf style.TableCellIdentity.Column.Name = "fechaInicio" Then
                    If cc.Renderer IsNot Nothing Then
                        If e.TableControl.Model.Modified = True Then
                            Dim text As String = cc.Renderer.ControlText
                            Dim r As Record = GridCompra.Table.CurrentRecord
                            If text.Trim.Length > 0 Then
                                If GridCompra.Table.CurrentRecord IsNot Nothing Then
                                    ...
                                    If ((Date.Now).Date < CDate(r.GetValue("fechaInicio")).Date) Then
                                        Dim Cant As Integer = 0                         ' Almacenar total de dias 
                                        Dim Ini As DateTime = r.GetValue("fechaInicio")           ' Fecha Inicial
                                        Dim Fin As DateTime = r.GetValue("fechaFin")               ' Fecha Final
                                        ...
                                        'Next
                                        r.SetValue("diasHospedado", 1)

                                        r.SetValue("fechaFin", DateAdd(DateInterval.Day, CInt(1), r.GetValue("fechaInicio")))

                                        Dim precioVentaValue As Decimal = (r.GetValue("precioventa") / r.GetValue("cantidad")) * 1
                                        Dim canti As Decimal = 1
                                        Dim baseImponible As Decimal = 0
                                        Dim Iva As Decimal = 0
                                        Dim total As Decimal = (r.GetValue("totalmn") / r.GetValue("cantidad")) * 1
                                        baseImponible = Math.Round(CDec(CalculoBaseImponible(total, 1.18)), 2)
                                        Iva = Math.Round(total - baseImponible, 2)


                                        r.SetValue("pumn", precioVentaValue)
                                        r.SetValue("totalmn", total)
                                        r.SetValue("vcmn", baseImponible)
                                        r.SetValue("igvmn", Iva)
                                        r.SetValue("cantidad", 1)
                                        r.SetValue("precioventa", precioVentaValue)

                                        ActualizarDatosxItem(1, r.GetValue("distribucionID"), r.GetValue("fechaInicio"), r.GetValue("fechaFin"))
                                        GetTotalesDocumento()

                                    Else
                                        r.SetValue("fechaInicio", Date.Now)
                        ...
                        End If
                    End If
                ElseIf style.TableCellIdentity.Column.Name = "fechaFin" Then
                    If cc.Renderer IsNot Nothing Then
                        If e.TableControl.Model.Modified = True Then
                            Dim text As String = cc.Renderer.ControlText
                            Dim r As Record = GridCompra.Table.CurrentRecord
                            If text.Trim.Length > 0 Then
                                If GridCompra.Table.CurrentRecord IsNot Nothing Then
                                    ...
                                    '    strCadena += item.Value & " de " & item.Key
                                    'Next
                                    r.SetValue("diasHospedado", Cant)
                                    r.SetValue("fechaFin", DateAdd(DateInterval.Day, CInt(Cant), r.GetValue("fechaInicio")))

                                    Dim precioVentaValue As Decimal = (r.GetValue("precioventa") / r.GetValue("cantidad"))
                                    Dim canti As Decimal = (Cant)
                                    Dim baseImponible As Decimal = 0
                                    Dim Iva As Decimal = 0
                                    Dim total As Decimal = (r.GetValue("totalmn") / r.GetValue("cantidad")) * (Cant)
                                    baseImponible = Math.Round(CDec(CalculoBaseImponible(total, 1.18)), 2)
                                    Iva = Math.Round(total - baseImponible, 2)
                                    ...
                                    ActualizarDatosxItem(canti, r.GetValue("distribucionID"), r.GetValue("fechaInicio"), r.GetValue("fechaFin"))
                                    GetTotalesDocumento()
        ...
        If r IsNot Nothing Then
            If GridCompra.Table.Records.Count > 0 Then
                If UCPreciosCanastaVenta IsNot Nothing Then
                    ...
                    Dim value As String = r.GetValue("distribucionID").ToString()
                    'Dim prod = ListaproductosVendidos.Where(Function(o) o.CodigoCosto = value).Single
                    ...
                    '    Case TipoExistencia.ServicioGasto
                    '    Case TipoExistencia.Infraestructura
                    UCHospedados.ListInventario.Items.Clear()
                    UCHospedados.GetDetallePersonaBeneficioID(ListaPersonasHospedadas, value)
    ...
    Private Sub GridCompra_QueryCellStyleInfo(sender As Object, e As GridTableCellStyleInfoEventArgs) Handles GridCompra.QueryCellStyleInfo
        If e.Style.TableCellIdentity.Column IsNot Nothing AndAlso e.Style.TableCellIdentity.Column.Name = "agregarHospedados" AndAlso e.TableCellIdentity.DisplayElement.Kind = Syncfusion.Grouping.DisplayElementKind.Record Then
            'Setting the ToolTip text.
            e.Style.CellTipText = "Agregar Hospedados"
        End If
        'If e.TableCellIdentity.Column IsNot Nothing AndAlso e.TableCellIdentity.Column.Name = "fechaInicio" AndAlso e.TableCellIdentity.DisplayElement.Kind = Syncfusion.Grouping.DisplayElementKind.Record Then

        '    Dim value As String = e.TableCellIdentity.DisplayElement.GetRecord().GetValue("fechaInicio").ToString()
        ...
        '    value.MINDATE = DateTime.Today

        '    e.Style.MINDATE = DateTime.Today
        ...
        '    If (el.Kind = Syncfusion.Grouping.DisplayElementKind.Record AndAlso e.TableCellIdentity.Column IsNot Nothing AndAlso (e.TableCellIdentity.Column.Name = "fechaInicio")) Then
        '        Dim value As String = e.TableCellIdentity.DisplayElement.GetRecord().GetValue("fechaInicio").ToString()
    ...
    Private Sub BunifuFlatButton15_Click(sender As Object, e As EventArgs) Handles BunifuFlatButton15.Click, BunifuFlatButton3.Click, BunifuFlatButton2.Click, BunifuFlatButton4.Click
        Try
            Dim btn = sender
    ...
    Private Sub TxtCheckOn_ValueChanged(sender As Object, e As EventArgs)
        Try
    ...
    Private Sub TxtInfraestructura_Click(sender As Object, e As EventArgs) Handles txtInfraestructura.Click
        Try
            Dim f As New Tab_RecepcionCliente()
            f.StartPosition = FormStartPosition.CenterParent
            f.fechaIngreso = txtCheckIn.Value
            f.fechaFin = txtCheckOn.Value
            f.ShowDialog()
            If f.Tag IsNot Nothing Then
                Dim c = CType(f.Tag, List(Of documentoventaAbarrotesDet))
                For Each item In c
    ...
    Private Sub TxtCheckIn_ValueChanged(sender As Object, e As EventArgs) Handles txtCheckIn.ValueChanged
        Try
            ...
            Dim Cant As Integer = 0                         ' Almacenar total de dias 
            Dim Ini As DateTime = txtCheckIn.Value              ' Fecha Inicial
            Dim Fin As DateTime = txtCheckOn.Value              ' Fecha Final
            Dim diferencia As TimeSpan = Fin.Subtract(Ini)  ' Diferencia entre el rango 
            Dim dic As New Dictionary(Of String, String)    ' Diccionario.
    ...
    Private Sub TxtCheckOn_ValueChanged_1(sender As Object, e As EventArgs) Handles txtCheckOn.ValueChanged
        Try
    ...
    Private Sub GridCompra_TableControlPushButtonClick(sender As Object, e As GridTableControlCellPushButtonClickEventArgs) Handles GridCompra.TableControlPushButtonClick
        ...
        Try
            If e.Inner.ColIndex = 22 Then
                ...
                Dim f As New frmRecepHospedados(Me, GridCompra.TableModel(e.Inner.RowIndex, 24).CellValue)
                f.CARGARFECHAS(GridCompra.TableModel(e.Inner.RowIndex, 18).CellValue)
                f.txtCheckIn.Value = GridCompra.TableModel(e.Inner.RowIndex, 18).CellValue
                'f.txtCheckOn.Value = GridCompra.TableModel(e.Inner.RowIndex, 19).CellValue
                f.txtdias.Text = GridCompra.TableModel(e.Inner.RowIndex, 20).CellValue
                f.txtHabitacion.Text = GridCompra.TableModel(e.Inner.RowIndex, 3).CellValue
                f.StartPosition = FormStartPosition.CenterParent
                f.ShowDialog()
                If f.Tag IsNot Nothing Then
                    Dim c = CType(f.Tag, List(Of personaBeneficio))
                    For Each Item In c
                        ListaPersonasHospedadas.Add(Item)
                        Conteo += 1
                    Next

                    Dim FilaProducto = ListaproductosVendidos.Where(Function(o) o.idDistribucion = GridCompra.TableModel(e.Inner.RowIndex, 24).CellValue).FirstOrDefault

                    If (Not IsNothing(FilaProducto)) Then
                        FilaProducto.listaPersonaHospedada = ListaPersonasHospedadas
                    End If

                    GridCompra.TableModel(e.Inner.RowIndex, 21).CellValue = Conteo
                End If
    ...
    Private Sub GridCompra_TableControlCurrentCellShowingDropDown(sender As Object, e As GridTableControlCurrentCellShowingDropDownEventArgs) Handles GridCompra.TableControlCurrentCellShowingDropDown
        If TypeOf e.TableControl.CurrentCell.Renderer Is GridDropDownMonthCalendarCellRenderer Then
            ...
            If de.ColIndex = 19 Then
                TryCast(de.DropDownContainer.Controls(0), MonthCalendar).ShowTodayCircle = False
                TryCast(de.DropDownContainer.Controls(0), MonthCalendar).MinDate = Date.Now.Date.AddDays(1)
            ElseIf (de.ColIndex = 18) Then
                TryCast(de.DropDownContainer.Controls(0), MonthCalendar).ShowTodayCircle = False
                TryCast(de.DropDownContainer.Controls(0), MonthCalendar).MinDate = Date.Now
            End If
        End If
    End Sub
