...
Public Class FormVentaRecepcionCliente
...
#Region "Attributes"
    ...
    Public UCEstructuraCabeceraVentaV2 As UCEstructuraRecepcionCliente
    Public UCCondicionesPago As UCPagoVentaRecepcion
...
#Region "Constructors"
    ...
    Public Sub New(CodigoProforma As String)
        ...
        GetComboPrincipal()
        Me.KeyPreview = True
        UCEstructuraCabeceraVentaV2 = New UCEstructuraRecepcionCliente(Me)
        UCCondicionesPago = New UCPagoVentaRecepcion(Me)
...
#End Region
    ...
    Private Sub MappingDocumentoCompraCabeceraDetalle(obj As documento)
        ...
        For Each i In UCEstructuraCabeceraVentaV2.ListaproductosVendidos
            ...
            If (i.importeMN > 0) Then
                Select Case i.tipoExistencia
                    ...
                    Case TipoExistencia.Infraestructura
                        ...
                        objOcupacion = New ocupacionInfraestructura With
                                        ...
                                        .[idEntidad] = CInt(UCEstructuraCabeceraVentaV2.TextProveedor.Tag),
                                        .[chek_in] = i.fechaIngreso,
                                        .[check_on] = i.fechaFin,
                                        .[estado] = "A",
                                        .[glosario] = "OCUPADO",
                        ...
                        Dim ConsultaPer = (From z In i.listaPersonaHospedada Where z.distribucionID = i.idDistribucion).ToList
                ...
                End If
                objDet.personaBeneficio = listaHospedados
        ...
        If UCEstructuraCabeceraVentaV2.ComboTerminosPago.Text = "CONTADO" Then
            ...
            Select Case UCEstructuraCabeceraVentaV2.RBFullName.Checked = True
                Case True
                    ...
                    If (UCCondicionesPago.UCPagoCompletoDocumento.TextSaldo.Text > 0) Then
                        MessageBox.Show("Debe Realizar el pago completo!", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                        listaErrores += 1
                        ErrorProvider1.SetError(UCCondicionesPago.UCPagoCompletoDocumento.TextSaldo, Nothing)
    ...
    Private Sub MappingDatosComplementarios(obj As documento)
        Dim objDet As ocupacionInfraestructura
        obj.ocupacionInfra = New ocupacionInfraestructura

        'Dim consulta = UCEstructuraCabeceraVentaV2.ObjDatosComplementarios

        'If (Not IsNothing(consulta)) Then
        '    objDet = New ocupacionInfraestructura With
        '    {
        '    .[idEmpresa] = consulta.idEmpresa,
        '    .[idEstablecimiento] = consulta.idEstablecimiento,
        '    .[idDistribucion] = consulta.[idDistribucion],
        '    .[idEntidad] = consulta.[idEntidad],
        '    .[chek_in] = consulta.[chek_in],
        '    .[check_on] = consulta.[check_on],
        '    .[estado] = consulta.[estado],
        '      .[glosario] = consulta.[glosario],
        '    .[usuarioActualizacion] = consulta.[usuarioActualizacion],
        '    .[fechaActualizacion] = consulta.[fechaActualizacion]
        '    }
        '    obj.ocupacionInfra = (objDet)
        'Else
        '    objDet = New ocupacionInfraestructura With
        '    {
        '    .[idEmpresa] = Gempresas.IdEmpresaRuc,
        ...
        '    .[chek_in] = UCEstructuraCabeceraVentaV2.txtCheckIn.Value,
        '    .[check_on] = UCEstructuraCabeceraVentaV2.txtCheckOn.Value,
        '    .[estado] = "A",
        '      .[glosario] = "Fecha de ingres y Salida",
        '    .[usuarioActualizacion] = usuario.IDUsuario,
        '    .[fechaActualizacion] = Date.Now
        '    }
        '    obj.ocupacionInfra = (objDet)
        ...
        Dim objpER As personaBeneficio
        obj.personaBeneficio = New List(Of personaBeneficio)

        Dim ListaOBj = UCEstructuraCabeceraVentaV2.ListaPersonasHospedadas

        For Each ite In ListaOBj
            objpER = New personaBeneficio With
          {
          .[idEmpresa] = Gempresas.IdEmpresaRuc,
          .[idEstablecimiento] = GEstableciento.IdEstablecimiento,
          ...
          .[fechaActualizacion] = Date.Now
            ...
            obj.personaBeneficio.Add(objpER)
        Next
    ...
    Private Sub Grabarventa()
        ...
        Try
            ...
            Select Case ComboComprobante.Text
                Case "OTRA SALIDA DE ALMACEN", "PROFORMA", "PEDIDO"
                    'Dim Vendedor = GetCodigoVendedor()
                    'If Vendedor Is Nothing Then
                    '    Throw New Exception("Debe indicar el codigo del vendedor!")
                    'End If
                    obj.usuarioActualizacion = UCEstructuraCabeceraVentaV2.txtIDResponsable
                Case Else
                    'Dim Vendedor = GetCodigoVendedor()
                    'If Vendedor Is Nothing Then
                    '    Throw New Exception("Debe indicar el codigo del vendedor!")
                    ...
                    Dim codigoVendedor = UCEstructuraCabeceraVentaV2.txtIDResponsable 'Vendedor.codigo ' UCCondicionesPago.UCPagoCompletoDocumento.TextCodigoVendedor.Text.Trim
                    ...
                    obj.usuarioActualizacion = UCEstructuraCabeceraVentaV2.txtIDResponsable 'usuarioSel.IDUsuario
            ...
            '    MappingDatosComplementarios(obj)
            'End If
...
#End Region
    ...
    Private Function GetConfiguracionUsuario(usuarioSel As Seguridad.Business.Entity.Usuario, cajaUsuario As cajaUsuario) As EnvioImpresionVendedorPernos
        ...
        envio = New EnvioImpresionVendedorPernos With
            {
            .CodigoVendedor = UCEstructuraCabeceraVentaV2.txtIDResponsable, 'UCCondicionesPago.UCPagoCompletoDocumento.TextCodigoVendedor.Text.Trim,
            .IDCaja = cajaUsuario.idcajaUsuario,' ComboCaja.SelectedValue,
            .IDVendedor = UCEstructuraCabeceraVentaV2.txtIDResponsable, ' usuarioSel.IDUsuario,
            .print = True,
    ...
    Private Sub MappingPagos(envio As EnvioImpresionVendedorPernos, obj As documento)
        ...
        Else
            MessageBox.Show("Debe realizar todo el pago")
            Exit Sub
...
#Region "Events"
    ...
    Private Sub btGrabar_Click(sender As Object, e As EventArgs) Handles btGrabar.Click
        ...
        If (ManipulacionEstado = ENTITY_ACTIONS.INSERT) Then
            Select Case ComboComprobante.Text
                ...
                Case Else
                    'If (UCEstructuraCabeceraVentaV2.ListaPersonasHospedadas.Count <= 0) Then
                    '    ErrorProvider1.SetError(UCEstructuraCabeceraVentaV2.RoundButton21, "Debe registrar un Huésped")
                    '    Cursor = Cursors.Default
                    '    Exit Sub
                    'Else
                    '    ErrorProvider1.SetError(UCEstructuraCabeceraVentaV2.RoundButton21, Nothing)
                    'End If
            End Select


            If (UCEstructuraCabeceraVentaV2.txtdias.Text = "0") Then
                ErrorProvider1.SetError(UCEstructuraCabeceraVentaV2.txtdias, "El día debe ser mayor a 0")
                Cursor = Cursors.Default
                Exit Sub
            Else
                ErrorProvider1.SetError(UCEstructuraCabeceraVentaV2.txtdias, Nothing)
            End If
    ...
    Private Sub ToolImportar_Click(sender As Object, e As EventArgs) 
        Try
    ...
    Private Sub btServicio_Click(sender As Object, e As EventArgs) 
                ...
                If MessageBox.Show("¿Desea salir de la venta?", "Salir de la venta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = vbYes Then
                    ...
                    If (ManipulacionEstado = ENTITY_ACTIONS.INSERT) Then
                        ...
                        For Each ITEM In UCEstructuraCabeceraVentaV2.GridCompra.Table.Records
                            distribucionInfraestructuraBE.idEmpresa = Gempresas.IdEmpresaRuc
                            distribucionInfraestructuraBE.listaEstado.Add(ITEM.GetValue("distribucionID"))
                            distribucionInfraestructuraBE.estado = "A"
                        Next
    ...
    Private Sub CheckStock_OnChange(sender As Object, e As EventArgs) Handles CheckStock.OnChange
        If (CheckStock.Checked = True) Then
            ComboComprobante.Text = "PEDIDO"
            '''ComboComprobante_SelectedValueChanged(sender, e)
