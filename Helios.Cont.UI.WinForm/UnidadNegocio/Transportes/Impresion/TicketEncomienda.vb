...
Public Class TicketEncomienda
    ...
    Public Property DescuentoTotal As Decimal = 0.0
    Public Property TotalImporte As Decimal = 0.0

    Public Property RUTA As String = String.Empty

    Public Property DIRECIONEMBAR As String = String.Empty
    ...
    Private Sub DibujaLaSubCabeceraIzquierda()
        ...
        lineaEspacio = lineaEspacio + 3
        'Se imprime los datos dentro del rectangulo
        For Each Cabecera As String In LineasDeEncabezadoIzquierda
            'se crea un rectangulo para manipular texto dentro del rectangulo

            '/RUTA/
            Dim R112 As New RectangleF(1, lineaEspacio, 60, 25)

            CadenaPorEscribirEnLinea = ordTot.ObtenerLugar(Cabecera)
            gfx.DrawString("ORIGEN: " & CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R112, StringFormat)
            lineaEspacio += 3

            '/RUTA/
            Dim R11 As New RectangleF(1, lineaEspacio, 60, 25)

            CadenaPorEscribirEnLinea = ordTot.ObtenerdireccionEntrega(Cabecera)
            gfx.DrawString("DESTINO: " & CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R11, StringFormat)
            lineaEspacio += 3

            ''/RUTA/
            'Dim R113 As New RectangleF(11, lineaEspacio, 60, 25)

            'CadenaPorEscribirEnLinea = ordTot.ObtenerHabitacion(Cabecera)
            'gfx.DrawString("SERVICIO: " & CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R113, StringFormat)
            'lineaEspacio += 3



            ''//
            'Dim R1 As New RectangleF(14, lineaEspacio, 60, 25)

            'CadenaPorEscribirEnLinea = ordTot.OrdernarfechaEmision(Cabecera)

            'gfx.DrawString("PAX: " & CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R1, StringFormat)
            'lineaEspacio += 3

            'Dim R2 As New RectangleF(14, lineaEspacio, 60, 25)

            ''se captura NumeroRuc del Array
            'CadenaPorEscribirEnLinea = ordTot.ObtenerTipoHabitacion(Cabecera)

            'gfx.DrawString("HORA: " & CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R2, StringFormat)
            'lineaEspacio += 3
            ...
            gfx.DrawString("DOC. REM: " & CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R4, StringFormat)
            lineaEspacio += 3

            CadenaPorEscribirEnLinea = CStr("REM: " & ordTot.ObtenerNombre(Cabecera))
            total = CStr(CadenaPorEscribirEnLinea).Length
            Dim saltodeLineaNombre As Integer = 1
            If (total <= 30) Then
                saltodeLineaNombre = saltodeLineaNombre * 3
                Dim R5 As New RectangleF(11, lineaEspacio, 60, saltodeLineaNombre)
                'se captura NumeroRuc del Array
            ...
            Dim FuenteImpresaEncabezado2 = New Font("Tahoma", 9, FontStyle.Bold)

            ''//SI EXISTE RUC

            Dim R44 As New RectangleF(1, lineaEspacio, 60, 25)
            'documkentop
            CadenaPorEscribirEnLinea = ordTot.Obtenerdireccion(Cabecera)
            gfx.DrawString("DNI CON: " & CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R44, StringFormat)
            lineaEspacio += 3

            CadenaPorEscribirEnLinea = CStr("CON: " & ordTot.ObtenerTelefono(Cabecera))
            total = CStr(CadenaPorEscribirEnLinea).Length
            Dim saltodeLineaNombre2 As Integer = 1
            If (total <= 30) Then
                saltodeLineaNombre2 = saltodeLineaNombre2 * 3
                Dim R55 As New RectangleF(1, lineaEspacio, 60, saltodeLineaNombre2)
                'se captura NumeroRuc del Array
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R55, StringFormat)
                lineaEspacio += 3
            ElseIf (total >= 30 And total <= 60) Then
                saltodeLineaNombre2 = 2 * 3
                Dim R55 As New RectangleF(1, lineaEspacio, 60, saltodeLineaNombre2)
                'se captura NumeroRuc del Array
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R55, StringFormat)
                lineaEspacio += 6
            ElseIf (total > 60 And total <= 90) Then
                saltodeLineaNombre2 = 3 * 3
                Dim R55 As New RectangleF(1, lineaEspacio, 60, saltodeLineaNombre2)
                'se captura NumeroRuc del Array
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R55, StringFormat)
                lineaEspacio += 6
        ...
        Dim r8 As New RectangleF(2, lineaEspacio, 70, +5)
        'Colocar la cantidad a la derecha.
        Dim StringFormatpuntos As New StringFormat
        StringFormatpuntos.Alignment = StringAlignment.Near
        gfx.DrawString("-----------------------------------------------------------------------", FuenteImpresaEncabezado, Brushes.Black, r8, StringFormatpuntos)
    ...
    Private Sub DibujaElementosFactura()
        ...
        Dim StringFormatLinea As New StringFormat
        StringFormatLinea.Alignment = StringAlignment.Center

        Dim RLINEA As New RectangleF(-2, lineaEspacio, 70, +5)
        gfx.DrawString("---------------------------------------------------------------------", FuenteImpresaEncabezado, Brushes.Black, RLINEA, StringFormatLinea)
        lineaEspacio += 3
        'Crear texto del EncabezaDO
        Dim REncabezado As New RectangleF(-4, lineaEspacio, 70, +5)
        gfx.DrawString("CANT  │           ARTICULO               │  IMPORTE   ", FuenteImpresaEncabezado, Brushes.Black, REncabezado, StringFormatLinea)
        ...
        lineaEspacio += 2
        Dim RLINEA2 As New RectangleF(-2, lineaEspacio, 70, +5)
        ...
        For Each Item As String In LineasElementosFactura
            'se crea un rectangulo para manipular texto dentro del rectangulo
            Dim R0 As New RectangleF(3, lineaEspacio, 7, +5)
            'con 7 de tamaño
            ...
            'se crea un rectangulo para manipular texto dentro del rectangulo
            Dim R6 As New RectangleF(42, lineaEspacio, 20, +5)
            'Colocar la cantidad a la derecha.
            ...
            TotalImporte += FormatNumber(ordTot.ObtenerVentaTotal(Item), 2)
            ...
            Dim StringFormatItem As New StringFormat
            StringFormatItem.Alignment = StringAlignment.Near
            If (total <= 29) Then
                saltodeLineaNombre = saltodeLineaNombre * 3
                Dim R15 As New RectangleF(11, lineaEspacio, 40, saltodeLineaNombre)
                'se captura NumeroRuc del Array
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R15, StringFormatItem)
                lineaEspacio += 3
            ElseIf (total >= 30 And total <= 60) Then
                saltodeLineaNombre = 2 * 3
                Dim R15 As New RectangleF(11, lineaEspacio, 40, saltodeLineaNombre)
            ...
            ElseIf (total > 60 And total <= 90) Then
                ...
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R15, StringFormatItem)
                lineaEspacio += 9
            ElseIf (total >= 91 And total <= 120) Then
                saltodeLineaNombre = 3 * 4
                Dim R15 As New RectangleF(11, lineaEspacio, 40, saltodeLineaNombre)
                'se captura NumeroRuc del Array
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R15, StringFormatItem)
                lineaEspacio += 12
            ElseIf (total >= 121 And total <= 150) Then
                saltodeLineaNombre = 3 * 5
                Dim R15 As New RectangleF(11, lineaEspacio, 40, saltodeLineaNombre)
                'se captura NumeroRuc del Array
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R15, StringFormatItem)
                lineaEspacio += 115
            ElseIf (total >= 151 And total <= 180) Then
                saltodeLineaNombre = 3 * 6
                Dim R15 As New RectangleF(11, lineaEspacio, 40, saltodeLineaNombre)
                'se captura NumeroRuc del Array
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R15, StringFormatItem)
                lineaEspacio += 118
            ElseIf (total >= 181) Then
                saltodeLineaNombre = 3 * 7
                Dim R15 As New RectangleF(11, lineaEspacio, 40, saltodeLineaNombre)
                'se captura NumeroRuc del Array
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R15, StringFormatItem)
                lineaEspacio += 21
    ...
    Private Sub DibujaTotalFacturaFinal()
        ...
        'Fuente de la impresion
        Dim FuenteImpresaEncabezado = New Font("Tahoma", 7, FontStyle.Regular)
        Dim FuenteImpresaEncabezadoTexto = New Font("Tahoma", 7, FontStyle.Regular)
        Dim FuenteImpresaEncabezadoTotal = New Font("Tahoma", 9, FontStyle.Regular)
        'Se dibuja uin rectangulo parte derecha
        ' DIRECCION A  LA IZQUIERDA, ESPACIO PARA ABAJO, ANCHO DEL RECTANGULO, LARGO DEL RTECTRANGULO
        Dim blackPen As New Pen(Color.Black, 0)
        ...
        'gfx.DrawRectangle(blackPen, 20, 80, 64, 42)
        'gfx.DrawRectangle(blackPen, 140, 261, 65, 20)

        'Encabezados de los importes Generales
        'CadenaPorEscribirEnLinea = "OP. GRATUITAS"
        'gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTexto, ColorDeLaFuente, 15, lineaEspacio + 5, New StringFormat())

        'CadenaPorEscribirEnLinea = "OP. EXONERADAS"
        'gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTexto, ColorDeLaFuente, 1, lineaEspacio + 3, New StringFormat())

        'CadenaPorEscribirEnLinea = "OP. INAFECTO"
        'gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTexto, ColorDeLaFuente, 15, lineaEspacio + 6, New StringFormat())

        'CadenaPorEscribirEnLinea = "OP. GRAVADA"
        'gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTexto, ColorDeLaFuente, 15, lineaEspacio + 9, New StringFormat())

        'If (DESTINO = "1") Then


        CadenaPorEscribirEnLinea = "OP. GRAVADA"
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTexto, ColorDeLaFuente, 11, lineaEspacio + 3, New StringFormat())

            CadenaPorEscribirEnLinea = "IGV"
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTexto, ColorDeLaFuente, 11, lineaEspacio + 6, New StringFormat())

            'CadenaPorEscribirEnLinea = "ICBPER"
            'gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTexto, ColorDeLaFuente, 15, lineaEspacio + 15, New StringFormat())

            CadenaPorEscribirEnLinea = "IMPORTE TOTAL"
        gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTexto, ColorDeLaFuente, 11, lineaEspacio + 9, New StringFormat())
        ...
        '    CadenaPorEscribirEnLinea = "IMPORTE TOTAL"
        '    gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTexto, ColorDeLaFuente, 1, lineaEspacio + 6, New StringFormat())
        ...
        For Each Elemento As String In LineasTotalFactura
            Dim R5 As New RectangleF(35, lineaEspacio + 3, 10, +5)
            ...
            'gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTexto, ColorDeLaFuente, 27, lineaEspacio + 5, StringFormatTotal)

            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R5, StringFormatTotal)

            Dim R6 As New RectangleF(38, lineaEspacio + 3, 25, +5)
            'CadenaPorEscribirEnLinea = ordTot.ObtenerTotalNombre(Elemento)
            'Dim R5 As New RectangleF(35, lineaEspacio + 3, 15, +5)
            CadenaPorEscribirEnLinea = FormatNumber(ordTot.ObtenerTotalImporte(Elemento), 2)
            'CadenaPorEscribirEnLinea = AlineaTextoaLaDerecha(CadenaPorEscribirEnLinea.Length) + CadenaPorEscribirEnLinea

            If (conteo = 5) Then
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTotal, Brushes.Black, R6, StringFormatTotal)
                lineaEspacio += 4
            Else
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R6, StringFormatTotal)
                lineaEspacio += 3
            End If
        ...
        Dim r8 As New RectangleF(2, lineaEspacio + 5, 65, +5)
        'Colocar la cantidad a la derecha.
        Dim StringFormatpuntos As New StringFormat
        StringFormatpuntos.Alignment = StringAlignment.Near
        gfx.DrawString("---------------------------------------------------------------------", FuenteImpresaEncabezado, Brushes.Black, r8, StringFormatpuntos)
    ...
    Private Sub DibujaDatosEmpresa()
        ...
        'Fuente de la impresion
        Dim FuenteImpresaEncabezado = New Font("Tahoma", 8, FontStyle.Italic)
        ...
        '    Dim total = ordTot.ObtenerTotalNombre(Elemento).Count
        '    Dim saltodeLineaNombre As Integer = 1
        '    Dim StringFormatItem As New StringFormat
        '    StringFormatItem.Alignment = StringAlignment.Center
        '    CadenaPorEscribirEnLinea = (ordTot.ObtenerTotalNombre(Elemento))
        '    If (total <= 29) Then
        '        saltodeLineaNombre = saltodeLineaNombre * 3
        '        Dim R15 As New RectangleF(2, lineaEspacio, 60, saltodeLineaNombre)
        '        'se captura NumeroRuc del Array
        '        gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R15, StringFormatItem)
        '        lineaEspacio += 3
        '    ElseIf (total >= 30 And total <= 60) Then
        ...
        '        lineaEspacio += 9
        '    ElseIf (total >= 91 And total <= 120) Then
        '        saltodeLineaNombre = 3 * 8
        '        Dim R15 As New RectangleF(2, lineaEspacio, 60, saltodeLineaNombre)
        '        'se captura NumeroRuc del Array
        '        gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R15, StringFormatItem)
        '        lineaEspacio += 12
        '    ElseIf (total >= 121 And total <= 150) Then
        '        saltodeLineaNombre = 3 * 9
        '        Dim R15 As New RectangleF(2, lineaEspacio, 60, saltodeLineaNombre)
        '        'se captura NumeroRuc del Array
        '        gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R15, StringFormatItem)
        '        lineaEspacio += 15
        '    ElseIf (total >= 151 And total <= 180) Then
        '        saltodeLineaNombre = 3 * 10
        '        Dim R15 As New RectangleF(2, lineaEspacio, 60, saltodeLineaNombre)
        '        'se captura NumeroRuc del Array
        '        gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R15, StringFormatItem)
        '        lineaEspacio += 18
        '    ElseIf (total >= 181) Then
        '        saltodeLineaNombre = 3 * 11
        '        Dim R15 As New RectangleF(2, lineaEspacio, 60, saltodeLineaNombre)
        '        'se captura NumeroRuc del Array
        '        gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R15, StringFormatItem)
        '        lineaEspacio += 21
        ...
        For Each Elemento As String In LineasDatosEmpresa
            ...
            ElseIf (NumTotalNombre > 60) Then
                Dim R2 As New RectangleF(1, lineaEspacio, 60, 20)
                CadenaPorEscribirEnLinea = (ordTot.ObtenerTotalNombre(Elemento))
                gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R2, StringFormat)
                lineaEspacio += 12
            Else
    ...
    '****************************** NOMBRE importes TOTALES ******************
    'Se declara el array para la Factura parte izquierda - datos generales
    Public LineasDatosComprobanteExtra As ArrayList = New ArrayList()

    'Linea para capturar el ruc y numeroComprobante de la Factura
    Public Sub AnadirLineaComprobanteExtra(ByVal nombreComprobante As String)

        Dim ordTot As OrdernarEmpresa = New OrdernarEmpresa()
        LineasDatosComprobanteExtra.Add(ordTot.GenerarTotal(nombreComprobante))
    End Sub

    'Dibuja `parte de la derecha de la factura
    Private Sub DibujaDatosComprobanteExtra()
        'se crea un rectangulo para manipular texto dentro del rectangulo

        'Fuente de la impresion
        Dim FuenteImpresaEncabezado = New Font("Tahoma", 10, FontStyle.Bold)
        ...
        For Each Elemento As String In LineasDatosComprobanteExtra
            Dim NumTotalNombre = ordTot.ObtenerTotalNombre(Elemento).Count
            If (NumTotalNombre >= 30 And NumTotalNombre <= 60) Then
                Dim R2 As New RectangleF(1, lineaEspacio, 60, 12)
                CadenaPorEscribirEnLinea = (ordTot.ObtenerTotalNombre(Elemento))
    ...
    '****************************** NOMBRE importes TOTALES ******************
    'Se declara el array para la Factura parte izquierda - datos generales
    Public LineasDatosDescuento As ArrayList = New ArrayList()

    'Linea para capturar el ruc y numeroComprobante de la Factura
    Public Sub AnadirLineasDescuento(ByVal nombreComprobante As String)

        Dim ordTot As OrdernarEmpresa = New OrdernarEmpresa()
        LineasDatosDescuento.Add(ordTot.GenerarTotal(nombreComprobante))
    End Sub

    'Dibuja `parte de la derecha de la factura
    Private Sub DibujaLineasDesccuento()
        'se crea un rectangulo para manipular texto dentro del rectangulo
        ...
        Dim R1 As New RectangleF(1, lineaEspacio, 60, 3)
        CadenaPorEscribirEnLinea = (DescuentoTotal)
        gfx.DrawString("TOTAL DSCTO.: " & CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R1, StringFormatTotal)
        lineaEspacio += 5

        Dim R3 As New RectangleF(1, lineaEspacio, 60, 3)
        Dim FuenteImpresaEncabezadoPago = New Font("Tahoma", 7, FontStyle.Bold)
        gfx.DrawString("PAGO:", FuenteImpresaEncabezadoPago, Brushes.Black, R3, StringFormatTotal)
        lineaEspacio += 3

        Dim CONTEO = 0
        For Each Elemento As String In LineasDatosDescuento
            'If (CONTEO = 0) Then
            '    Dim StringFormatNrioCuenta As New StringFormat
            '    StringFormatNrioCuenta.Alignment = StringAlignment.Far
            '    Dim R5 As New RectangleF(10, lineaEspacio - 3, 60, 5)
            '    CadenaPorEscribirEnLinea = (ordTot.ObtenerTotalNombre(Elemento))
            '    gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R5, StringFormatNrioCuenta)
            '    lineaEspacio += 3
            '    CONTEO += 1
            'Else
            Dim R2 As New RectangleF(5, lineaEspacio, 60, 5)
            CadenaPorEscribirEnLinea = (ordTot.ObtenerTotalNombre(Elemento))
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R2, StringFormat)
            lineaEspacio += 3
            'End If
        ...
        'Dim R4 As New RectangleF(5, lineaEspacio, 60, 3)
        'CadenaPorEscribirEnLinea = (DescuentoTotal)
        'gfx.DrawString("PENDIENTE: " & TotalImporte - DescuentoTotal, FuenteImpresaEncabezado, Brushes.Black, R4, StringFormat)
        'lineaEspacio += 3
    ...
    'Linea para capturar el ruc y numeroComprobante de la Factura
    Public Sub AnadirLineaDatos(ByVal NumeroBoleta As String, ByVal fechaViaje As String, ByVal ObtenerHoraViaje As String,
                                ByVal origen As String, ByVal destino As String, ByVal asiento As String, ByVal importe As String, ByVal piso As String)

        Dim ordTot As OrdenarTransporteCliente = New OrdenarTransporteCliente()
        LineasDatosComplementario.Add(ordTot.GenerarTotal(NumeroBoleta, fechaViaje, ObtenerHoraViaje, origen, destino, asiento, importe, piso))
    End Sub
    ...
    Private Sub DibujaDatosComplementarios()
        ...
        Dim FuenteImpresaEncabezado2 = New Font("Tahoma", 10, FontStyle.Bold)

        'MUESTRA QUE ESPACIO SE DARA AL PROPIETARIO PARA QUE EL ESPACIO NO SEA MUY GRANDE
        'lineaEspacio += 5
        'Dim R777 As New RectangleF(1, lineaEspacio, 65, +5)
        'Dim StringFormatGeneral As New StringFormat
        'StringFormatGeneral.Alignment = StringAlignment.Center
        'gfx.DrawString("-------------------------------------------------------------", FuenteImpresaEncabezado, Brushes.Black, R777, StringFormatGeneral)
        'lineaEspacio += 5
        For Each Elemento As String In LineasDatosComplementario

            Dim R2 As New RectangleF(1, lineaEspacio, 60, 6)
            CadenaPorEscribirEnLinea = (ordTot.ObtenerNumeroBoleta(Elemento))
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado2, Brushes.Black, R2, StringFormat2)

            lineaEspacio += 5
            Dim R3 As New RectangleF(1, lineaEspacio, 60, 3)
            CadenaPorEscribirEnLinea = (ordTot.ObtenerFechaViaje(Elemento))
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R3, StringFormat)
            lineaEspacio += 3

            Dim R4 As New RectangleF(1, lineaEspacio, 60, 3)
            CadenaPorEscribirEnLinea = (ordTot.ObtenerHoraViaje(Elemento))
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R4, StringFormat)
            lineaEspacio += 3
            Dim R5 As New RectangleF(1, lineaEspacio, 60, 3)
            CadenaPorEscribirEnLinea = (ordTot.ObtenerOrigen(Elemento))
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R5, StringFormat)
            lineaEspacio += 3
            Dim R6 As New RectangleF(1, lineaEspacio, 60, 3)
            CadenaPorEscribirEnLinea = (ordTot.ObtenerDestino(Elemento))
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R6, StringFormat)
            lineaEspacio += 3
            Dim R7 As New RectangleF(1, lineaEspacio, 60, 3)
            CadenaPorEscribirEnLinea = (ordTot.Obtenerasiento(Elemento))
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R7, StringFormat)
            lineaEspacio += 3
            Dim R8 As New RectangleF(1, lineaEspacio, 60, 3)
            CadenaPorEscribirEnLinea = (ordTot.ObtenerImporte(Elemento))
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R8, StringFormat)
            lineaEspacio += 3
            Dim R9 As New RectangleF(1, lineaEspacio, 60, 3)
            CadenaPorEscribirEnLinea = (ordTot.ObtenerPiso(Elemento))
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, Brushes.Black, R9, StringFormat)

        Next

        lineaEspacio += 1
        'Dim R767 As New RectangleF(1, lineaEspacio, 65, +5)

        'gfx.DrawString("", FuenteImpresaEncabezado, Brushes.Black, R767, StringFormat)

        'lineaEspacio += 3
        'Dim R77 As New RectangleF(1, lineaEspacio, 65, +5)
        'Dim StringFormatGeneral2 As New StringFormat
        'StringFormatGeneral.Alignment = StringAlignment.Center
        'gfx.DrawString("FELIZ VIAJE", FuenteImpresaEncabezado2, Brushes.Black, R77, StringFormatGeneral2)
