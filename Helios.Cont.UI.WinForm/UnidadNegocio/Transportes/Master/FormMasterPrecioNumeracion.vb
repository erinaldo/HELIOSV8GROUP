...
Public Class FormMasterPrecioNumeracion
    ...
    Private Sub ToolStripButton1_Click(sender As Object, e As EventArgs)
        Try
            If Not IsNothing(GridCompra.Table.CurrentRecord) Then
                ...
                Dim f As New FormConsolidarSalidaEmbarque(rec.GetValue("ID"))
                f.RoundButton24.Visible = True
                'f.RoundButton23.Visible = False
                f.TextSeriePlaca.Tag = rec.GetValue("idBus")
                f.TextSeriePlaca.Text = rec.GetValue("nombreBus")
                f.TextCodigoPlaca.Text = rec.GetValue("placaBus")
                f.IDaCTIVO = rec.GetValue("idBus")
    ...
    Private Sub ToolStripButton2_Click(sender As Object, e As EventArgs)
        Try
            If Not IsNothing(GridCompra.Table.CurrentRecord) Then
                ...
                'Dim f As New FormConsolidarSalidaEmbarque(rec.GetValue("ID"))
                'f.RoundButton24.Visible = False
                'f.RoundButton23.Visible = True
                'f.TextSeriePlaca.Text = rec.GetValue("idBus")
                'f.TextSeriePlaca.Tag = rec.GetValue("nombreBus")
                'f.TextCodigoPlaca.Text = rec.GetValue("idBus")
                'f.StartPosition = FormStartPosition.CenterParent
                'f.Show()

                Dim f As New FormViewResumenLiquidacion(Integer.Parse(rec.GetValue("ID")), "pasajes")
                f.StartPosition = FormStartPosition.CenterParent
                f.ShowDialog(Me)
    ...
    Private Sub ToolStripButton4_Click(sender As Object, e As EventArgs) Handles ToolStripButton4.Click
        Try
            If Not IsNothing(GridCompra.Table.CurrentRecord) Then
                ...
                Dim f As New TabTR_ConfiguracionPrecio
                f.cboActivosFijos.Visible = True
                f.idProgramacion = rec.GetValue("ID")
                f.getCargarCombos(rec.GetValue("idBus"))
                f.TextFechaProgramada.Value = TextFechaProgramada.Value
                f.TXTORIGEN.Text = rec.GetValue("origen")
                f.TXTDESTINO.Text = rec.GetValue("Destino")
                f.cargarBus(rec.GetValue("idBus"), "")
                f.StartPosition = FormStartPosition.CenterParent
                f.Show()
