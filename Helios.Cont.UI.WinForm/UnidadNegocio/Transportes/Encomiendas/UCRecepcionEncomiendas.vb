...
Public Class UCRecepcionEncomiendas
...
#Region "Methods"
    ...
    Private Sub GetEncomiendasDelDia(now As Date)
        ...
        For Each i In ventaSA.GetConsultaEncomiendasFecha(New Business.Entity.documentoventaTransporte With
                                                          {
    ...
    Private Sub GetEncomiendasMes(fecha As Date)
        ...
        dt.Columns.Add("numero")
        Dim comprador As String = String.Empty
        For Each i In ventaSA.GetConsultaEncomiendasSelMes(New Business.Entity.documentoventaTransporte With
                                                          {
                                                          .idEmpresa = Gempresas.IdEmpresaRuc,
                                                          .idOrganizacion = ComboAgenciaOrigen.SelectedValue,
                                                          .fechadoc = fecha
    ...
    Private Sub ToolStripButton3_Click(sender As Object, e As EventArgs) Handles ToolStripButton3.Click
        ...
        Try
            ...
            If r IsNot Nothing Then
                ...
                Dim comprobante = documentoSA.DocumentoTransporteSelID(New documentoventaTransporte With {.idDocumento = Integer.Parse(r.GetValue("id"))})

                Dim entidad = entidadSA.UbicarEntidadPorID(comprobante.razonSocial).FirstOrDefault

                ''ComboPrint.Text
                ''adelaida --pichanaki
                ImprimirTicket("TICKET", Integer.Parse(r.GetValue("id")), comprobante, entidad)
                'ImprimirTicket("\\192.168.1.30\TICKET", Integer.Parse(r.GetValue("id")), comprobante, entidad)

                'LA MERCED
                'ImprimirTicket("EPSON TM-T20II Receipt5", Integer.Parse(r.GetValue("id")), comprobante, entidad)

                ' otroas empresas
                'ImprimirTicketA4v2("TICKET/RUTA", Integer.Parse(r.GetValue("id")), comprobante, entidad)
            Else
                MessageBox.Show("DEBE SELECCIONAR UNA ENCOMIENDA")
            End If
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Critical, "Atención")
        End Try
