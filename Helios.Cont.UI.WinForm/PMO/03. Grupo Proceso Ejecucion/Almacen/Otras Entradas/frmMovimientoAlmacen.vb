...
#Region "Constructors"
    Public Sub New()
        ...
        txtfechaTraslado.Value = DateTime.Now
        txtfehcaEmision.Value = DateTime.Now
...
#Region "Asientos"
    ...
    Sub RegistrarMovimiento(nAsiento As asiento)
        ...
        Dim cosnulta = (From i In ListaMovimientos
    ...
    Sub UbicarAsientoPorId(asiento As asiento)
        Dim consulta = (From n In ListaAsientonTransito
...
#Region "PROVEEDOR"
    Public Sub InsertProveedor()
        ...
        Try
            ...
            TextPersona.Text = objCliente.nombreCompleto
            TextDNI.Text = objCliente.nrodoc
...
#End Region
    ...
    Public Sub UbicarDocumento(IdDocumento As Integer)
        ...
        Dim fechaEnvio = listaDocumentoCompra(0).FechaDoc
        TxtDia.Text = fechaEnvio.Value.Day
        cboMesCompra.SelectedValue = String.Concat("{0:00}", fechaEnvio.Value.Month)
        cboAnio.Text = fechaEnvio.Value.Year
    ...
    Private Sub cargarDatosCuenta(idEstable As Integer)
        ...
        If Not IsNothing(cboAlmacen.SelectedValue) Then
            If IsNumeric(codAlmacen) Then
                ...
                almacenDestino = almacenSA.GetListar_almacenExceptAV(New almacen With {.idEmpresa = Gempresas.IdEmpresaRuc, .idEstablecimiento = idEstable, .TipoConsulta = StatusTipoConsulta.XUNIDAD_ORGANICA})
    ...
    Sub GuiaRemision(objDocumentoCompra As documento)
        ...
        With guiaRemisionBE
            ...
            .fechaDoc = New DateTime(cboAnio.Text, CInt(cboMesCompra.SelectedValue), TxtDia.Text, DateTime.Now.Hour, DateTime.Now.Minute, DateTime.Now.Second) '
            .periodo = lblPerido.Text
            .tipoDoc = "09"
            .idEntidad = CInt(TextPersona.Tag)
        ...
        For Each r As Record In dgvTransferencia.Table.Records
            If r.GetValue("tipoExistencia") <> "GS" Then
                ...
                documentoguiaDetalle.nombreRecepcion = TextPersona.Text
                documentoguiaDetalle.dniRecepcion = TextDNI.Text
    ...
    Sub AsientoTransferenciaEntreAlmacenes()
        ...
        Try
            ...
            ElseIf RBOtros.Checked = True Then
                asientoBL.tipoEntidad = TIPO_ENTIDAD.PERSONA_GENERAL
            End If
...
#End Region
    ...
    Sub Grabar()
        ...
        For Each r As Record In dgvTransferencia.Table.Records
            ...
            objDocumentoCompraDet.codigoLote = r.GetValue("codigoLote")
            ...
            objDocumentoCompraDet.nombreRecepcion = TextPersona.Text
            objDocumentoCompraDet.dniRecepcion = TextDNI.Text
            objDocumentoCompraDet.puntoLlegada = "ORIGEN"
    ...
    Sub UpdateCompra()
        ...
        With nDocumentoCompra
            ...
            .idEmpresa = Gempresas.IdEmpresaRuc
            .idCentroCosto = GEstableciento.IdEstablecimiento
            .fechaDoc = New DateTime(cboAnio.Text, CInt(cboMesCompra.SelectedValue), TxtDia.Text, DateTime.Now.Hour, DateTime.Now.Minute, DateTime.Now.Second) 'txtFechaComprobante.Value
            .fechaContable = lblPerido.Text
            .serie = txtSerie.Text.Trim
            .numeroDoc = txtNumero.Text
            If RBProveedor.Checked = True Then
    ...
    Public Sub CargarListas()
        ...
        almacenDestino = almacenSA.GetListar_almacenExceptAV(cboEstablecimiento.SelectedValue)
...
#Region "Events"
            ...
            If lsvExistencias.SelectedItems.Count > 0 Then
                ...
                Select Case lblMovimiento.Text
                    Case "TRANSFERENCIA ENTRE ALMACENES"
                        ...
                        Me.dgvTransferencia.Table.CurrentRecord.SetValue("codigoLote", lsvExistencias.SelectedItems(0).SubItems(11).Text)
    ...
    Private Sub btnGRabarProv_Click(sender As Object, e As EventArgs) Handles btnGRabarProv.Click
        If Not txtDocProveedor.Text.Trim.Length > 0 Then
            ...
            Me.pcProveedor.ParentControl = Me.TextPersona
            ...
            Select Case lblMovimiento.Text
                Case "TRANSFERENCIA ENTRE ALMACENES"
                    ...
                    If ManipulacionEstado = ENTITY_ACTIONS.INSERT Then
                        If dgvTransferencia.Table.Records.Count > 0 Then
                            Dim valida As Boolean = empresaPeriodoSA.EstadoMesCerrado(New empresaCierreMensual With {.anio = fechaEnvio.Year, .mes = fechaEnvio.Month})
                            ...
                            If MessageBox.Show("Desea realizar la transferencia con fecha: " & vbCrLf &
    ...
    Private Sub ToolStripButton10_Click(sender As Object, e As EventArgs) Handles ToolStripButton10.Click
        ...
        If Not IsNothing(consulta) Then
            Dim listaMov = (From i In ListaMovimientos
    ...
    Private Sub PictureBox1_Click_1(sender As Object, e As EventArgs) Handles PictureBox1.Click
        ...
        f.tipoPersona(TIPO_ENTIDAD.CLIENTE, TextDNI.Text)
    ...
    Private Sub dgvTransferencia_QueryCellStyleInfo(sender As Object, e As GridTableCellStyleInfoEventArgs) Handles dgvTransferencia.QueryCellStyleInfo
        If (e.TableCellIdentity.TableCellType = GridTableCellType.RecordFieldCell OrElse e.TableCellIdentity.TableCellType = GridTableCellType.AlternateRecordFieldCell) Then
            ...
            If e.TableCellIdentity.Column.MappingName = "Can1" AndAlso CDbl(Fix(e.Style.CellValue)) > 0 Then
                ...
                If el IsNot Nothing AndAlso el.Kind = DisplayElementKind.Record Then
                    ...
                    Dim cant As Decimal? = CDec(r.GetValue("Can1"))
                    Dim cantDis As Decimal? = CDec(r.GetValue("colDisponible"))
