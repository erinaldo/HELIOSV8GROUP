...
Public Class frmOtrasSalidasDeProduccion

    Inherits frmMaster


    'Public Property RecursoEnvio As New recursoCostoDetalle
    Public Property ObjetoRecurso As New recursoCostoDetalle
    Public Property listaRecursos As New List(Of recursoCostoDetalle)
...
#End Region
    ...
    Sub CargarDetalleAutomatico(idAlmacen As Integer, iditem As Integer, cantenv As Decimal)
        ...
        Dim impme As Decimal = 0
        Dim pu As Decimal = 0
        Dim pume As Decimal = 0
        Dim imptotal As Decimal = 0
        ...
        Try
            ...
            With itemsSA.InvocarProductoID(iditem)
                    ...
                    Me.dgvMov.Table.CurrentRecord.SetValue("cantidad", cantenv)
                    Me.dgvMov.Table.CurrentRecord.SetValue("cantDisponible", cant)
                    Me.dgvMov.Table.CurrentRecord.SetValue("precMN", pu)
                    Me.dgvMov.Table.CurrentRecord.SetValue("importeMN", imptotal)
                    Me.dgvMov.Table.CurrentRecord.SetValue("precME", pume)
                    Me.dgvMov.Table.CurrentRecord.SetValue("importeME", imptotalme)
        ...
        For Each r As Record In dgvMov.Table.Records
            ...
            If txtTipoCosto.Text = "HC" Then
                If lbltipoentregable.Text = TipoCosto.COSTOS_POR_VALORACION Then
                    ...
                    nAsiento.importeMN = CDec(r.GetValue("importeMN"))
                    nAsiento.importeME = CDec(r.GetValue("importeME"))
...
#Region "Métodos"
    ...
    Public Sub CargarListas()
        ...
        almacen = almacenSA.GetListar_almacenesTipo(GEstableciento.IdEstablecimiento, "AP")
...
#End Region
    ...
    Public Sub UbicarDocumento(ByVal intIdDocumento As Integer)
        ...
        Try
            ...
            CompraDet = objDocCompraDet.UbicarDocumentoCompraDetalle(intIdDocumento)
    ...
    Public Function Glosa() As String
        ...
        Else
            str = "Por movimientos de almacén-" & lblMovimiento.Text & ", con fecha: " & txtFechaComprobante.Value
        End If
        Return str
    End Function
    ...
    Sub GrabarDefault()
        ...
        Dim ObjetoRecurso As New recursoCostoDetalle
        Dim listaRecursoas As New List(Of recursoCostoDetalle)
        ...
        If dgvMov.Table.Records IsNot Nothing AndAlso dgvMov.Table.Records.Count > 0 Then
            For Each r As Record In dgvMov.Table.Records
                ...
                'LLENAR COSTEO 
                'RecursoEnvio.cant = CDec(r.GetValue("cantidad"))
                'RecursoEnvio.montoMN = CDec(r.GetValue("importeMN"))
                'RecursoEnvio.montoME = CDec(r.GetValue("importeME"))



                'ObjetoRecurso = New recursoCostoDetalle

                'ObjetoRecurso.idCosto = txtidEntregable.Text
                'ObjetoRecurso.fechaRegistro = DateTime.Now
                'ObjetoRecurso.iditem = Val(r.GetValue("idItem"))
                ''ObjetoRecurso.destino = Me.GridGroupingControl1.Table.CurrentRecord.GetValue("destino")
                'ObjetoRecurso.descripcion = r.GetValue("item")
                'ObjetoRecurso.um = r.GetValue("idUM")
                'ObjetoRecurso.cant = CDec(r.GetValue("cantidad"))
                'ObjetoRecurso.puMN = 0
                'ObjetoRecurso.puME = 0
                'ObjetoRecurso.montoMN = CDec(r.GetValue("importeMN"))
                'ObjetoRecurso.montoME = CDec(r.GetValue("importeME"))
                ''.documentoRef = CInt(Me.GridGroupingControl1.Table.CurrentRecord.GetValue("idDocumento")),
                '' ObjetoRecurso.itemRef = CInt(Me.GridGroupingControl1.Table.CurrentRecord.GetValue("secuencia"))
                'ObjetoRecurso.operacion = cboOperacion.SelectedValue
                'ObjetoRecurso.procesado = "N"
                'ObjetoRecurso.fechaTrabajo = DateTime.Now
                'ObjetoRecurso.Periodo = PeriodoGeneral
                'ObjetoRecurso.idProceso = txtidEntregable.Text
                'ObjetoRecurso.cuenta = txtCuentaCosto.Text
                'ObjetoRecurso.NombreProceso = txtEntregable.Text
                'ObjetoRecurso.tipoCosto = "HC"

                'listaRecursoas.Add(ObjetoRecurso)



                'If RecursoEnvio.tipoCosto = "HC" Then  ' SI VIENE DEL MODULO COSTOS
                '    If CDec(r.GetValue("cantidad")) = CDec(r.GetValue("cantDisponible")) Then


                '        RecursoEnvio.tipoCosto = "HC"  ' si se envio todo
                '    Else

                '        RecursoEnvio.tipoCosto = "PC"
                '    End If
                'ElseIf RecursoEnvio.tipoCosto = "HG" Then ' SI VIENE DEL MODULO GASTOS

                '    If CDec(r.GetValue("cantidad")) = CDec(r.GetValue("cantDisponible")) Then


                '        RecursoEnvio.tipoCosto = "HG"  ' si se envio todo
                '    Else

                '        RecursoEnvio.tipoCosto = "PG"
                '    End If
                'End If
                ...
                '    '--------------------------------------------------------------------
                '    'costoSA.GetCostoById(New recursoCosto With {.idCosto = r.GetValue("elementocosto")}).codigo,
                '    nMovimiento = New movimiento With {
                '        .cuenta = "91",
                '        .descripcion = r.GetValue("item"),
                '        .tipo = "D",
...
#End Region
    ...
    'Private Sub rbCosto_CheckChanged(sender As Object, e As EventArgs)
    '    If rbCosto.Checked = True Then
    '        'Me.dgvMov.TableDescriptor.Columns("elementocosto").Width = 100
    '        'Me.dgvMov.TableDescriptor.Columns("proceso").Width = 120
    ...
    'Private Sub rbGasto_CheckChanged(sender As Object, e As EventArgs)
    '    If rbGasto.Checked = True Then

    '        Me.dgvMov.TableDescriptor.Columns("elementocosto").Width = 0
    '        Me.dgvMov.TableDescriptor.Columns("proceso").Width = 0

    '        ToolStripButton4.Enabled = True
    '        dockingManager1.SetDockVisibility(Panel2, True)
    '        'GetItemsCosto("HG")
    '        ''  cboTipoCosto_SelectedIndexChanged(sender, e)
    '    End If
    'End Sub

    'Private Sub cboOperacion_Click(sender As Object, e As EventArgs)
    ...
    Private Sub cboCosto_Click(sender As Object, e As EventArgs)
