...
Public Class FormConfirmrarVentaVendedor
...
#Region "Variables"
    ...
    Public Property CompradorID As String = String.Empty
...
#End Region
    ...
    Private Sub GetClientes2(tipo As String, empresa As String)
        Dim lista As New List(Of entidad)
        lista = New List(Of entidad)
        '   Dim varios = entidadSA.UbicarEntidadVarios("VR", General.Gempresas.IdEmpresaRuc, String.Empty)
        'Dim lista = entidadSA.ObtenerListaEntidad(tipo, empresa)
    ...
    Sub GrabarProforma(dgvCompra As GridGroupingControl)
        ...
        Dim miInterfaz As ICommitOperacionMKTIenda = TryCast(Me.Owner, ICommitOperacionMKTIenda)
        If miInterfaz IsNot Nothing Then miInterfaz.Commit(True, cod, txtTienda.Text, "-")
        objPleaseWait.Close()
        Close()

    End Sub

    Sub GrabarVentaCasoEspecial(GRID As GridGroupingControl, envio As EnvioImpresionVendedorPernos)
        Dim VentaSA As New documentoVentaAbarrotesSA
        ...
        End If
        Dim miInterfaz As ICommitOperacionMKTIenda = TryCast(Me.Owner, ICommitOperacionMKTIenda)
        If miInterfaz IsNot Nothing Then miInterfaz.Commit(True, lista(0).idDocumento, txtTienda.Text, FormaPago)
        objPleaseWait.Close()
        btGrabar.Enabled = True
        Close()
    End Sub
    ...
    Private Function GetGrabarVentaComprobante(DetalleVenta As List(Of documentoventaAbarrotesDet), envio As EnvioImpresionVendedorPernos) As documento
        ...
        nDocumentoVenta = New documentoventaAbarrotes With {
            .horaVenta = txtFecha.Value.TimeOfDay.ToString(),
                  ...
                  .terminos = "CONTADO",
                  .glosa = "Por ventas",
                  .fechaVcto = txtFechaVencimiento.Value,
                  .nroGuia = TextNumeroGuia.Text,
                  .nroOrdenVenta = TextOrdenventa.Text,
        ...
        ElseIf ChPagoAvanzado.Checked = True Then
            ...
            '    If c.Count > 0 Then
            Dim ListaPagos = ListaPagosCajas(ndocumento.documentoventaAbarrotes, ndocumento.documentoventaAbarrotes.documentoventaAbarrotesDet, envio)
    ...
    Public Function AddPagoCuponCaja(venta As documentoventaAbarrotes, ventaDetalle As List(Of documentoventaAbarrotesDet), envio As EnvioImpresionVendedorPernos) As documento
    ...
    Public Function ListaPagosCajas(venta As documentoventaAbarrotes, ventaDetalle As List(Of documentoventaAbarrotesDet), envio As EnvioImpresionVendedorPernos) As List(Of documento)
        ...
        If PanelCupon.Visible Then
            If TextCuponImporte.DecimalValue > 0 Then
                ListaDoc.Add(AddPagoCuponCaja(venta, ventaDetalle, envio))
            End If
        End If
    ...
    Private Sub ListaDetalleCajaGenerico(doc As documentoCaja, detalleVenta As List(Of documentoventaAbarrotesDet), envio As EnvioImpresionVendedorPernos)
    ...
    Sub GrabarNotaDeVenta(dgvCompra As GridGroupingControl, envio As EnvioImpresionVendedorPernos)
        'objPleaseWait = New FeedbackForm()
        'objPleaseWait.StartPosition = FormStartPosition.CenterScreen
        'objPleaseWait.Show()
        Application.DoEvents()
        ...
        If ListaDetalle.Where(Function(o) o.monto1 = 0).Count = 0 Then
            ...
            Dim miInterfaz As ICommitOperacionMKTIenda = TryCast(Me.Owner, ICommitOperacionMKTIenda)
            If miInterfaz IsNot Nothing Then miInterfaz.Commit(True, idDocuentoGrabado, txtTienda.Text, FormaPago)
            objPleaseWait.Close()
            btGrabar.Enabled = True
            Close()
    ...
    Private Sub GetUbicarClienteGeneral()
        ...
        'cargar documento confirmado
        cboTipoDoc.Text = formventa.ComboComprobante.Text
        TXTcOMPRADOR.Text = formventa.TXTcOMPRADOR.Text
        TXTcOMPRADOR.Tag = formventa.TXTcOMPRADOR.Tag
        txtruc.Text = formventa.txtruc.Text
        If formventa.TXTcOMPRADOR.ForeColor = Color.FromKnownColor(KnownColor.HotTrack) Then
            TXTcOMPRADOR.ForeColor = Color.FromKnownColor(KnownColor.HotTrack)
        End If
        If IsNumeric(ComboCaja.SelectedValue) Then
            GetMappingColumnsGridByUsuario(Integer.Parse(ComboCaja.SelectedValue))
    ...
    Private Sub FormConfirmaVenta_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
    ...
    Private Sub ChPagoAvanzado_OnChange(sender As Object, e As EventArgs) Handles ChPagoAvanzado.OnChange
        ...
        Else
            ...
            txtFechaVencimiento.Value = Date.Now.AddDays(30)
    ...
    '    '                                     .idEstablecimiento = GEstableciento.IdEstablecimiento
    '    '                                     })
    '    'listaCuentas.Where(Function(o) o.IDFormaPago <> "9991").ToList
        ...
        Try
            ...
            Select Case cboTipoDoc.Text
                ...
                Case "BOLETA ELECTRONICA", "BOLETA"
                    ...
                    If TXTcOMPRADOR.Tag = VarClienteGeneral.idEntidad Then

                    Else
                        Dim rsp = validarDNIRUC(txtruc.Text.Trim)
                        If rsp = False Then
                ...
                If ValidarGrabado() = True Then
                    ...
                    Select Case cboTipoDoc.Text
                        Case "BOLETA", "FACTURA"
                            ...
                            GrabarVentaCasoEspecial(formventa.dgvCompra, envio)
                        ...
                        Case "NOTA DE VENTA"
                            GrabarNotaDeVenta(formventa.dgvCompra, envio)
                        Case "PROFORMA"
    ...
    Private Sub LinkLabel1_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LinkLabel1.LinkClicked
        Try
            If TXTcOMPRADOR.Tag IsNot Nothing Then
                If TXTcOMPRADOR.Text.Trim.Length > 0 Then
                    'Dim f As New FormAntReclamacionesPendientes(New entidad With {
                    '                                            .idEntidad = Val(TXTcOMPRADOR.Tag),
                    '                                            .nombreCompleto = TXTcOMPRADOR.Text,
                    '                                            .nrodoc = txtruc.Text
                    '                                            })
                    'f.StartPosition = FormStartPosition.CenterParent
                    'f.ShowDialog(Me)
    ...
    Private Sub ComboFechaVencimiento_SelectedValueChanged(sender As Object, e As EventArgs) Handles ComboFechaVencimiento.SelectedValueChanged
        Select Case ComboFechaVencimiento.Text
            Case "30 DIAS"
                txtFechaVencimiento.Value = Date.Now.AddDays(30)
            Case "60 DIAS"
                txtFechaVencimiento.Value = Date.Now.AddDays(60)
            Case "90 DIAS A MAS"
                txtFechaVencimiento.Value = Date.Now.AddDays(90)
        End Select
    End Sub
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        If TXTcOMPRADOR.Tag IsNot Nothing Then
            ...
            If Not TXTcOMPRADOR.Tag = VarClienteGeneral.idEntidad Then

                Dim f As New frmCrearENtidades(CInt(TXTcOMPRADOR.Tag))
                f.CaptionLabels(0).Text = "Editar Cliente"
                f.strTipo = TIPO_ENTIDAD.CLIENTE
                '   f.ManipulacionEstado = ENTITY_ACTIONS.UPDATE 
                f.intIdEntidad = TXTcOMPRADOR.Tag
                ...
                GetClientes2(General.TIPO_ENTIDAD.CLIENTE, General.Gempresas.IdEmpresaRuc)

                Dim ClienteEdit = (From i In listaClientes
                                   Where i.idEntidad = TXTcOMPRADOR.Tag).FirstOrDefault


                txtruc.ForeColor = Color.FromKnownColor(KnownColor.HotTrack)
                TXTcOMPRADOR.ForeColor = Color.FromKnownColor(KnownColor.HotTrack)
                txtruc.Text = ClienteEdit.nrodoc
                TXTcOMPRADOR.Text = ClienteEdit.nombreCompleto
                TXTcOMPRADOR.Tag = ClienteEdit.idEntidad
