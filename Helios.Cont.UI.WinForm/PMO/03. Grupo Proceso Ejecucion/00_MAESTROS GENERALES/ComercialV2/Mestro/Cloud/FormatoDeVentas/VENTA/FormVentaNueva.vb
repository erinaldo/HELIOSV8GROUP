...
#Region "Attributes"
    ...
    Public UCEstructuraCabeceraVentaV2 As UCEstructuraCabeceraVentaV2
    Public UCCondicionesPago As UCPagoVenta
...
#Region "Constructors"
    ...
    Public Sub New(CodigoProforma As String)
        ...
        UCCondicionesPago = New UCPagoVenta(Me)
    ...
    Public Sub New(tipo As String, order As String)

        ' This call is required by the designer.
        InitializeComponent()

        ' Add any initialization after the InitializeComponent() call.
        If tipo = "OTRA SALIDA DE ALMACEN" Then
            GetComboPrincipalSalida()
        ElseIf tipo = "PRE VENTA" Then
            GetComboPrincipalPreventa()
        ElseIf tipo = "COTIZACION" Then
            GetComboPrincipalCotizacion()
        End If
...
#Region "Methods"
    ...
    Private Sub GetComboPrincipalSalida()
        ...
        'ComboComprobante.Items.Add("COTIZACION")
        'ComboComprobante.Items.Add("PRE VENTA")
        ComboComprobante.Items.Add("OTRA SALIDA DE ALMACEN")
        'ComboComprobante.Items.Add("TRANSFERENCIA ENTRE ALMACENES")
    End Sub


    Private Sub GetComboPrincipalCotizacion()
        ComboComprobante.Items.Clear()
    ...
    Private Sub GetComboPrincipalPreventa()
        ComboComprobante.Items.Clear()
...
If(i.CustomProducto.tipoOtroImpuesto = "ICBPER", "ICBPER", "-"), i.CustomProducto.otroImpuesto.GetValueOrDefault, i.CustomProducto.otroImpuesto.GetValueOrDefault)
    ...
    Private Sub VerCabeceraDocumento(venta As documentoventaAbarrotes, ent As entidad)
        ...
        Select Case venta.tipoVenta
            ...
            Case TIPO_VENTA.COTIZACION
                ComboComprobante.Text = "COTIZACION"
        End Select
    ...
    Public Function ListaPagosCajas(venta As documentoventaAbarrotes, ventaDetalle As List(Of documentoventaAbarrotesDet), envio As EnvioImpresionVendedorPernos) As List(Of documento)
        ...
        For Each i In UCCondicionesPago.UCPagoCompletoDocumento.GridCompra.Table.Records
            If Decimal.Parse(i.GetValue("monto")) > 0 Then
                ...
                'If i.GetValue("tipoEntidad") = "BC" Then
                ...
                '    'objCaja.tipoEntidadFinanciera = i.GetValue("tipoEntidad")


                '    objCaja.entidadFinancieraDestino = Integer.Parse(i.GetValue("idCuenta"))
                '    objCaja.NombreEntidad = i.GetValue("Cuenta")
                '    objCaja.idCajaUsuarioDestino = Integer.Parse(i.GetValue("idCajaUsuario")) 'envio.IDCaja
                '    objCaja.tipoEntidadFinanciera = i.GetValue("tipoEntidad")
                '    objCaja.fechaProcesoDestino = venta.fechaDoc


                'Else ' si es caja pos o adm efectivo
                ...
                If usuario.tipoCaja = "POS" Then
                    ...
                    If i.GetValue("tipoEntidad") = "BC" Or i.GetValue("tipoEntidad") = "TC" Then
                        objCaja.entidadFinancieraDestino = Integer.Parse(i.GetValue("idCuenta"))
                        objCaja.idCajaUsuarioDestino = Integer.Parse(i.GetValue("idCajaUsuario")) 'envio.IDCaja
                        objCaja.tipoEntidadFinanciera = i.GetValue("tipoEntidad")
                        objCaja.fechaProcesoDestino = venta.fechaDoc
                        objCaja.confirmacionOperacion = "NO"
                        objCaja.numeroOperacion = i.GetValue("nrooper")
                    Else
                        objCaja.confirmacionOperacion = "SI"
                        objCaja.fechaconfirmacionOperacion = DateTime.Now
                    End If

                Else
                    objCaja.entidadFinancieraDestino = Integer.Parse(i.GetValue("idCuenta"))
                    objCaja.NombreEntidad = i.GetValue("Cuenta")
                    objCaja.idCajaUsuarioDestino = envio.IDCaja
                    objCaja.tipoEntidadFinanciera = i.GetValue("tipoEntidad") '"EF"
                    objCaja.fechaProcesoDestino = venta.fechaDoc
                    If i.GetValue("tipoEntidad") = "EF" Then
                        objCaja.idRol = usuario.IDRol
                        objCaja.IdUsuarioTransaccion = usuario.IDUsuario
                        objCaja.confirmacionOperacion = "SI"
                        objCaja.fechaconfirmacionOperacion = DateTime.Now
                    ElseIf i.GetValue("tipoEntidad") = "BC" Then
                        objCaja.confirmacionOperacion = "NO"
                        objCaja.numeroOperacion = i.GetValue("nrooper")
                    End If
                End If
                    ...
                    If i.MontoSaldo > 0 Then
                        ...
                        If UCEstructuraCabeceraVentaV2.cboMoneda.SelectedValue = "1" Then
                            GetDetallePago.Add(New documentoCajaDetalle With
                                       ...
                                       .montoUsd = Math.Round(CDec(i.MontoPago / objCaja.tipoCambio), 2),'0,'(i.MontoPagoME + i.montoIcbper.GetValueOrDefault),' FormatNumber((i.MontoPago + i.montoIcbper.GetValueOrDefault) / TmpTipoCambio, 2),
    ...
    Private Sub MappingDocumentoCompraCabeceraDetalle(obj As documento)
        ...
        For Each i In UCEstructuraCabeceraVentaV2.ListaproductosVendidos
            ...
            Select Case i.tipoExistencia
                ...
                Case Else
                    If i.CustomEquivalencia.contenido_neto <= 0 Then 'fraccionUnidad <= 0 Then
                    ...
                    objDet = New documentoventaAbarrotesDet With
                            ...
                            .unidad2 = Nothing,
                            .monto2 = i.PrecioUnitarioVentaMN,
                            .precioUnitario = i.monto1 * i.CustomEquivalencia.contenido_neto,'fraccionUnidad,'   i.precioUnitario.GetValueOrDefault,
                            .precioUnitarioUS = i.precioUnitarioUS.GetValueOrDefault,
                            .importeMN = i.importeMN,
                            .importeME = i.importeME.GetValueOrDefault,
                            .montokardex = i.montokardex,
                            ...
                            .usuarioModificacion = obj.usuarioActualizacion,' usuario.IDUsuario,
                            .fechaModificacion = Date.Now,
                            .idLote = i.idLote.GetValueOrDefault,
                            .idLoteDetalle = i.idLoteDetalle.GetValueOrDefault
                            }
    ...
    Private Function ValidarForm() As Boolean
        ...
        Else
            If ComboComprobante.Text = "VENTA" Then

                If UCEstructuraCabeceraVentaV2.TextSerie.Text.Trim.Length = 0 Then
                    ErrorProvider1.SetError(UCEstructuraCabeceraVentaV2.TextSerie, "Ingrese la serie")
                    listaErrores += 1
                Else
                    ErrorProvider1.SetError(UCEstructuraCabeceraVentaV2.TextSerie, Nothing)
                End If

                If UCEstructuraCabeceraVentaV2.TextNumber.Text.Trim.Length = 0 Then
                    ErrorProvider1.SetError(UCEstructuraCabeceraVentaV2.TextNumber, "Ingrese el número de venta")
                    listaErrores += 1
                Else
                    ErrorProvider1.SetError(UCEstructuraCabeceraVentaV2.TextNumber, Nothing)
                End If
        ...
        Select Case ComboComprobante.Text
            ...
            Case Else
                If CDec(UCEstructuraCabeceraVentaV2.txtTotalPagar.Value) <= 0 Then
    ...
    Private Sub MappingDocumentoCompraCabecera(be As documento)
        ...
        Select Case be.moneda
            Case "1"
                ...
                total = CDec(UCEstructuraCabeceraVentaV2.txtTotalPagar.Value)
            ...
            Case "2"
                ...
                base1ME = UCEstructuraCabeceraVentaV2.txtTotalBaseME.DecimalValue
                base2ME = UCEstructuraCabeceraVentaV2.txtTotalBase2ME.DecimalValue

                base1 = UCEstructuraCabeceraVentaV2.txtTotalBase.DecimalValue
                base2 = UCEstructuraCabeceraVentaV2.txtTotalBase2.DecimalValue

                iva1ME = UCEstructuraCabeceraVentaV2.txtTotalIvaME.DecimalValue
                iva1 = UCEstructuraCabeceraVentaV2.txtTotalIva.DecimalValue

                totalME = CDec(UCEstructuraCabeceraVentaV2.DigitalME.Value)
                total = CDec(UCEstructuraCabeceraVentaV2.txtTotalPagar.Value)

                icbperME = UCEstructuraCabeceraVentaV2.txtTotalIcbperME.DecimalValue
                icbper = UCEstructuraCabeceraVentaV2.txtTotalIcbper.DecimalValue
        End Select
        ...
        Dim estadoEntrega = ""
        Dim ExistenPendientesDeTraslado = UCEstructuraCabeceraVentaV2.ListaproductosVendidos.Any(Function(o) o.estadoEntrega = EstadoTrasladoVenta.Pedido)

        If ExistenPendientesDeTraslado Then
        ...
        .descripcionVenta = "Observaciones: Se entregarán los productos una vez culmine la  cuarentena.",
        ...
        Else
            If ComboComprobante.Text = "VENTA" Then
                obj.serieVenta = UCEstructuraCabeceraVentaV2.TextSerie.Text.Trim
                obj.numeroVenta = UCEstructuraCabeceraVentaV2.TextNumber.Text.Trim
            End If
    ...
    Private Sub Grabarventa()
        ...
        Try
            ...
            Select Case ComboComprobante.Text
                Case "OTRA SALIDA DE ALMACEN", "TRANSFERENCIA ENTRE ALMACENES", "COTIZACION", "PRE VENTA"


                    Dim Vendedor As New Seguridad.Business.Entity.Usuario
                    If ValidarTipoPedido() = 1 Then
                    ...
                    ElseIf ValidarTipoPedido() = 2 Then

                        Vendedor = (From i In UsuariosList Where i.IDUsuario = usuario.IDUsuario).FirstOrDefault
                ...
                Case Else
                    ...
                    ElseIf ValidarTipoPedido() = 2 Then
                        ...
                        End If
                    ElseIf ValidarTipoPedido() = 3 Then
                        Vendedor = (From i In UsuariosList Where i.IDUsuario = usuario.IDUsuario).FirstOrDefault
                        If Vendedor Is Nothing Then
                            Throw New Exception("Usuario no Valido")
                        End If
            ...
            If validarCanastaDeVentas(obj) Then
                ...
                If UCEstructuraCabeceraVentaV2.cboTipoDoc.Text = "FACTURA" Or UCEstructuraCabeceraVentaV2.cboTipoDoc.Text = "BOLETA" Then
                    ...
                    Else
                        ...
                        'Alert.Show()
                    End If
                ElseIf UCEstructuraCabeceraVentaV2.cboTipoDoc.Text = "COTIZACION" Or UCEstructuraCabeceraVentaV2.cboTipoDoc.Text = "NOTA DE VENTA" Or UCEstructuraCabeceraVentaV2.cboTipoDoc.Text = "NOTA" Then
                    Dim comprobante = ventaSA.GetVentaID(New documento With {.idDocumento = doc.idDocumento})
                    doc.documentoventaAbarrotes = comprobante
                    doc.ListaCustomDocumento = ListaPagos
                    FormImpresionNuevo = New FormImpresionEquivalencia(doc)  ' frmVentaNuevoFormato
...
#End Region
        ...
        UCEstructuraCabeceraVentaV2.TextFiltrar.Select()
        UCEstructuraCabeceraVentaV2.TextFiltrar.Focus()
        UCEstructuraCabeceraVentaV2.GetConfiguracionEmpresa()
    End Sub
...
#Region "Events"
    ...
    Sub IrPago()
        ...
        If UCCondicionesPago IsNot Nothing Then
            ...
            'Else
            UCCondicionesPago.UCPagoCompletoDocumento.TextCompraTotal.DecimalValue = CDec(UCEstructuraCabeceraVentaV2.txtTotalPagar.Value)
            UCCondicionesPago.UCPagoCompletoDocumento.TextCompraTotalME.DecimalValue = CDec(UCEstructuraCabeceraVentaV2.DigitalME.Value)
            'End If
            ...
            If UCCondicionesPago.RBPagoAcumulado.Checked = True Then
                If UCEstructuraCabeceraVentaV2.cboMoneda.Text = "NUEVO SOL" Then
                    UCCondicionesPago.UCPagoCompletoDocumento.LabelTotalCobrarCliente.Text = CDec(UCEstructuraCabeceraVentaV2.txtTotalPagar.Value)
                Else
                    UCCondicionesPago.UCPagoCompletoDocumento.LabelTotalCobrarCliente.Text = CDec(UCEstructuraCabeceraVentaV2.DigitalME.Value)
                End If
            ...
            Case "VENTA"
                ...
                If UCEstructuraCabeceraVentaV2 IsNot Nothing Then
                    If Gempresas.ubigeo > 0 Then
                        ...
                        UCEstructuraCabeceraVentaV2.TextNumber.Visible = False
                        UCEstructuraCabeceraVentaV2.TextSerie.Visible = False
                        ToggleButton1.ToggleState = ToggleButtonState.Active
                        ToggleButton1.Visible = True
                        ToggleButton1.Enabled = True

                    ElseIf Gempresas.ubigeo = 0 Then
                        UCEstructuraCabeceraVentaV2.TextNumber.Visible = False
                        UCEstructuraCabeceraVentaV2.TextSerie.Visible = False
                        ToggleButton1.Visible = True
                        ToggleButton1.ToggleState = ToggleButtonState.Inactive
                        ToggleButton1.Enabled = False
                        UCEstructuraCabeceraVentaV2.TextNumber.Visible = True
                        UCEstructuraCabeceraVentaV2.TextSerie.Visible = True
                        UCEstructuraCabeceraVentaV2.TextNumber.Text = ""
                        UCEstructuraCabeceraVentaV2.TextSerie.Text = ""
                    End If
    ...
    Private Sub ToolStripButton5_Click(sender As Object, e As EventArgs) Handles ToolStripButton5.Click
        Dim f As New FormTipoBusqueda(Me.UCEstructuraCabeceraVentaV2)
        f.StartPosition = FormStartPosition.CenterParent
        f.ShowDialog(Me)
    End Sub

    Private Sub ToggleButton1_ToggleStateChanged(sender As Object, e As Syncfusion.Windows.Forms.Tools.ToggleStateChangedEventArgs) Handles ToggleButton1.ToggleStateChanged
        If UCEstructuraCabeceraVentaV2 IsNot Nothing Then
            If e.ToggleState = ToggleButtonState.Active Then
                UCEstructuraCabeceraVentaV2.TextNumber.Visible = False
                UCEstructuraCabeceraVentaV2.TextSerie.Visible = False
                UCEstructuraCabeceraVentaV2.lblSerieFisica.Visible = False
                UCEstructuraCabeceraVentaV2.lblNumerofisica.Visible = False
            ElseIf e.ToggleState = ToggleButtonState.Inactive Then
                UCEstructuraCabeceraVentaV2.TextNumber.Clear()
                UCEstructuraCabeceraVentaV2.TextNumber.Visible = True
                UCEstructuraCabeceraVentaV2.TextSerie.Visible = True
                UCEstructuraCabeceraVentaV2.lblSerieFisica.Visible = True
                UCEstructuraCabeceraVentaV2.lblNumerofisica.Visible = True
            End If
        End If
    End Sub
    ...
    Private Sub ToggleButton1_Click(sender As Object, e As EventArgs) Handles ToggleButton1.Click
