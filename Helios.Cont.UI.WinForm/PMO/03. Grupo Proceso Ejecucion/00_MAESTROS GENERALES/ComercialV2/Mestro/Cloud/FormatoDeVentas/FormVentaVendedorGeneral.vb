...
Public Class FormVentaVendedorGeneral
    Implements IForm, IPrecio, IListaInventario, IProductoConsignado,
        IOferta, ICommitOperacionMKTIenda, IListaServicio
...
#Region "Constructors"
    Public Sub New()
        ...
        Me.KeyPreview = True
        'Me.DockingClientPanel1.BorderStyle = System.Windows.Forms.BorderStyle.
        dockingManager1.VisualStyle = Syncfusion.Windows.Forms.VisualStyle.VS2010
        dockingManager1.ShowCaption = True
        dockingManager1.SetDockLabel(PanelBenficios, "Beneficios")
        dockingManager1.DockControlInAutoHideMode(PanelBenficios, DockingStyle.Left, 270)
        dockingManager1.SetDockLabel(PanelPedidosReservados, "Reservas")
        dockingManager1.DockControlInAutoHideMode(PanelPedidosReservados, DockingStyle.Left, 270)
        Me.dockingManager1.DockControl(Me.PanelBenficios, PanelPedidosReservados, Syncfusion.Windows.Forms.Tools.DockingStyle.Tabbed, 200)
        ' dockingManager1.DockControl(PanelPedidosReservados, DockingClientPanel1, DockStyle.Bottom, 80)
        ...
        'CalculoGridCeldas()
        GetTableGrid()
        GetTableGridBeneficios()
        threadClientes()

        '   If Not tmpConfigInicio.FormatoVenta = "MKT" Then
        ...
        ListaventasEnEspera = New List(Of documentoventaAbarrotes)

        '//prueba maykol del datagrid
        Me.dgvCompra.TableDescriptor.Columns(5).Appearance.AnyCell.WrapText = True
        'the size set evenly according to the content of cells.
        Me.dgvCompra.TableModel.RowHeights.ResizeToFit(GridRangeInfo.Table())
        'Event hooking
        AddHandler dgvCompra.TableModel.QueryRowHeight, AddressOf TableModel_QueryRowHeight
...
#End Region
    ...
    Private Sub UbicarClienteGeneral()
        ComboComprobante.Enabled = True
...
#Region "Methods"
    ...
    Sub GetToolTipControls()
        toolTipInfo1.Body.Size = New System.Drawing.Size(20, 20)
        toolTipInfo1.Body.Text = "Canasta de productos"
        ...
        Me.SuperToolTip1.SetToolTip(ToolStripButton1, toolTipInfo1)
    End Sub
    ...
    Private Function GetDetalleBeneficios() As List(Of documentoventaAbarrotesDet)
        ...
        For Each r As Record In GridBeneficios.Table.Records
            ...
            objDocumentoVentaDet.fechaVcto = Date.Now
    ...
    Public Sub GetDocumentoVentaIDDone()
        ...
        With documentoVenta
            ...
            LabelFecha.Text = String.Format("{0}: {1}", "Fecha", .fechaDoc)
            '    txtNumero.Visible = True
            Dim codigoComprobante = .tipoDocumento
            Select Case codigoComprobante
                Case "12.1", "03"
                    ComboComprobante.Text = "BOLETA"
                    LabelComprobante.Text = String.Format("{0}: {1}", "Comprobante", "BOLETA")
                Case "12.2", "01"
                    ComboComprobante.Text = "FACTURA"
                    LabelComprobante.Text = String.Format("{0}: {1}", "Comprobante", "FACTURA")
                Case "9907"
                    ComboComprobante.Text = "NOTA DE VENTA"
                    LabelComprobante.Text = String.Format("{0}: {1}", "Comprobante", "NOTA DE VENTA")
                Case "9901"
                    ComboComprobante.Text = "PROFORMA"
                    LabelComprobante.Text = String.Format("{0}: {1}", "Comprobante", "PROFORMA")
            End Select
            ...
            '            If inicioComprobante = "PROFORMA" Then
            '                GroupBox1.Enabled = False
            '                'txtSerie.Text = .serie
            '                'txtNumero.Text = .numeroDoc

            '                LabelNumero.Text = String.Format("{0}: {1}-{2}", "Número", .serie, .numeroDoc)
            '            Else
            '                'txtSerie.Text = .serieVenta
            '                'txtNumero.Text = .numeroVenta

            '                LabelNumero.Text = String.Format("{0}: {1}-{2}", "Número", .serieVenta, .numeroVenta)
            '            End If
            '        Else
            '            'txtSerie.Text = .serieVenta
            '            'txtNumero.Text = .numeroVenta
            ...
            ' Case Else
            'If ComboComprobante.Text = "PROFORMA" Then
            ...
            dgvCompra.TableDescriptor.Columns("pumn").Width = 70
            ...
            If Not IsNothing(entidad) Then
                ...
                'TXTcOMPRADOR.ForeColor = Color.FromKnownColor(KnownColor.HotTrack)
                'txtruc.Visible = True
                '      LabelCliente.Text = String.Format("{0}: {1}", "Cliente", TXTcOMPRADOR.Text)
            Else
                '  TXTcOMPRADOR.Text = .nombrePedido
                '  LabelCliente.Text = String.Format("{0}: {1}", "Cliente", TXTcOMPRADOR.Text)
            End If
            txtGlosa.Text = .glosa
            LabelMoneda.Text = String.Format("{0}: {1}", "Moneda", "NUEVO SOL")
        End With
    ...
    Private Sub GetImpresionTicketsEspecialNota(idDocumento As Integer, nombreTienda As String, FormaPago As String)
        Dim ventaSA As New documentoVentaAbarrotesSA
            ...
            If My.Computer.Network.Ping("148.102.27.231") Then
                If Gempresas.ubigeo > 0 Then
                    If ComboComprobante.Text = "FACTURA ELECTRONICA" Then
                        ...
                        EnviarFacturaElectronica(idDocumento, Gempresas.ubigeo)
    ...
    '        nDocumentoCaja.tipoDoc = conf.TipoComprobante ' ComboComprobantePago.SelectedValue
    ...
    '        objCaja.TipoDocumentoPago = conf.TipoComprobante 'ComboComprobantePago.SelectedValue
    ...
    Sub ConfiguracionInicio()
        ...
        '  ComboComprobantePago.Enabled = True
        txtGlosa.Text = "Por la venta según " & ComboComprobante.Text
    End Sub
    ...
    'Sub TotalTalesIncluidoDescuentos()
    '    '  Dim totalNotaMN As Decimal = 0
    '    Dim totalNotaME As Decimal = 0
    '    Dim totalpercepMN As Decimal = 0
    '    Dim totalpercepME As Decimal = 0
    ...
    '    TextTotalDescuentos.DecimalValue = TotalesXcanbeceras.PercepcionMN
    ...
    '            DigitalGauge1.Text = total.ToString("N2")
    '            DigitalGauge1.Value = total.ToString("N2")
                ...
                Case "GS"
                    ...
                    Select Case colDestinoGravado
                        Case 1
                            ...
                            Me.dgvCompra.Table.CurrentRecord.SetValue("totalpagar", Math.Round(totalMN, 2))
...
#Region "Events"
                            ...
                            If dgvCompra.Table.Records.Count > 0 Then
                                Select Case ComboComprobante.Text
                                    Case "PROFORMA"
                                        GrabarProforma()
                                    Case Else
                                        Dim f As New FormConfirmrarVentaVendedor(dgvCompra, TotalesXcanbeceras, Me)
                                        'If RBVenta.Checked = True Then
                                        '    f.GetDocsVenta()
                                        'ElseIf RBProforma.Checked = True Then
                                        '    f.GetDocProforma()
                                        'End If

                                        f.StartPosition = FormStartPosition.CenterParent
                                        f.txtTotalBase.DecimalValue = txtTotalBase.DecimalValue
                                        f.txtTotalBase2.DecimalValue = txtTotalBase2.DecimalValue
                                        f.txtTotalBase3.DecimalValue = txtTotalBase3.DecimalValue
                                        f.txtTotalIva.DecimalValue = txtTotalIva.DecimalValue
                                        f.TextTotalDescuentos.DecimalValue = TextTotalDescuentos.DecimalValue
                                        f.txtTotalPagar.DecimalValue = txtTotalPagar.DecimalValue
                                        f.CompradorID = TextComprador.Text
                                        f.ShowDialog(Me)
                                End Select
    ...
    Sub GrabarProforma()
        ...
        EliminarReserva(TextComprador.Text.Trim)
        FillListViewPedidos()
        LimpiarControles()
        Alert = New Alert("Proforma registrada", alertType.success)
        Alert.TopMost = True
        Alert.Show()
        GetImpresionTicketsEspecialNota(cod, "", "")
        ToolStripButton1.Select()
    End Sub

    Private Sub GrabarPedido(Vendedor As Helios.Seguridad.Business.Entity.Usuario)
        Dim VentaSA As New documentoVentaAbarrotesSA
        Dim ndocumento As New documento()
        Dim DocCaja As New documento
        ...
        proveedor = TXTcOMPRADOR.Text '¨Pedido"
        idProveedor = TXTcOMPRADOR.Tag ' VarClienteGeneral.idEntidad
        ...
        Else
            ndocumento.idProyecto = GProyectos.IdProyectoActividad
        End If
        ndocumento.tipoDoc = tipoComprobante '"9907"
        ndocumento.fechaProceso = fechaSel
        ndocumento.nroDoc = GConfiguracion.Serie
        ndocumento.idOrden = Nothing ' Me.IdOrden
        ndocumento.moneda = "1"
        ndocumento.idEntidad = Val(idProveedor)
        ...
        nDocumentoVenta = New documentoventaAbarrotes With {
            .IdDocumentoCotizacion = False,
                 .TipoConfiguracion = If(GConfiguracion Is Nothing, Nothing, GConfiguracion.TipoConfiguracion),
                  ...
                  .tipoOperacion = "01",
                  .codigoLibro = "14",
                  .tipoDocumento = tipoComprobante,' "9907",
                  .idEmpresa = Gempresas.IdEmpresaRuc,
                  .idEstablecimiento = GEstableciento.IdEstablecimiento,
                  .fechaDoc = fechaSel,
                  .fechaPeriodo = GetPeriodo(fechaSel, True),
                  .serie = GConfiguracion.Serie,
                  .numeroDocNormal = Nothing,
                  .idCliente = idProveedor,
        ...
        For Each r As Record In dgvCompra.Table.Records
            ...
            objDocumentoVentaDet.IdEmpresa = Gempresas.IdEmpresaRuc
            objDocumentoVentaDet.IdEstablecimiento = GEstableciento.IdEstablecimiento ' almacenSA.GetUbicar_almacenPorID(i.Cells(24).Value()).idEstablecimiento
            objDocumentoVentaDet.FechaDoc = fechaSel
            objDocumentoVentaDet.Serie = GConfiguracion.Serie
            objDocumentoVentaDet.NumDoc = Nothing
        ...
        Dim cod = VentaSA.GrabarCotizacion(ndocumento)
        MessageBox.Show("Pedido registrado", "Done!", MessageBoxButtons.OK, MessageBoxIcon.Information)
        EliminarReserva(TextComprador.Text.Trim)
        FillListViewPedidos()
        LimpiarControles()
        ...
        Try
            ...
            Else
                ...
                With dgvCompra.Table
                    ...
                    .CurrentRecord.SetValue("puKardex", totalMN / productoBE.cantidad)
                    .CurrentRecord.SetValue("pukardeme", totalME / productoBE.cantidad)
                    ...
                    .CurrentRecord.SetValue("empresa", Gempresas.IdEmpresaRuc)
                    .CurrentRecord.SetValue("tipoventa", "V")
                    .CurrentRecord.SetValue("cantidad2", productoBE.cantidad)
...
#End Region
    ...
    Public Sub UbicarDocumentoImportado(documento As documento)
        ...
        With documento.documentoventaAbarrotes
            ...
            If entidad.ToString.Trim.Length > 0 Then
                ...
                'TXTcOMPRADOR.Text = entidadSel.FirstOrDefault.nombreCompleto
                'TXTcOMPRADOR.ForeColor = Color.FromKnownColor(KnownColor.HotTrack)
                'txtruc.Visible = True
                'TXTcOMPRADOR.Tag = entidadSel.FirstOrDefault.idEntidad
            Else
                '  TXTcOMPRADOR.Text = .nombrePedido
            End If
            ...
            '    TXTcOMPRADOR.Text = entidad.FirstOrDefault.nombreCompleto
            '    TXTcOMPRADOR.ForeColor = Color.FromKnownColor(KnownColor.HotTrack)
            '    txtruc.Visible = True
            '    TXTcOMPRADOR.Tag = entidad.FirstOrDefault.idEntidad
            'Else
        ...
        For Each i In documento.documentoventaAbarrotes.documentoventaAbarrotesDet.ToList
            ...
            Me.dgvCompra.Table.CurrentRecord.SetValue("cantidad", 0)
            Me.dgvCompra.Table.CurrentRecord.SetValue("vcmn", 0)
            Me.dgvCompra.Table.CurrentRecord.SetValue("totalmn", 0) 'i.importeMN)

            Me.dgvCompra.Table.CurrentRecord.SetValue("totalpagar", 0) 'i.importeMN)
            Me.dgvCompra.Table.CurrentRecord.SetValue("vcme", 0)
            Me.dgvCompra.Table.CurrentRecord.SetValue("totalme", 0) 'i.importeME)
            Me.dgvCompra.Table.CurrentRecord.SetValue("igvmn", 0)
            Me.dgvCompra.Table.CurrentRecord.SetValue("igvme", 0)
            Me.dgvCompra.Table.CurrentRecord.SetValue("tipoExistencia", i.tipoExistencia)
            'Me.dgvCompra.Table.CurrentRecord.SetValue("marca", If(lote.productoSustentado = True, "Doc.", "Not."))
    ...
    Public Sub Commit(Confirmado As Boolean, idDocumento As Integer, nombreTienda As String, FormaPago As String) Implements ICommitOperacionMKTIenda.Commit
        If Confirmado = True Then
            'Me.KeyPreview = False
            GetImpresionTicketsEspecialNota(idDocumento, nombreTienda, FormaPago)
...
#Region "VentaPruebas"
        'Dim f As New FormCanstaVentaEquivalencia()
        'f.StartPosition = FormStartPosition.CenterParent
        'f.ShowDialog(Me)
#End Region
        ...
        ''''''With FormInventarioCanastaTotales
        ''''''    .CheckSinInventario.Checked = True
        ''''''    .monedaVenta = MonedaSel
        ''''''    .validaSelPrecioVenta = True
        ''''''    .chCantidadPrevia.Checked = True
        ''''''    .validaStocks = valida ' True
    ...
    'Private Sub TXTcOMPRADOR_TextChanged_1(sender As Object, e As EventArgs)
    ...
    Private Sub chPedido_CheckedChanged(sender As Object, e As EventArgs) Handles chPedido.CheckedChanged
        'If chPedido.Checked = True Then
        '    GroupBox1.Visible = False
        '    btGrabar.Text = "Guardar - F2"
        '    PanelCliSel.Size = New Size(577, 36)
        '    Panel2.Visible = True
        'Else
        '    GroupBox1.Visible = True
        '    btGrabar.Text = "Cobrar - F2"
        '    PanelCliSel.Size = New Size(0, 36)
    ...
    Private Function ValidarPedidoExistente(Comprador As String) As Boolean
        ValidarPedidoExistente = True
        Dim pedido = ListaventasEnEspera.Where(Function(o) o.nombrePedido = Comprador).FirstOrDefault
        If pedido IsNot Nothing Then
            If MessageBox.Show("El nombre del comprador ya se encuentrá registrado, desea reemplazarlo?", "Atención", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                EliminarReserva(pedido.nombrePedido)
                ValidarPedidoExistente = True
            Else
                ValidarPedidoExistente = False
            End If
        End If
    End Function
    ...
    Private Sub RoundButton21_Click(sender As Object, e As EventArgs) Handles RoundButton21.Click
        ...
        Try
            If TextComprador.Text.Trim.Length = 0 Then
                MessageBox.Show("Debe indicar el nombre del comprador!", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If

            If dgvCompra.Table.Records.Count > 0 Then
                If ValidarPedidoExistente(TextComprador.Text.Trim) Then
                    Dim idCliente As Integer
                    Dim nomCliente As String

                    If TXTcOMPRADOR.Text.Trim.Length = 0 Then
                        idCliente = VarClienteGeneral.idEntidad
                        nomCliente = VarClienteGeneral.nombreCompleto
                    Else
                        idCliente = TXTcOMPRADOR.Tag
                        nomCliente = TXTcOMPRADOR.Text
                    End If

                    Dim venta As New documentoventaAbarrotes With
                        {
                        .idCliente = idCliente,
                        .NombreEntidad = nomCliente,
                        .nombrePedido = TextComprador.Text.Trim,
                        .NroDocEntidad = txtruc.Text,
                        .tipoDocumento = ComboComprobante.Text,
                        .ImporteNacional = DigitalGauge2.Value
                    }
                    ...
                    For Each r As Record In dgvCompra.Table.Records
                        ...
                        objDocumentoVentaDet.salidaCostoMN = CDec(r.GetValue("costoMN")) ' CDec(i.Cells(6).Value()) * CDec(i.Cells(5).Value())
                        objDocumentoVentaDet.salidaCostoME = CDec(r.GetValue("costoME")) 'CDec(i.Cells(28).Value()) * CDec(i.Cells(5).Value())
                        objDocumentoVentaDet.codigoBarra = If(r.GetValue("codBarra").ToString.Trim.Length > 0, r.GetValue("codBarra"), String.Empty)
                        objDocumentoVentaDet.Menor = Decimal.Parse(r.GetValue("menor"))
                        objDocumentoVentaDet.Mayor = Decimal.Parse(r.GetValue("mayor"))
                        objDocumentoVentaDet.GMayor = Decimal.Parse(r.GetValue("gmayor"))
                        objDocumentoVentaDet.canDisponible = Decimal.Parse(r.GetValue("canDisponible"))
                        objDocumentoVentaDet.tipobeneficio = "-"
                        objDocumentoVentaDet.beneficiobase = 0
                        listaventa.Add(objDocumentoVentaDet)
                    Next
                    venta.documentoventaAbarrotesDet = listaventa
                    AddVentaReserva(venta)
                    FillListViewPedidos()
                    ConteoReserva()
                ...
                End If
            Else
                MessageBox.Show("Debe ingresar una canasta de ventas!", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    ...
    Private Sub ConteoReserva()
        ToolConteo.Text = $"{ListaventasEnEspera.Count} {"Reservas"}"
    End Sub

    Private Sub AddVentaReserva(venta As documentoventaAbarrotes)
        ListaventasEnEspera.Add(venta)
    End Sub

    Private Sub EliminarReserva(nombre As String)
        Dim vent = ListaventasEnEspera.Where(Function(o) o.nombrePedido = nombre).FirstOrDefault
        If vent IsNot Nothing Then
            ListaventasEnEspera.Remove(vent)
        End If
        ConteoReserva()
    End Sub

    Private Sub SeleccionarReserva(pedido As String)
        Dim invSA As New documentoVentaAbarrotesSA
        Dim vent = ListaventasEnEspera.Where(Function(o) o.nombrePedido = pedido).FirstOrDefault
        If vent IsNot Nothing Then
            dgvCompra.Table.Records.DeleteAll()
            LimpiarControles()

            TXTcOMPRADOR.Text = vent.NombreEntidad
            TXTcOMPRADOR.Tag = vent.idCliente
            txtruc.Text = vent.NroDocEntidad
            TextComprador.Text = vent.nombrePedido
            ComboComprobante.Text = vent.tipoDocumento

            For Each i In vent.documentoventaAbarrotesDet.ToList
                ...
                With dgvCompra.Table
                    ...
                    .CurrentRecord.SetValue("gravado", i.destino)
                    .CurrentRecord.SetValue("idProducto", i.idItem)
                    .CurrentRecord.SetValue("item", i.DetalleItem)
                    .CurrentRecord.SetValue("um", i.unidad1)
                    .CurrentRecord.SetValue("cantidad", i.monto1)
                    .CurrentRecord.SetValue("canDisponible", productoInventory.stock.GetValueOrDefault)

                    .CurrentRecord.SetValue("tipoExistencia", i.tipoExistencia)
                    .CurrentRecord.SetValue("marca", Nothing) ') If(productoBE.CustomLote.productoSustentado = True, "Doc.", "Not.")) 'Susuentado y no sustentado
                    .CurrentRecord.SetValue("vcmn", i.montokardex)
                    .CurrentRecord.SetValue("MontoSaldo", i.importeMN)
                    .CurrentRecord.SetValue("vcme", 0)
                    .CurrentRecord.SetValue("igvmn", i.montoIgv)
                    .CurrentRecord.SetValue("igvme", 0)
                    .CurrentRecord.SetValue("pumn", i.precioUnitario)
                    .CurrentRecord.SetValue("pume", 0)
                    .CurrentRecord.SetValue("menor", i.Menor)
                    .CurrentRecord.SetValue("mayor", i.Mayor)
                    .CurrentRecord.SetValue("gmayor", i.GMayor)
                    ...
                    .CurrentRecord.SetValue("valBonif", "N")
                    .CurrentRecord.SetValue("almacen", i.idAlmacenOrigen)
                    .CurrentRecord.SetValue("presentacion", i.NombreAlmacen)

                    .CurrentRecord.SetValue("percepcionMN", FormatNumber(0, 2))
                    .CurrentRecord.SetValue("percepcionME", FormatNumber(0, 2))
                    .CurrentRecord.SetValue("costoMN", 0)
                    .CurrentRecord.SetValue("costoME", 0)
                    .CurrentRecord.SetValue("tipoPrecio", i.tipoVenta)
                    .CurrentRecord.SetValue("cboprecio", i.tipoVenta)
                    .CurrentRecord.SetValue("cat", StatusCategoriaVenta.Productos)
                    .CurrentRecord.SetValue("codigoLote", 0) ' productoBE.codigoLote)
                    .CurrentRecord.SetValue("codBarra", i.codigoBarra)
                    .CurrentRecord.SetValue("empresa", Gempresas.IdEmpresaRuc)
                    .CurrentRecord.SetValue("tipoventa", "V")
                    .CurrentRecord.SetValue("cantidad2", i.monto1)

                    .CurrentRecord.SetValue("totalmn", i.importeMN)
                    .CurrentRecord.SetValue("totalpagar", i.importeMN)
                    .CurrentRecord.SetValue("totalme", 0)
    ...
    Private Sub FillListViewPedidos()
        ListReservas.Items.Clear()

        For Each i In ListaventasEnEspera
            Dim n As New ListViewItem(i.nombrePedido)
            n.SubItems.Add(i.ImporteNacional)
            ListReservas.Items.Add(n)
        Next
    End Sub
    ...
    Private Sub ListReservas_MouseDoubleClick(sender As Object, e As MouseEventArgs) Handles ListReservas.MouseDoubleClick
        Try
            If ListReservas.SelectedItems.Count > 0 Then
                SeleccionarReserva(ListReservas.SelectedItems(0).SubItems(0).Text)
                TotalTalesXcolumna()
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub ToolStripButton5_Click_2(sender As Object, e As EventArgs) Handles ToolStripButton5.Click
        If ListReservas.SelectedItems.Count > 0 Then
            EliminarReserva(ListReservas.SelectedItems(0).SubItems(0).Text)
            ListReservas.SelectedItems(0).Remove()
        End If
    End Sub
    ...
    Private Sub TableModel_QueryRowHeight(ByVal sender As Object, ByVal e As GridRowColSizeEventArgs)
        ...
        If e.Index > 0 Then
            ...
            Dim style As GridStyleInfo = Me.dgvCompra.TableModel(e.Index, 5)
            Dim TipoExis As GridStyleInfo = Me.dgvCompra.TableModel(e.Index, 18)

            If (TipoExis.CellValue.ToString() = TipoExistencia.ServicioGasto) Then
                Dim graphicsProvider As IGraphicsProvider = Me.dgvCompra.TableModel.GetGraphicsProvider()
                Dim g As Graphics = graphicsProvider.Graphics
                Dim model As GridCellModelBase = style.CellModel
                e.Size = model.CalculatePreferredCellSize(g, e.Index, 5, style, GridQueryBounds.Height).Height
                e.Handled = True
            End If
        End If
    End Sub
    ...
    Private Sub dgvCompra_TableControlCellDoubleClick(sender As Object, e As GridTableControlCellClickEventArgs) Handles dgvCompra.TableControlCellDoubleClick
        ...
        'If style3.Enabled Then
        '    If style3.TableCellIdentity.Column.Name = "item" Then
        '        style3.CellValue.SetValue(0, style3.Text.Length)
        '    End If
