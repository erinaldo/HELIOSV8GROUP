...
Public Class FormExistenciaPreciosEquivalencia
...
#Region "Methods"
    ...
    Private Sub GetProductos(IDCategoria As Integer)
        Dim conteo As Integer = 0
        Dim listaSA As New detalleitemsSA
        Dim tipoex As String = String.Empty

        listaProductos = listaSA.GetProductosWithEquivalenciasSelCategory(New detalleitems With
                                                          {
                                                          .idEmpresa = Gempresas.IdEmpresaRuc,
                                                          .idEstablecimiento = GEstableciento.IdEstablecimiento,
                                                          .idItem = IDCategoria
...
#End Region
    ...
    Private Sub AsigarAUnaCategoríaToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AsigarAUnaCategoríaToolStripMenuItem.Click
        If listaProductos Is Nothing Then Exit Sub

        If listaProductos.Where(Function(o) o.Assign = True).Count = 0 Then
            MessageBox.Show("Debe seleccionar al menos un item", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        End If

        If listaProductos.Where(Function(o) o.Assign = True).Count > 0 Then
            Dim FormAsignCategory As New FormAsignCategory(listaProductos.Where(Function(o) o.Assign = True).ToList)
            FormAsignCategory.StartPosition = FormStartPosition.CenterParent
            FormAsignCategory.ShowDialog(Me)
            If FormAsignCategory.Tag IsNot Nothing Then
                Dim valor = CType(FormAsignCategory.Tag, Boolean)
                If valor = True Then
                    GetUpdateGrid(listaProductos.Where(Function(o) o.Assign = True).ToList, True)
                    'CleanPrices()
                    GridEquivalencia.Table.Records.DeleteAll()
                    GridPrecios.Table.Records.DeleteAll()
                End If
    ...
    Private Sub GetUpdateGrid(Listproducts As List(Of detalleitems), valor As Boolean)
        For Each i In Listproducts
            For Each rec In GridProductos.Table.Records
                If i.codigodetalle = Integer.Parse(rec.GetValue("idproducto")) Then
                    rec.SetValue("categoria", If(valor = True, i.idItem, Nothing))
                End If
            Next
        Next
    End Sub

    Private Sub GridProductos_TableControlMouseDown(sender As Object, e As GridTableControlMouseEventArgs) Handles GridProductos.TableControlMouseDown
        Dim styleinfo As GridTableCellStyleInfo = e.TableControl.PointToTableCellStyle(New Point(e.Inner.X, e.Inner.Y))

        If e.Inner.Button = MouseButtons.Right AndAlso styleinfo.TableCellIdentity.TableCellType = GridTableCellType.RecordFieldCell Then
            Me.ContextMenuStrip1.Show(Me.GridProductos, Me.GridProductos.PointToClient(Control.MousePosition))
        ElseIf e.Inner.Button = MouseButtons.Right AndAlso styleinfo.TableCellIdentity.TableCellType = GridTableCellType.AlternateRecordFieldCell Then
            Me.ContextMenuStrip1.Show(Me.GridProductos, Me.GridProductos.PointToClient(Control.MousePosition))
        End If
    End Sub

    Private Sub NuevaCategoríaToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles NuevaCategoríaToolStripMenuItem.Click
        Dim f As New FormCrearCategory
        f.Manipulacion = ENTITY_ACTIONS.INSERT
        f.StartPosition = FormStartPosition.CenterParent
        f.ShowDialog(Me)
        If f.Tag IsNot Nothing Then
            Dim cat = CType(f.Tag, item)
            ListCategories.Add(cat)
            GetCategories()
        End If
    End Sub

    Private Sub sfListView1_DrawItem(sender As Object, e As Syncfusion.WinForms.ListView.Events.DrawItemEventArgs) Handles sfListView1.DrawItem
        If (e.ItemType = Syncfusion.WinForms.ListView.Enums.ItemType.Header) Then
            e.Text = "Lista de categorías"
        End If
    End Sub

    Private Sub sfListView1_MouseDoubleClick(sender As Object, e As MouseEventArgs) Handles sfListView1.MouseDoubleClick
        Cursor = Cursors.WaitCursor
        If TryCast(sender, SfListView).SelectedItems.Count > 0 Then
            Dim obj = CType(sfListView1.SelectedItem, item)
            GetProductos(obj.idItem)
        End If
        Cursor = Cursors.Default
    End Sub

    Private Sub sfListView1_Click(sender As Object, e As EventArgs) Handles sfListView1.Click

    End Sub

    Private Sub GridProductos_TableControlCheckBoxClick(sender As Object, e As GridTableControlCellClickEventArgs) Handles GridProductos.TableControlCheckBoxClick
        Me.Cursor = Cursors.WaitCursor
        Dim obj As New documentocompra
        Dim RowIndex As Integer = e.Inner.RowIndex
        Dim cc As GridCurrentCell = GridProductos.TableControl.CurrentCell
        ...
        If RowIndex > -1 Then
            ...
            If style3.Enabled Then
                If style3.TableCellIdentity.Column.Name = "sel" Then
                    ...
                    If sty.TableCellIdentity.DisplayElement.Kind = DisplayElementKind.Record Then
                        ...
                        Dim valCheck = record.GetValue("sel") 'Me.GridCompra.TableModel(RowIndex, 15).CellValue
                        Select Case valCheck
                            Case "False" 'TRUE
                                ...
                                'MessageBox.Show(True)
                                Dim item = listaProductos.Where(Function(o) o.codigodetalle = record.GetValue("idproducto")).SingleOrDefault
                                item.Assign = True
    ...
    Private Sub SeleccionarToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SeleccionarToolStripMenuItem.Click
        ...
        Dim obj = CType(sfListView1.SelectedItem, item)
        ' GetProductos(obj.idItem)
        'MsgBox(obj.descripcion)
        If obj Is Nothing Then Exit Sub
        Dim f As New FormCrearCategory(obj)
        f.StartPosition = FormStartPosition.CenterParent
        f.ShowDialog(Me)
        If f.Tag IsNot Nothing Then
            Dim c = CType(f.Tag, item)
            Dim cat = ListCategories.Where(Function(o) o.idItem = c.idItem).Single
            cat.precioCompra = c.precioCompra
            cat.preciocompratipo = c.preciocompratipo
            cat.firstpercent = c.firstpercent
            cat.beforepercent = c.beforepercent
    ...
    Private Sub sfListView1_MouseDown(sender As Object, e As MouseEventArgs) Handles sfListView1.MouseDown
        If e.Button = MouseButtons.Right Then
            Me.ContextMenuStrip2.Show(Me.sfListView1, Me.sfListView1.PointToClient(Control.MousePosition))
    ...
    Private Sub BunifuFlatButton11_Click(sender As Object, e As EventArgs) Handles BunifuFlatButton11.Click
        Dim r As Record = GridProductos.Table.CurrentRecord
        If r IsNot Nothing Then
            Dim prod = listaProductos.Where(Function(o) o.codigodetalle = Integer.Parse(r.GetValue("idproducto"))).SingleOrDefault
            ActualizarPreciosProduct(prod)
        End If

    End Sub

    Private Sub ActualizarPreciosProduct(_product As detalleitems)
        ...
        obj.preciocompratipo = If(ComboBoxAdv1.Text = "PORCENTAJE", "PCT", "FJ")
        If _product.item IsNot Nothing Then 'Tiene  Categoria
            obj.firstpercent = _product.firstpercent.GetValueOrDefault
            obj.beforepercent = _product.beforepercent.GetValueOrDefault
            Select Case _product.item.preciocompratipo
                Case "PCT"
    ...
    Private Sub TextPrecioCompraNuevo_ValueChanged(sender As Object, e As EventArgs) Handles TextPrecioCompraNuevo.ValueChanged

    End Sub

    Private Sub TexUltimaCompra_ValueChanged(sender As Object, e As EventArgs)

    End Sub

    Private Sub QuitarAsiganciónToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles QuitarAsiganciónToolStripMenuItem.Click
        If listaProductos Is Nothing Then Exit Sub
        ...
        If listaProductos.Where(Function(o) o.Assign = True).Count = 0 Then
            ...
            Exit Sub
        End If
        If MessageBox.Show("Quitar asiganción de los items seleccionados?", "Atención", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            If listaProductos.Where(Function(o) o.Assign = True).Count > 0 Then
                Dim itemSA As New itemSA
                Dim listSend = listaProductos.Where(Function(o) o.Assign = True).ToList
                itemSA.EditarPropertycategoryProducts(listSend, 0)
                MessageBox.Show("Operación realizada con éxito!", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Information)
                For Each i In listaProductos.Where(Function(o) o.Assign = True).ToList
                    i.item = Nothing
                    i.idItem = Nothing
                Next
                GetUpdateGrid(listaProductos.Where(Function(o) o.Assign = True).ToList, False)
                GridEquivalencia.Table.Records.DeleteAll()
                GridPrecios.Table.Records.DeleteAll()
            End If
        End If
    ...
    '    If r IsNot Nothing Then
    '        Dim idProducto = Integer.Parse(r.GetValue("idproducto"))
    '        Dim f As New FormNuevoCalzado(idProducto)
    '        f.EstadoManipulacion = ENTITY_ACTIONS.UPDATE
    '        f.StartPosition = FormStartPosition.CenterParent
    '        f.ShowDialog()
    '    Else
    ...
    Private Sub ComboBoxAdv1_SelectedValueChanged(sender As Object, e As EventArgs) Handles ComboBoxAdv1.SelectedValueChanged
        Select Case ComboBoxAdv1.Text
            Case "PORCENTAJE"
                TextFirstDescuento.Visible = True
                TextBeforeDescuento.Visible = True
                '     TexUltimaCompra.Visible = True
                ...
                BunifuFlatButton11.Visible = True
            Case Else
                TextFirstDescuento.Visible = False
                TextBeforeDescuento.Visible = False
                '   TexUltimaCompra.Visible = True
                TextPrecioCompraNuevo.Visible = True
