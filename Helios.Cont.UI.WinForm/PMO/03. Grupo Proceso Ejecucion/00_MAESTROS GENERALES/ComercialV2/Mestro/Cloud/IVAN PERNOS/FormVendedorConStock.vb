...
Imports Helios.Cont.Presentation.WinForm.FormVendedorPernos
...
Public Class FormVendedorConStock
...
#Region "Fields"
    ...
    Public Property frmVendedorCode As FormVendedorCode
...
#Region "Constructors"
    Public Sub New()
        ...
        FormatoGridAvanzado(dgvCompra, False, False, 12)
...
#Region "Methods"
    ...
    Sub GrabarNotaDeVenta()
        frmVendedorCode = New FormVendedorCode
        frmVendedorCode.StartPosition = FormStartPosition.CenterParent
        frmVendedorCode.ShowDialog()

        If frmVendedorCode.Tag IsNot Nothing Then
            Dim envio = CType(frmVendedorCode.Tag, EnvioImpresionVendedorPernos)
            GrabarVentaPernos(envio)
        Else
    ...
    Sub EditarCotizacion()
        ...
        For Each r As Record In dgvCompra.Table.Records
            ...
            Else
                'objDocumentoVentaDet.idAlmacenOrigen = 0 'CInt(r.GetValue("almacen"))
                objDocumentoVentaDet.tipoVenta = 0 'r.GetValue("tipoPrecio")
    ...
    Private Sub GrabarVentaPernos(envio As EnvioImpresionVendedorPernos)
        ...
        ndocumento.tipoEntidad = TIPO_ENTIDAD.CLIENTE
        ndocumento.tipoOperacion = StatusTipoOperacion.VENTA
        ndocumento.usuarioActualizacion = envio.IDVendedor
        ndocumento.fechaActualizacion = DateTime.Now
        ...
        nDocumentoVenta = New documentoventaAbarrotes With {
                  ...
                  .usuarioActualizacion = envio.IDVendedor,
                  .fechaActualizacion = DateTime.Now}
        'tipoEstado,
        ndocumento.documentoventaAbarrotes = nDocumentoVenta
        ...
        For Each r As Record In dgvCompra.Table.Records
            ...
            objDocumentoVentaDet.categoria = Nothing
            objDocumentoVentaDet.preEvento = Nothing
            objDocumentoVentaDet.usuarioModificacion = envio.IDVendedor
            objDocumentoVentaDet.idCajaUsuario = envio.IDCaja
        ...
        If ChPagoDirecto.Checked Then
            ndocumento.ListaCustomDocumento = ListaDocumentoCaja(ndocumento.documentoventaAbarrotes, envio)
        ...
        ElseIf ChPagoAvanzado.Checked = True Then
            ...
            If f.Tag IsNot Nothing Then
                ...
                If c.Count > 0 Then
                    Dim ListaPagos = ListaPagosCajas(c, ndocumento.documentoventaAbarrotes, ndocumento.documentoventaAbarrotes.documentoventaAbarrotesDet, envio)
        ...
        If ListaDetalle.Where(Function(o) o.monto1 = 0).Count = 0 Then
            ...
            idDocuentoGrabado = VentaSA.Grabar_VentaNotaSinInventario(ndocumento)

            '*********************************** Print --************************************

            'ImprimirTicketv2(envio.Nombreprint, idDocuentoGrabado)
            'Dim f As New FormImpresionNuevo  ' frmVentaNuevoFormato
            'f.DocumentoID = idDocuentoGrabado
            'f.StartPosition = FormStartPosition.CenterScreen
            'f.ManipulacionEstado = ENTITY_ACTIONS.INSERT
            'f.Show(Me)
            objDatosGenrales = envio.datosGenerales
            Select Case envio.tipoFormato
                Case "Ticket"
                    ImprimirTicket(envio.Nombreprint, idDocuentoGrabado)
                Case "A4"
                    'ImprimirTicketA4(envio.Nombreprint, idDocuentoGrabado)
            End Select
    ...
    Sub ImprimirTicketv2(imprimir As String, intIdDocumento As Integer)
        ...
        'a.TextoCentroEmpresa(Gempresas.NomCorto)
    ...
    Sub GrabarVentaGeneral()
        frmVendedorCode = New FormVendedorCode
        frmVendedorCode.StartPosition = FormStartPosition.CenterParent
        frmVendedorCode.ShowDialog(Me)

        If frmVendedorCode.Tag IsNot Nothing Then
            Dim envio = CType(frmVendedorCode.Tag, EnvioImpresionVendedorPernos)
            GrabarVentaPernosDoc(envio)
        Else
    ...
    Private Sub GrabarVentaPernosDoc(envio As EnvioImpresionVendedorPernos)
        ...
        ListProductosVendidos = GetDetalleVenta(envio)
        If ListProductosVendidos.Count = 0 Then
            objPleaseWait.Close()
            Throw New Exception("Debe ingresar artículos a la canasta de venta")
        End If
        ...
        If ListProductosVendidos.Count > 0 Then
            Dim DocComprobante =
            CType(GetGrabarVentaComprobante(ListProductosVendidos.ToList(), envio), documento)
        ...
        'ImprimirTicketv2(envio.Nombreprint, lista.FirstOrDefault.idDocumento)
        'Dim f As New FormImpresionNuevo  ' frmVentaNuevoFormato
        'f.DocumentoID = lista.FirstOrDefault.idDocumento
        'f.StartPosition = FormStartPosition.CenterScreen
        '' f.ManipulacionEstado = ENTITY_ACTIONS.INSERT
        'f.Show(Me)
        objDatosGenrales = envio.datosGenerales
        Select Case envio.tipoFormato
            Case "Ticket"
                ImprimirTicket(envio.Nombreprint, lista.FirstOrDefault.idDocumento)
            Case "A4"
                'ImprimirTicketA4(envio.Nombreprint, lista.FirstOrDefault.idDocumento)
        End Select
    ...
    Private Function GetDetalleVenta(envio As EnvioImpresionVendedorPernos) As List(Of documentoventaAbarrotesDet)
        Dim TipoEntrega = TipoEntregado.Entregado
        Dim objDocumentoVentaDet As documentoventaAbarrotesDet
        ...
        For Each r As Record In dgvCompra.Table.Records
            ...
            objDocumentoVentaDet.preEvento = Nothing
            objDocumentoVentaDet.usuarioModificacion = envio.IDVendedor ' usuario.IDUsuario
            objDocumentoVentaDet.idCajaUsuario = envio.IDCaja
            objDocumentoVentaDet.fechaModificacion = DateTime.Now
            objDocumentoVentaDet.Glosa = txtGlosa.Text.Trim
            objDocumentoVentaDet.NomMarca = Nothing ' r.GetValue("marca")
    ...
    Public Function ListaPagosCajas(lista As List(Of documentoCaja), venta As documentoventaAbarrotes, ventaDetalle As List(Of documentoventaAbarrotesDet),
                                    envio As EnvioImpresionVendedorPernos) As List(Of documento)
    ...
    Private Function GetGrabarVentaComprobante(DetalleVenta As List(Of documentoventaAbarrotesDet), envio As EnvioImpresionVendedorPernos) As documento
        Dim ndocumento As New documento()
        Dim DocCaja As New documento
        ...
        ndocumento.tipoEntidad = TIPO_ENTIDAD.CLIENTE
        ndocumento.tipoOperacion = StatusTipoOperacion.VENTA
        ndocumento.usuarioActualizacion = envio.IDVendedor ' usuario.IDUsuario
        ndocumento.fechaActualizacion = DateTime.Now
        TipoCobro = TIPO_VENTA.PAGO.COBRADO
        Dim TipoEntrega = TipoEntregado.Entregado
        Dim tipoEstado = TipoGuia.Entregado
        ...
        nDocumentoVenta = New documentoventaAbarrotes With {
            .horaVenta = txtFecha.Value.TimeOfDay.ToString(),
                  ...
                  .usuarioActualizacion = envio.IDVendedor,' usuario.IDUsuario,
                  .fechaActualizacion = DateTime.Now}
        'tipoEstado,
        ndocumento.documentoventaAbarrotes = nDocumentoVenta
        ...
        GuiaRemisionGenerico(ndocumento, envio)

        If ChPagoDirecto.Checked Then
            ndocumento.ListaCustomDocumento = ListaDocumentoCajaGenerico(ndocumento.documentoventaAbarrotes, envio)
            ndocumento.documentoventaAbarrotes.estadoCobro = TIPO_VENTA.PAGO.COBRADO
            ndocumento.documentoventaAbarrotes.terminos = "CONTADO"
        ElseIf ChPagoAvanzado.Checked = True Then
            Dim f As New frmFormatoPagoComprobantes
    ...
    'Function ListaDocumentoCaja(VENTA As documentoventaAbarrotes, envio As EnvioImpresionVendedorPernos) As List(Of documento)
    '    Dim nDocumentoCaja As New documento
    '    Dim objCaja As New documentoCaja
    '    Dim ListaDoc As New List(Of documento)
    ...
    '    nDocumentoCaja.tipoEntidad = TIPO_ENTIDAD.CLIENTE
    '    nDocumentoCaja.tipoOperacion = StatusTipoOperacion.COBRO_A_CLIENTES
    '    nDocumentoCaja.usuarioActualizacion = envio.IDCaja
    '    nDocumentoCaja.fechaActualizacion = DateTime.Now
    ...
    '    objCaja.idCajaUsuario = envio.IDCaja ' GFichaUsuarios.IdCajaUsuario
    '    objCaja.usuarioModificacion = envio.IDCaja ' usuario.IDUsuario
    '    objCaja.entidadFinanciera = envio.EntidadFinanciera ' cbocajaPago.SelectedValue
    '    objCaja.NombreEntidad = envio.NombreCajero ' cbocajaPago.Text
    '    objCaja.fechaModificacion = DateTime.Now

    '    nDocumentoCaja.documentoCaja = objCaja
    '    ListaDoc.Add(nDocumentoCaja)
    '    ListaDetalleCaja(nDocumentoCaja.documentoCaja, envio)
    '    ' asientoDocumento(nDocumentoCaja.documentoCaja)
    ...
    Function ListaDocumentoCajaGenerico(be As documentoventaAbarrotes, envio As EnvioImpresionVendedorPernos) As List(Of documento)
        Dim nDocumentoCaja As New documento
        Dim objCaja As New documentoCaja
        Dim ListaDoc As New List(Of documento)
        ...
        nDocumentoCaja.documentoCaja = objCaja
        ListaDoc.Add(nDocumentoCaja)
        ListaDetalleCajaGenerico(nDocumentoCaja.documentoCaja, be.documentoventaAbarrotesDet.ToList, envio)
        ' asientoDocumento(nDocumentoCaja.documentoCaja)
    ...
    Private Sub ListaDetalleCajaGenerico(doc As documentoCaja, detalleVenta As List(Of documentoventaAbarrotesDet),
                                         envio As EnvioImpresionVendedorPernos)
        ...
        For Each i In detalleVenta
            ...
            obj.entregado = "SI"
            obj.idCajaUsuario = envio.IDCaja ' GFichaUsuarios.IdCajaUsuario
            obj.usuarioModificacion = envio.IDCaja ' usuario.IDUsuario
    ...
    Private Sub ListaDetalleCaja(doc As documentoCaja, envio As EnvioImpresionVendedorPernos)
        Dim obj As New documentoCajaDetalle
        Dim lista As New List(Of documentoCajaDetalle)
        For Each i As Record In dgvCompra.Table.Records
            ...
            obj = New documentoCajaDetalle
            obj.fecha = Date.Now
            obj.codigoLote = Integer.Parse(i.GetValue("codigoLote"))
            obj.otroMN = Integer.Parse(i.GetValue("codigoLote"))
    ...
    Sub GuiaRemisionGenerico(beDocumento As documento, envio As EnvioImpresionVendedorPernos)
        Dim guiaRemisionBE As New documentoGuia
        Dim documentoguiaDetalle As New documentoguiaDetalle
        Dim ListaGuiaDetalle As New List(Of documentoguiaDetalle)
        'REGISTRANDO LA GUIA DE REMISION
        ...
        For Each r In beDocumento.documentoventaAbarrotes.documentoventaAbarrotesDet.ToList
            If r.tipoExistencia <> "GS" Then
                ...
                documentoguiaDetalle.puntoLlegada = "ORIGEN"
                documentoguiaDetalle.estado = TipoGuiaDetalle.Entrega_Total
                documentoguiaDetalle.usuarioModificacion = envio.IDVendedor '  usuario.IDUsuario
                documentoguiaDetalle.fechaModificacion = DateTime.Now
                ListaGuiaDetalle.Add(documentoguiaDetalle)
                ' End If
            End If
    ...
    Public Sub EnviarProducto(productoBE As totalesAlmacen) Implements IForm.EnviarProducto
        Try
            ...
            If IsNumeric(cantidad) Then
                ...
                Dim existeEnCanasta = VerificarItemDuplicadoV2(CDec(cantidad), productoBE.codigoLote, productoBE.idItem)
...
#End Region
    ...
    Private Sub btGrabar_Click(sender As Object, e As EventArgs) Handles btGrabar.Click
        ...
        '        btGrabar.Enabled = False
        '        Select Case cboTipoDoc.Text
        '            Case "BOLETA", "FACTURA", "FACTURA ELECTRONICA", "BOLETA ELECTRONICA"
        '                GrabarVentaGeneral()
        '            Case "NOTA DE VENTA"
        '                GrabarNotaDeVenta()
        '        End Select
    ...
    Private Sub FormVendedorConStock_Leave(sender As Object, e As EventArgs) Handles Me.Leave

    End Sub

    Private Sub FormVendedorConStock_Load(sender As Object, e As EventArgs) Handles Me.Load
        dgvCompra.TableDescriptor.Columns("pumn").Width = 60
        ...
        If tmpConfigInicio.FormatoVenta = "MKT" Then
            ...
            'Panel1Header.Visible = False
            'PanelMontos.Visible = False
            ''GroupBar1.Visible = False
        ...
        Else
            ...
            'DigitalGauge2.Visible = False
            '' GroupBar1.Visible = True
            'PanelPrecios2.Visible = False
            'Panel1Header.Visible = True
    ...
    Private Sub FormVendedorConStock_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Escape) Then
            Me.Close()
        End If
    End Sub

    Private Sub FormVendedorConStock_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        If dgvCompra.Table.Records.Count > 0 Then
...
#Region "imprimir"
    Sub ImprimirTicket(imprimir As String, intIdDocumento As Integer)
        ...
        End If
        'ruc
        a.TextoIzquierda("R.U.C.: " & objDatosGenrales.ruc)
        ...
        Select Case comprobante.tipoDocumento
            Case "12.1"
                a.AnadirLineaCaracteresDatosGEnerales(CStr(Date.Now), "TICKET BOLETA   N° " & comprobante.serieVenta & "-" & comprobante.numeroVenta, nombreCliente, "Jiron Ricardo Palma 881", "", rucCliente, "NAC", "966557413")

                'a.AnadirLineaSubcabeza("Ticket Boleta    N° " & comprobante.serieVenta & "-" & comprobante.numeroVenta)
            Case "12.2"
                a.AnadirLineaCaracteresDatosGEnerales(CStr(Date.Now), "TICKET FACTURA   N° " & comprobante.serieVenta & "-" & comprobante.numeroVenta, nombreCliente, "Jiron Ricardo Palma 881", "", rucCliente, "NAC", "966557413")

                'a.AnadirLineaSubcabeza("Ticket Factura    N° " & comprobante.serieVenta & "-" & comprobante.numeroVenta)
            Case "03"
                If (comprobante.tipoVenta = "VELC") Then
                    a.AnadirLineaCaracteresDatosGEnerales(CStr(Date.Now), "BOLETA ELECTRONICA   N° " & comprobante.serieVenta & "-" & comprobante.numeroVenta, nombreCliente, "Jiron Ricardo Palma 881", "", rucCliente, "NAC", "966557413")
                ...
                ElseIf (comprobante.tipoVenta = "VPOS") Then
                    a.AnadirLineaCaracteresDatosGEnerales(CStr(Date.Now), "BOLETA   N° " & CStr(comprobante.serieVenta).PadLeft(3, "0"c) & "-" & comprobante.numeroVenta, nombreCliente, "Jiron Ricardo Palma 881", "", rucCliente, "NAC", "966557413")

                    'a.AnadirLineaSubcabeza("Boleta de venta    N° " & CStr(comprobante.serieVenta).PadLeft(3, "0"c) & "-" & comprobante.numeroVenta)
                End If
            Case "01"
                If (comprobante.tipoVenta = "VELC") Then
                    a.AnadirLineaCaracteresDatosGEnerales(CStr(Date.Now), "FACTURA ELECTRONICA   N° " & comprobante.serieVenta & "-" & comprobante.numeroVenta, nombreCliente, "Jiron Ricardo Palma 881", "", rucCliente, "NAC", "966557413")

                    'a.AnadirLineaSubcabeza("Factura electronica    N° " & comprobante.serieVenta & "-" & comprobante.numeroVenta)
                ElseIf (comprobante.tipoVenta = "VPOS") Then
                    a.AnadirLineaCaracteresDatosGEnerales(CStr(Date.Now), "FACTURA   N° " & CStr(comprobante.serieVenta).PadLeft(3, "0"c) & "-" & comprobante.numeroVenta, nombreCliente, "Jiron Ricardo Palma 881", "", rucCliente, "NAC", "966557413")

                    'a.AnadirLineaSubcabeza("Fartura    N° " & CStr(comprobante.serieVenta).PadLeft(3, "0"c) & "-" & comprobante.numeroVenta)
                End If
            Case "9901"
                a.AnadirLineaCaracteresDatosGEnerales(CStr(Date.Now), "PROFORMA   N° " & 0, nombreCliente, "Jiron Ricardo Palma 881", "", rucCliente, "NAC", "966557413")

            Case Else
                a.AnadirLineaCaracteresDatosGEnerales(CStr(Date.Now), "TICKET NOTA   N° " & comprobante.numeroVenta, nombreCliente, "Jiron Ricardo Palma 881", "", rucCliente, "NAC", "966557413")
    ...
    '    Dim a As TicketA4Sorayda = New TicketA4Sorayda
    '    ' Logo de la Empresa
    ...
    '        Case "12.1"
    '            'ticket.TextoExtremos("Caja # 1", "Ticket boleta # " & comprobante.serieVenta & "-" & comprobante.numeroVenta)
    '            a.AnadirLineaCaracteresFactura(objDatosGenrales.ruc, comprobante.serieVenta & " - " & CStr(comprobante.numeroVenta).PadLeft(8, "0"c), "BOLETA")
    '            'ticket.TextoIzquierda("Ticket Boleta # " & comprobante.serieVenta & "-" & comprobante.numeroVenta)
    ...
    '            '  ticket.TextoExtremos("Caja # 1", "Ticket factura # " & comprobante.serieVenta & "-" & comprobante.numeroVenta)
    '            'ticket.TextoIzquierda("Ticket Factura # " & comprobante.serieVenta & "-" & comprobante.numeroVenta)
    '            a.AnadirLineaCaracteresFactura(objDatosGenrales.ruc, comprobante.serieVenta & " - " & CStr(comprobante.numeroVenta).PadLeft(8, "0"c), "FACTURA")
    '            nombreComprabante = "FACTURA" & comprobante.serieVenta & comprobante.numeroVenta
    '            tipoComprobante = "1"
    '        Case "03"
    '            If (comprobante.tipoVenta = "VELC") Then
    '                a.AnadirLineaCaracteresFactura(objDatosGenrales.ruc, comprobante.serieVenta & " - " & CStr(comprobante.numeroVenta).PadLeft(8, "0"c), "BOLETA ELECTRONICA")
    '                nombreComprabante = "FACTURA" & comprobante.serieVenta & comprobante.numeroVenta
    '                tipoComprobante = "2"
    '            ElseIf (comprobante.tipoVenta = "VPOS") Then
    '                a.AnadirLineaCaracteresFactura(objDatosGenrales.ruc, comprobante.serieVenta & " - " & CStr(comprobante.numeroVenta).PadLeft(8, "0"c), "BOLETA")
    ...
    '        Case "01"
    '            If (comprobante.tipoVenta = "VELC") Then
    '                a.AnadirLineaCaracteresFactura(objDatosGenrales.ruc, comprobante.serieVenta & " - " & CStr(comprobante.numeroVenta).PadLeft(8, "0"c), "FACTURA ELECTRONICA")
    '                nombreComprabante = "FACTURA" & comprobante.serieVenta & comprobante.numeroVenta
    '                tipoComprobante = "2"
    '            ElseIf (comprobante.tipoVenta = "VPOS") Then
    '                a.AnadirLineaCaracteresFactura(objDatosGenrales.ruc, comprobante.serieVenta & " - " & CStr(comprobante.numeroVenta).PadLeft(8, "0"c), "FACTURA")
    ...
    '            End If
    '        Case "9901"
    '            a.AnadirLineaCaracteresFactura(objDatosGenrales.ruc, 0 & " - " & CStr(0).PadLeft(8, "0"c), "PROFORMA")
    '            nombreComprabante = "PROFORMA" & comprobante.serieVenta & comprobante.numeroVenta
    '            tipoComprobante = "1"
    '        Case Else

    '            a.AnadirLineaCaracteresFactura(objDatosGenrales.ruc, comprobante.serieVenta & " - " & CStr(comprobante.numeroVenta).PadLeft(8, "0"c), "NOTA")
