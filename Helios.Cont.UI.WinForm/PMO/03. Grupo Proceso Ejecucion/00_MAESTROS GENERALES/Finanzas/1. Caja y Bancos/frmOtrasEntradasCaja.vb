...
Public Class frmOtrasEntradasCaja
...
#Region "Events"
    Private Sub ButtonAdv4_Click(sender As Object, e As EventArgs) Handles ButtonAdv4.Click
        ...
        If Not IsNothing(r) Then
            ...
            f.operacion = StatusTipoOperacion.OTRAS_ENTRADAS_DE_DINERO
            f.StartPosition = FormStartPosition.CenterParent
            f.ShowDialog()
            Dim periodoG As String = cboMesCompra.SelectedValue & "/" & AnioGeneral
            GetMovimientosPeriodo(GEstableciento.IdEstablecimiento, periodoG, "OEC")
        End If
    End Sub

    Private Sub frmOtrasEntradasCaja_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub ButtonAdv2_Click(sender As Object, e As EventArgs) Handles ButtonAdv2.Click
        ...
        If Not IsNothing(cajaUsuario) Then
            ...
            f.txtPeriodo.Value = New Date(AnioGeneral, CInt(cboMesCompra.SelectedValue), Date.Now.Day)
        ...
        If Not IsNothing(Me.dgvOtrosMov.Table.CurrentRecord) Then
            ...
            If (dgvOtrosMov.Table.CurrentRecord.GetValue("saldoMN") > 0) Then
                ...
                If Not IsNothing(cajaUsuario) Then
                    ...
                    f.txtAnioCompra.Text = CInt(cboAnio.Text)
                    f.txtPeriodo.Value = New Date(CInt(cboAnio.Text), CInt(cboMesCompra.SelectedValue), DateTime.Now.Day)
                    f.lblMovimiento.Tag = "OEC"
                    f.lblMovimiento.Text = "OTRAS ENTRADAS A CAJA"
                    f.ManipulacionEstado = ENTITY_ACTIONS.INSERT
                    f.cboMesCompra.SelectedValue = cboMesCompra.SelectedValue
                    f.txtHora.Value = New DateTime(cboAnio.Text, CInt(cboMesCompra.SelectedValue), DateTime.Now.Day, Date.Now.Hour, Date.Now.Minute, Date.Now.Second)
                    f.DigitalGauge2.Value = dgvOtrosMov.Table.CurrentRecord.GetValue("saldoMN")
                    f.idDocumentoPadre = dgvOtrosMov.Table.CurrentRecord.GetValue("idDocumento")
                    f.UbicarDocumentoEntradasSalidas(Me.dgvOtrosMov.Table.CurrentRecord.GetValue("idDocumento"), Me.dgvOtrosMov.Table.CurrentRecord.GetValue("tipoPersona"))
                    f.tipoEntidad = Me.dgvOtrosMov.Table.CurrentRecord.GetValue("tipoPersona")
                    f.txtDia.Value = New Date(CInt(cboAnio.Text), CInt(cboMesCompra.SelectedValue), DateTime.Now.Day)
                    f.StartPosition = FormStartPosition.CenterParent
                    f.ShowDialog()
                Else
                    MessageBoxAdv.Show("No registra una caja activa!", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Warning)
