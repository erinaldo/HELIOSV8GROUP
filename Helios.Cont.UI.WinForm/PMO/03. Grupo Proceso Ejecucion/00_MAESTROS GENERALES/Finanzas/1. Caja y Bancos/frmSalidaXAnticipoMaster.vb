...
Public Class frmSalidaXAnticipoMaster
...
#Region "Methods"
    ...
    Private Sub GetMovimientosPeriodo(intIdEstablecimiento As Integer, strPeriodo As String, strMovimiento As String)
        ...
        dt.Columns.Add(New DataColumn("reversionMN", GetType(Decimal)))
        dt.Columns.Add(New DataColumn("reversionME", GetType(Decimal)))
        ...
        '        listaDocCaja = documentoCajaSA.ObtenerMovimientosPorPeriodoFinanzasXiDCaja(Gempresas.IdEmpresaRuc, GEstableciento.IdEstablecimiento, strPeriodo, strMovimiento, GFichaUsuarios.IdCajaUsuario)
        '    Case Else
...
#Region "Events"
    ...
    '    Dim periodoG As String = cboMesCompra.SelectedValue & "/" & cint(cboAnio)
    '    GetMovimientosPeriodo(GEstableciento.IdEstablecimiento, periodoG, "OSC")
    '    Cursor = Cursors.Default
    'End Sub

    Private Sub ButtonAdv6_Click(sender As Object, e As EventArgs) Handles ButtonAdv6.Click
        LoadingAnimator.Wire(dgvOtrosMov.TableControl)
        ...
        End If
        LoadingAnimator.UnWire(dgvOtrosMov.TableControl)
    End Sub

    Private Sub ButtonAdv2_Click(sender As Object, e As EventArgs) Handles ButtonAdv2.Click
        Dim cajaUsuarioSA As New cajaUsuarioSA
        Cursor = Cursors.WaitCursor
        Dim fechaAnt = New Date(CInt(cboAnio.Text), CInt(cboMesCompra.SelectedValue), 1)
        ...
        Dim valida As Boolean = empresaPeriodoSA.EstadoMesCerrado(New empresaCierreMensual With {.idEmpresa = Gempresas.IdEmpresaRuc, .anio = CInt(cboAnio.Text), .mes = CInt(cboMesCompra.SelectedValue)})
            ...
            If (TIPO_ESTADO_NOMBRE_CAJA.NO_USADO = Me.dgvOtrosMov.Table.CurrentRecord.GetValue("estado")) Then
                If (dgvOtrosMov.Table.CurrentRecord.GetValue("saldoMN") > 0) Then
                    ...
                    If Not IsNothing(cajaUsuario) Then
                        ...
                        Dim f As New frmReversionModal(StatusTipoOperacion.ANTICIPOS_OTORGADOS)
                        f.txtAnioCompra.Text = CInt(cboAnio.Text)
                        f.txtPeriodo.Value = New Date(CInt(cboAnio.Text), CInt(cboMesCompra.SelectedValue), 1)
                        f.lblMovimiento.Tag = "AO"
                        f.lblMovimiento.Text = "ANTICIPOS OTORGADOS"
                        ...
                        '.ubicarCliente(dgvOtrosMov.Table.CurrentRecord.GetValue("idPersonal"), dgvOtrosMov.Table.CurrentRecord.GetValue("tipoPersona"))
                    ...
                    End If
                Else
                    MessageBoxAdv.Show("Debe ser mayor a 0!", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                End If
            Else
                MessageBoxAdv.Show("no puede revertir, ya existe transacciones!", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            End If
        Else
                MessageBox.Show("Debe seleccionar una item!", "Atención!", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        End If
    ...
    Private Sub ButtonAdv7_Click(sender As Object, e As EventArgs) Handles ButtonAdv7.Click
        ...
        If Not IsNothing(Me.dgvOtrosMov.Table.CurrentRecord) Then
            If (TIPO_ESTADO_NOMBRE_CAJA.NO_USADO = Me.dgvOtrosMov.Table.CurrentRecord.GetValue("estado")) Then
                ...
                End If
            Else
                MessageBoxAdv.Show("No puede editar!", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            End If
        Else
                MessageBox.Show("Debe seleccionar una item!", "Atención!", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        End If
    ...
    Private Sub ButtonAdv8_Click(sender As Object, e As EventArgs) Handles ButtonAdv8.Click
        ...
        If Not IsNothing(Me.dgvOtrosMov.Table.CurrentRecord) Then
            ...
            If (dgvOtrosMov.Table.CurrentRecord.GetValue("saldoMN") > 0) Then
                ...
                End If
            Else
                MessageBoxAdv.Show("El saldo debe ser mayor a 0!", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            End If
        Else
