...
Public Class TabLG_InventarioTransito
    Public Property invSA As New inventarioMovimientoSA
    Private itemCompraSA As New DocumentoCompraDetalleSA
    Dim tablaSA As New tablaDetalleSA
    Dim almacenSA As New almacenSA
    Friend Delegate Sub SetDelegateProveedores(ByVal dt As DataTable)
    Private ThreadProveedores As Thread
#Region "Constructors"
    Dim ventanaSel As FormMaestroLogistica
    Public Sub New(ventana As FormMaestroLogistica)
        ...
        'GetProveedoresEnTransito(New documentocompra With {.idEmpresa = empresa,
        '                         .idCentroCosto = estable,
        '                         .tipoCompra = TIPO_COMPRA.COMPRA})
        ProgressBar1.Visible = True
        ProgressBar1.Style = ProgressBarStyle.Marquee
        ThreadProveedores = New Thread(New System.Threading.ThreadStart(Sub() GetProveedoresEnTransito(New documentocompra With
                                                                                                       {.idEmpresa = empresa,
                                                                                                       .idCentroCosto = estable,
                                                                                                       .tipoCompra = TIPO_COMPRA.COMPRA
                                                                                                       }
...
#Region "Methods"
    ...
    Private Sub AsignarItem(intNumero As Integer)
    ...
    Public Sub GetInventarioTransitoByIdDocumento(idProveedor As Integer, idDocumentoCompra As Integer)
        ...
        For Each i In invSA.GetExistenciaTransitoByProveedor(compra)

            If i.SaldoCantidad > 0 Or i.SaldoMontoMN > 0 Then
                ...
                Select Case i.destino
                    ...
                    Case Else
                        If (CDec(i.monto1) > 0) Then
                            dr(11) = Math.Round(CDec(i.importe) / CDec(i.monto1), 2)
                            dr(20) = Math.Round(CDec(i.importeUS) / CDec(i.monto1), 2)
                        End If

                        dr(12) = (FormatNumber(i.importe, 2))
                        dr(13) = (FormatNumber(i.importeUS, 2))
                End Select
    ...
    Public Sub GetProveedoresEnTransito(be As documentocompra)
        ...
        SetDataSourceProveedores(dt)
    End Sub

    Private Sub SetDataSourceProveedores(dt As DataTable)
        If Me.InvokeRequired Then
            Dim deleg As New SetDelegateProveedores(AddressOf SetDataSourceProveedores)
            Invoke(deleg, New Object() {dt})
        Else
            GridProveedores.DataSource = dt
...
#Region "Events"
    ...
    Private Sub GridProveedores_TableControlCellClick(sender As Object, e As Syncfusion.Windows.Forms.Grid.Grouping.GridTableControlCellClickEventArgs) Handles GridProveedores.TableControlCellClick
        ...
        If r IsNot Nothing Then
            ...
            GetInventarioTransitoByIdDocumento(Integer.Parse(r.GetValue("idProv")), 0)
        Else
            dgvEnvioAlmacen.Table.Records.DeleteAll()
            dgvTransito.Table.Records.DeleteAll()
    ...
    Private Sub GridComprobantes_TableControlCellClick(sender As Object, e As Syncfusion.Windows.Forms.Grid.Grouping.GridTableControlCellClickEventArgs) Handles GridComprobantes.TableControlCellClick
        Cursor = Cursors.WaitCursor
        Dim r As Record = GridComprobantes.Table.CurrentRecord
        If r IsNot Nothing Then
            '    Dim r As Record = GridProveedores.Table.CurrentRecord
            If r IsNot Nothing Then
                If cboTipoExistenciaTransito.SelectedIndex > -1 Then
                    Dim idProv = GridProveedores.Table.CurrentRecord.GetValue("idProv")
                    Dim idDoc = r.GetValue("idDoc")

                    GetInventarioTransitoByIdDocumento(Integer.Parse(idProv), Integer.Parse(r.GetValue("idDoc")))
                End If
            Else
                MessageBox.Show("Debe seleccionar un proveedor válido", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            End If
    ...
    Private Sub btSelecAll_Click(sender As Object, e As EventArgs)
    ...
    Private Sub btSelecAll_Click_1(sender As Object, e As EventArgs)
    ...
    Private Sub btGrabar_Click(sender As Object, e As EventArgs) Handles btGrabar.Click
        Try
            If AutorizacionRolSA.TienePermiso(AsegurablesSistemaPOS.ENVIAR_A_ALMACEN_Botón___, AutorizacionRolList) Then
                If dgvTransito.Table.SelectedRecords.Count > 0 Then
                    If rbParcial.Checked = True Then

                        If dgvEnvioAlmacen.Table.Records.Count > 0 Then

                            Dim fechaEnvioDefault = CType(dgvTransito.Table.CurrentRecord.GetValue("fechaCompra"), DateTime)

                            Dim f As New frmEnvioExistencia
                            f.TxtDia.Text = "" 'fechaEnvioDefault.Day
                            f.cboMesCompra.SelectedValue = String.Format("{0:00}", fechaEnvioDefault.Month)
                            f.cboAnio.Text = fechaEnvioDefault.Year
    ...
    Private Sub dgvEnvioAlmacen_TableControlCellClick(sender As Object, e As Syncfusion.Windows.Forms.Grid.Grouping.GridTableControlCellClickEventArgs) Handles dgvEnvioAlmacen.TableControlCellClick
    ...
    Private Sub ButtonAdv1_Click(sender As Object, e As EventArgs) Handles ButtonAdv1.Click
        Dim rec As Record = dgvTransito.Table.CurrentRecord
        If rec IsNot Nothing Then
            RechazarCompraTransito(rec)
        Else
            MessageBox.Show("Debe seleccionar un item válido", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        End If
    End Sub

    Private Sub RechazarCompraTransito(rec As Record)
        Dim secuencia As Integer = Integer.Parse(rec.GetValue("secCompra"))
        itemCompraSA.actualizarEstadoTransitoItem(New documentocompradetalle With {.secuencia = secuencia, .ItemEntregadototal = StatusProductosTransito.Recahazado})
        dgvEnvioAlmacen.Table.Records.DeleteAll()
        dgvTransito.Table.Records.DeleteAll()
        ventanaSel.ThreadTransito()
    End Sub
