...
Public Class TabLG_InventarioValorizado
...
#Region "Contructors"
    Public Sub New()
        ...
        'FormatoGridPequeño(dgvKardexVal, False, 10.0F)
        FormatoGridPequeño(dgvKardexVal, False, 9.5F)
        LoadCombos()
        ComboStatus()
...
#Region "Methods"
    ...
    Private Sub GetInventarioValorizadoSinStockCero(idAlmacen As Integer, tipoex As String)
        Dim totalesAlmacenBE As New List(Of totalesAlmacen)
        Dim dt As New DataTable("Lista de productos ")
        'Clasificicacion
        dt.Columns.Add(New DataColumn("Clasificacion", GetType(String)))
        ...
        For Each i As totalesAlmacen In totalesAlmacenSA.GetProductosByAlmacen(New almacen With {.idEmpresa = Gempresas.IdEmpresaRuc,
                                                                               ...
                                                                               .idAlmacen = idAlmacen,
                                                                               .TipoConsulta = StatusTipoConsulta.XUNIDAD_ORGANICA}, tipoex).Where(Function(o) o.cantidad > 0).OrderBy(Function(o) o.descripcion).ToList
            Dim strGravado As String = IIf(i.origenRecaudo = 1, "Gravado", "No gravado")
            Dim dr As DataRow = dt.NewRow()
            dr(0) = i.Clasificicacion
            dr(1) = strGravado
    ...
    ''' <summary>
    ''' Inventario General: Acumulado o por lotes
    ''' </summary>
    Private Sub GetInventario(be As totalesAlmacen)
        ...
        For Each i As totalesAlmacen In totalesAlmacenSA.GetInventarioGeneral(be).Where(Function(o) o.cantidad > 0).OrderBy(Function(o) o.descripcion).ToList
            Dim strGravado As String = IIf(i.origenRecaudo = 1, "Gravado", "No gravado")
            Dim dr As DataRow = dt.NewRow()
            dr(0) = i.Clasificicacion
            dr(1) = strGravado
    ...
    Private Sub GetInventarioValorizado(idAlmacen As Integer, tipoex As String)
        ...
        For Each i As totalesAlmacen In totalesAlmacenSA.GetProductosByAlmacen(New almacen With {.idEmpresa = Gempresas.IdEmpresaRuc,
                                                                               .idEstablecimiento = GEstableciento.IdEstablecimiento,
                                                                               .idAlmacen = idAlmacen,
                                                                               .TipoConsulta = StatusTipoConsulta.XUNIDAD_ORGANICA}, tipoex).OrderBy(Function(o) o.descripcion).ToList
            Dim strGravado As String = IIf(i.origenRecaudo = 1, "Gravado", "No gravado")
            Dim dr As DataRow = dt.NewRow()
            dr(0) = i.Clasificicacion
            dr(1) = strGravado
    ...
    Private Sub ButtonAdv1_Click(sender As Object, e As EventArgs) Handles ButtonAdv1.Click
        ...
        If cboAlmacen.SelectedIndex > -1 Then
            ...
            If IsNumeric(codAlmacen) Then
                ProgressBar1.Visible = True
                ProgressBar1.Style = ProgressBarStyle.Marquee
                'Dim thread As System.Threading.Thread = New System.Threading.Thread(New System.Threading.ThreadStart(Sub() GetInventarioValorizadoSinStockCero(codAlmacen, tipoExistencia)))
                Dim thread As System.Threading.Thread = New System.Threading.Thread(New System.Threading.ThreadStart(Sub() GetInventario(New totalesAlmacen With {.idAlmacen = codAlmacen, .tipoExistencia = tipoExistencia, .InvAcumulado = ChInventAcum.Checked})))
                thread.Start()
            End If
        End If
                ...
                If IsNumeric(codAlmacen) Then
                    If txtBuscarDetalle.Text.Trim.Length > 0 Then
                        Dim t As New totalesAlmacen With {.idAlmacen = cboAlmacen.SelectedValue,
                                                          ...
                                                          .InvAcumulado = ChInventAcum.Checked}
                        ProgressBar1.Visible = True
                        ProgressBar1.Style = ProgressBarStyle.Marquee
