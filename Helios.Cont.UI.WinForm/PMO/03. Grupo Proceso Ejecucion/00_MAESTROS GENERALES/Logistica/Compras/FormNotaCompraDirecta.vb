...
Public Class FormNotaCompraDirecta
...
#Region "Constructors"
    ...
    Public Sub New(fechaVenta As DateTime?)
        ...
        GetCombos()
        GetThreadEntidades()
        TxtDia.Text = fechaVenta.Value.Day
        cboMesCompra.SelectedValue = String.Format("{0:00}", fechaVenta.Value.Month)
        cboAnio.Text = fechaVenta.Value.Year.ToString
        txtHora.Value = fechaVenta.Value.AddMinutes(-2)
...
#Region "Methods"
    ...
    Private Function GetDetalleNota(ndocumento As documento) As List(Of documentocompradetalle)
        ...
        For Each i In dgvCompra.Table.Records
            ...
            If Decimal.Parse(i.GetValue("menor")) > 0 Then
                precios.Add(
                ...
                .tipo = 1,
                .valPorcentaje = 0,
                .idproducto = Integer.Parse(i.GetValue("idProducto")),
                .idPrecio = 1,
                .descripcion = "Precio por Menor",
                .precioMN = Decimal.Parse(i.GetValue("menor")),
                .precioME = 0
                })
            End If

            If Decimal.Parse(i.GetValue("mayor")) > 0 Then
                precios.Add(
               New configuracionPrecioProducto With
               {
               .fecha = DateTime.Now,
               ...
               .precioMN = Decimal.Parse(i.GetValue("mayor")),
               .precioME = 0
               })
            End If

            If Decimal.Parse(i.GetValue("granmayor")) > 0 Then
                precios.Add(
              New configuracionPrecioProducto With
              {
              .fecha = DateTime.Now,
              ...
              .precioMN = Decimal.Parse(i.GetValue("granmayor")),
              .precioME = 0
              })
            End If
    ...
    '    Dim expField1 As ExpressionFieldDescriptor = New ExpressionFieldDescriptor("pumn", "([vcmn]/[cantidad])", GetType(System.Double))

    '    Dim expField2 As ExpressionFieldDescriptor = New ExpressionFieldDescriptor("vcmn", "([pumn]*[cantidad])", GetType(System.Double))
    '    dgvCompra.TableDescriptor.ExpressionFields.AddRange(New ExpressionFieldDescriptor() {expField0, expField1,expField2})
    'End Sub
    ...
    Private Sub GetCombos()
        ...
        ListaAlmacenes = almacenSA.GetListar_almacenesTipo(GEstableciento.IdEstablecimiento, "AF")
        ...
        cboAsignacion.Text = "CONTROL POR COMPROBANTE"
    End Sub
...
#Region "Events"
    ...
    Public Sub EnviarItem(productoBE As detalleitems) Implements IExistencias.EnviarItem
        ...
        Me.dgvCompra.Table.CurrentRecord.SetValue("menor", 0)
        Me.dgvCompra.Table.CurrentRecord.SetValue("mayor", 0)
        Me.dgvCompra.Table.CurrentRecord.SetValue("granmayor", 0)
        Me.dgvCompra.Table.CurrentRecord.SetValue("igv", 0)
        Me.dgvCompra.Table.AddNewRecord.EndEdit()
    ...
    Private Sub FormNotaCompraDirecta_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub FormNotaCompraDirecta_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        With FormCanastaCompras
            .GridItems.Table.Records.DeleteAll()
            .txtFiltrar.Clear()
        End With
