...
Public Class frmVentasAlCredito
    ...
    Public Sub UbicarDocumento(ByVal intIdDocumento As Integer)
        ...
        Try
            ...
            With objDocCompra.GetUbicar_documentoventaAbarrotesPorID(intIdDocumento)
                ...
                txtPedidoRef.Text = .nombrePedido
...
#Region "ASIENTOS Contables del ALMACEN*costos"
    'Private Sub ObtenerCabeceraCuentaDestino(ByVal ALMACEN As String, ByVal CONTADOR As Short, ByVal Establecimiento As String, ByVal strIDDocumento As String, ByVal strDoRef As String, ByVal intMN As Decimal, ByVal intME As Decimal)
    '    Try
    '        dgvCabeceraAsiento.Rows.Add(CONTADOR, ALMACEN, Establecimiento, strIDDocumento, strDoRef, intMN, intME)
    '    Catch ex As Exception
    '        MsgBox(ex.Message)
    '    End Try
    'End Sub
    ...
    '    Dim objService = HeliosSEProxy.CrearProxyHELIOS
    '    Dim objLista() As HeliosService.MascaraContable2BO
    '    Dim objListaExistencia() As HeliosService.MascaraContableExistenciaBO
    '    Try

    '        Select Case StrTipoExistencia
    '            Case "01"
    '                objLista = objService.GetObtenerCuentasMask(CEmpresa, cuentaContable)
    '                If objLista.Length = 0 Then

    '                Else
    '                    dgvDetalleAsientos.Rows.Add(objLista(0).pCuentaKardex, ALMACEN, objLista(0).pAsientoKardex, MONTOSOLES, MONTOUSD, CONTADOR)
    '                    dgvDetalleAsientos.Rows.Add(objLista(0).pCuentaKardex2, ALMACEN, objLista(0).pAsientoKardex2, MONTOSOLES, MONTOUSD, CONTADOR)
    '                    '    dgvDistribucion.EndEdit()
    '                End If
    '            Case Else
    '                objListaExistencia = objService.GetObtenerListaMaskExistencia(CEmpresa, StrTipoExistencia, cuentaContable)
    '                If objListaExistencia.Length = 0 Then

    '                Else
    '                    dgvDetalleAsientos.Rows.Add(objListaExistencia(0).DestinoCompra, ALMACEN, objListaExistencia(0).AsientoDestino, MONTOSOLES, MONTOUSD, CONTADOR)
    '                    dgvDetalleAsientos.Rows.Add(objListaExistencia(0).DestinoCompra2, ALMACEN, objListaExistencia(0).AsientoDestino2, MONTOSOLES, MONTOUSD, CONTADOR)
    '                End If
    '        End Select

    '        '   objLista = objService.GetObtenerCuentasMask(frmHistorialCompras.txtCodigoEmpresa.Text, cuentaContable)
    '    Catch ex As Exception
    '        MsgBox(ex.Message)
    '    End Try
    'End Sub
    ...
    '    '     With frmPedidosVentasPendiente
    '    dgvCabeceraAsiento.Rows.Clear()
    '    dgvDetalleAsientos.Rows.Clear()
    '    Dim contador As Short
    '    For Each i As DataGridViewRow In dgvNuevoDoc.Rows
    '        contador += 1
    '        ObtenerCabeceraCuentaDestino(String.Concat("AST. CONFIRMACION VENTA"), contador, CEstablecimiento, Nothing, Nothing, i.Cells(33).Value, i.Cells(34).Value)
    '        ObtenerCuentaDestino(String.Concat("ALM: ", i.Cells(3).Value), i.Cells(33).Value, i.Cells(34).Value, contador, i.Cells(25).Value, i.Cells(23).Value)
    '    Next
    '    '     End With
    'End Sub
#End Region
    ...
    '    nAsiento.nombreEntidad = txtPedidoRef.Text
...
#Region "Métodos"
    'Public Sub GenerarAsientos()

    '    Dim objService = HeliosSEProxy.CrearProxyHELIOS
    '    Dim objLista() As HeliosService.MascaraContable2BO
    '    dgvAsiento.Rows.Clear()
    '    For Each i As DataGridViewRow In dgvNuevoDoc.Rows
    '        objLista = objService.GetObtenerCuentasMask(CEmpresa, i.Cells(25).Value)
    '        If i.Cells(1).Value = "1" Then
    '            dgvAsiento.Rows.Add(objLista(0).pCuentaVenta, i.Cells(3).Value, "H", "0.00", i.Cells(14).Value, "0.00", i.Cells(18).Value)
    '        Else
    '            dgvAsiento.Rows.Add(objLista(0).pCuentaVenta, i.Cells(3).Value, "H", "0.00", i.Cells(10).Value, "0.00", i.Cells(11).Value)
    '        End If
    '    Next
    '    If CDec(lblTotalMontoIgv.Text) > 0 Then
    '        dgvAsiento.Rows.Add("40111", "IGV - CUENTA PROPIA", "H", "0.00", lblTotalMontoIgv.Text, "0.00", lblTotalMontoIgvUS.Text)
    '    End If
    '    If txtProveedor.Text.Trim.Length > 0 Then
    '        For Each i As DataGridViewRow In dgvNuevoDoc.Rows
    '            dgvAsiento.Rows.Add(txtCuenta.Text,
    '                               i.Cells(3).Value, "D", i.Cells(10).Value, "0.00", i.Cells(11).Value, "0.00")
    '        Next
    '        '  VerificarMontosAsientos()
    '    End If
    ...
    Sub Grabar()
        ...
        statusForm.Show()
        Dim xcod As Integer = VentaSA.SaveVentaTicket(ndocumento, ListaTotales)
        ...
        statusForm.lblMensaje.Text = (xcod)
        ...
        n.SubItems.Add(TIPO_VENTA.VENTA_AL_TICKET)
    ...
    Sub UpdateVenta()
        ...
        With frmMantenimientoVentaPagada.lsvListaPedidos
...
#End Region
    ...
    Private Sub txtSerieComp_LostFocus(sender As Object, e As System.EventArgs) Handles txtSerieComp.LostFocus
        Try
            Select Case txtIdComprobante.Text
                Case "01", "02", "03", "04", "07", "08", "23", "34", "35", "37", "55", "99"
    ...
    Private Sub frmVentasAlCredito_FormClosing(sender As Object, e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Dispose()
    End Sub

    Private Sub frmVentasAlCredito_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        Me.Cursor = Cursors.WaitCursor
    ...
    Public Sub UbicarDocSerie(ByVal strIdEmpresa As String, ByVal intIdEstablecimiento As Integer, ByVal strSerie As String)
        Dim numeracionSA As New NumeracionBoletaSA
        Dim objLista As List(Of numeracionBoletas)
        Try
            objLista = numeracionSA.ObtenerNumeracionEES(strIdEmpresa, intIdEstablecimiento, strSerie)
            If objLista(0).tipo = "BOL" Then
                txtIdComprobante.Text = "03"
                txtComprobante.Text = "BOLETA DE VENTA"
            Else
    ...
    Private Sub LinkLabel1_LinkClicked(sender As System.Object, e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles LinkLabel1.LinkClicked
        ...
        With frmModalSeries
            ...
            .ShowDialog()
            If datos.Count > 0 Then
                txtSerieComp.Text = datos(0).Serie
                lblTipoDocNumeracion.Text = datos(0).Comprobante
                UbicarDocSerie(Gempresas.IdEmpresaRuc, GEstableciento.IdEstablecimiento, txtSerieComp.Text)
            End If
        End With
        Me.Cursor = Cursors.Arrow
    End Sub
    ...
    Private Sub txtCliente_TextChanged(sender As System.Object, e As System.EventArgs)
    ...
    Private Sub txtCliente_KeyDown1(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles txtCliente.KeyDown
        If e.KeyCode = Keys.Enter Or e.KeyCode = Keys.Tab Then
            e.SuppressKeyPress = True
    ...
    Private Sub txtCliente_TextChanged_1(sender As System.Object, e As System.EventArgs) Handles txtCliente.TextChanged
