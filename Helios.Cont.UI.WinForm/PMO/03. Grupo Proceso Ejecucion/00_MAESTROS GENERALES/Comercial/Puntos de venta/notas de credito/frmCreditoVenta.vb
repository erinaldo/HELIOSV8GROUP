...
Public Class frmCreditoVenta
    ...
    Public Property ImporteAnticipoMN() As Decimal
    Public Property ImporteAnticipoME() As Decimal
...
#Region "Métodos"
    ...
    Public Sub ObtenerDetallePagos(ByVal idDocumento As Integer)
        Dim ventaDetalleSA As New DocumentoCajaSA
       
        Try
            lsvPagos.Columns.Clear()
            lsvPagos.Items.Clear()
            lsvPagos.Columns.Add("idDocuemnto", 0) '0
            lsvPagos.Columns.Add("codigoLibro", 150) '01
            lsvPagos.Columns.Add("Importe MN", 70, HorizontalAlignment.Center) '03
            lsvPagos.Columns.Add("Importe ME", 70, HorizontalAlignment.Center) '03
            lsvPagos.Columns.Add("IdcodigoLibro", 0) '04

            For Each i As documentoCaja In ventaDetalleSA.ObtenerCajaPagoPorVentaSL(idDocumento)
                Dim n As New ListViewItem(i.idDocumento)

                Select Case (i.codigoLibro)
                    Case "103"
                        n.SubItems.Add("PAGO CON ANTICIPO")
                        ImporteAnticipoMN = i.montoSoles
                        ImporteAnticipoME = i.montoUsd
                    Case "12.1"
                        n.SubItems.Add("PAGO EN EFECTIVO")
                    Case "12.2"
                        n.SubItems.Add("PAGO EN EFECTIVO")
                End Select
                n.SubItems.Add(FormatNumber(i.montoSoles, 2))
                n.SubItems.Add(FormatNumber(i.montoUsd, 2))
                n.SubItems.Add(i.codigoLibro)
                n.Checked = True

                lsvPagos.Items.Add(n)
            Next
            lsvPagos.MultiSelect = False
            lsvPagos.HideSelection = False
            lsvPagos.FullRowSelect = True
    ...
    Public Sub UbicarDetalle(intIddocumento As Integer)
        ...
        Try
            ...
            For Each i As documentoCajaDetalle In objLista.ObtenerCuentasPorPagarPorDetailsVentas(intIddocumento)
                detalle = detalleSA.SumaNotasXidPadreItemVentas(i.secuencia)
                Select Case TipoCompra
                    Case TIPO_VENTA.VENTA_AL_TICKET_DIRECTA, TIPO_VENTA.VENTA_AL_TICKET
                        cTotalmn = CDec(i.MontoDeudaSoles) - detalle.importeMN ' + detalle.ImporteDBMN
                        cTotalme = CDec(i.MontoDeudaUSD) - detalle.importeME '+ detalle.ImporteDBME
                    Case Else
                ...
                dr(18) = i.salidaCostoMN
                dr(19) = i.salidaCostoME
                dr(20) = Math.Round(i.salidaCostoMN / i.CantidadCompra, 2)
                dr(21) = Math.Round(i.salidaCostoME / i.CantidadCompra, 2)
                dt.Rows.Add(dr)
            Next
...
#Region "Manipulación Data"
    ...
    Public Function AS_Proveedor(cMonto As Decimal, cMontoUS As Decimal) As movimiento
        ...
        End If
        nMovimiento.descripcion = "Clientes" 'txtProveedor.Text
    ...
    Public Sub AsientoBeneficios_02(cMonto As Decimal, cMontoUS As Decimal)
        ...
        asientoTransitod.movimiento.Add(AS_Proveedor(CDec(txtTotalmn.Text), CDec(txtTotalme.Text)))
    ...
    Sub AsientoNotaCredito()
        ...
        '    nAsiento.movimiento.Add(AS_Proveedor(CDec(txtTotalmn.Text), CDec(txtTotalme.Text)))
        'Case TIPO_COMPRA.COMPRA_DIRECTA_CON_RECEPCION, TIPO_COMPRA.COMPRA_DIRECTA_SIN_RECEPCION
        '    nAsiento.movimiento.Add(AS_Caja(CDec(txtTotalmn.Text), CDec(txtTotalme.Text)))
        '  End Select
        nAsiento.movimiento.Add(AS_Proveedor(CDec(txtTotalmn.Text), CDec(txtTotalme.Text)))
                        ...
                        Select Case "01" ' strTipoNota
                            Case Notas_Credito.DEV_EXISTENCIA
                                ...
                                Select Case dgvNuevoDoc.Rows(i.Index).Cells(1).Value()
                                    Case "1"
                                        objTotalesDet.importeSoles = CDec(dgvNuevoDoc.Rows(i.Index).Cells(7).Value) * CDec(dgvNuevoDoc.Rows(i.Index).Cells(41).Value) '  CType(dgvNuevoDoc.Rows(i.Index).Cells(8).Value(), Decimal)
                                        objTotalesDet.importeDolares = CDec(dgvNuevoDoc.Rows(i.Index).Cells(7).Value) * CDec(dgvNuevoDoc.Rows(i.Index).Cells(42).Value) ' CType(dgvNuevoDoc.Rows(i.Index).Cells(11).Value(), Decimal)
                                    Case Else
                                        objTotalesDet.importeSoles = CDec(dgvNuevoDoc.Rows(i.Index).Cells(7).Value) * CDec(dgvNuevoDoc.Rows(i.Index).Cells(41).Value) ' CType(dgvNuevoDoc.Rows(i.Index).Cells(8).Value(), Decimal)
                                        objTotalesDet.importeDolares = CDec(dgvNuevoDoc.Rows(i.Index).Cells(7).Value) * CDec(dgvNuevoDoc.Rows(i.Index).Cells(42).Value) ' CType(dgvNuevoDoc.Rows(i.Index).Cells(11).Value(), Decimal)
                                End Select
    ...
    Private Function ListaTotalesDePago() As documentoCaja
        Dim objTotalesDet As New documentoCaja
        Dim ListaTotales As New List(Of documentoCaja)

        For Each item As ListViewItem In lsvPagos.Items
            If (item.SubItems(4).Text = "103") Then
                objTotalesDet = New documentoCaja
                objTotalesDet.idDocumento = item.SubItems(0).Text
                objTotalesDet.codigoLibro = item.SubItems(1).Text
                objTotalesDet.montoSoles = item.SubItems(2).Text
                objTotalesDet.montoUsd = item.SubItems(3).Text
            End If
        Next

        Return objTotalesDet
    End Function
    ...
    Function ComprobanteCaja() As documento
        ...
        objCaja.IdProveedor = Nothing 'txtProveedor.ValueMember
        objCaja.codigoLibro = "9912"
        objCaja.codigoProveedor = Nothing ' txtProveedor.ValueMember
        ...
        objCaja.montoSoles = CDec(txtTotalmn.Text - ImporteAnticipoMN)
        objCaja.montoUsd = CDec((txtTotalmn.Text - ImporteAnticipoMN) / txtTipoCambio.Value)
        ...
        objCajaDetalle.montoSoles = CDec(txtTotalmn.Text - ImporteAnticipoMN)
        objCajaDetalle.montoUsd = CDec((txtTotalmn.Text - ImporteAnticipoMN) / txtTipoCambio.Value) ' CDec(txtTotalme.Text)
        ...
        With asiento
            ...
            .idEntidad = Nothing ' txtProveedor.ValueMember
            .nombreEntidad = Nothing ' txtProveedor.Text
            .tipoEntidad = "PR"
            .fechaProceso = fecha
            .codigoLibro = "8"
            .tipo = "D"
            .tipoAsiento = "AS-NTC"
            .importeMN = CDec(txtTotalmn.Text - ImporteAnticipoMN)
            .importeME = CDec((txtTotalmn.Text - ImporteAnticipoMN) / txtTipoCambio.Value)
            .glosa = GlosaCompra()
            .usuarioActualizacion = "Jiuni"
            .fechaActualizacion = DateTime.Now
        End With
    ...
    Sub Grabar()
        ...
        With nDocumentoCompra
            ...
            '****************************************************************************************************************
            .ImporteNacional = IIf(txtTotalmn.Text = 0 Or txtTotalmn.Text = "0.00", CDec(0.0), CDec(txtTotalmn.Text))
            .ImporteExtranjero = IIf(txtTotalme.Text = 0 Or txtTotalme.Text = "0.00", CDec(0.0), CDec(txtTotalme.Text))
            ...
            If CDec(lblBonificaMN.Text) > 0 Then
                If chDeposito.Checked = True Then
                    .estadoCobro = Nota_Credito.DINERO_ENTREGADO
                Else
                    .estadoCobro = Nota_Credito.DINERO_PENDIENTE_DE_ENTREGA
                End If
            Else
                If chProceso.Checked = True Then
                    .estadoCobro = Nota_Credito.PROCESADO_SIN_MOVIMIENTOS
                End If
            End If
        ...
        For Each i As DataGridViewRow In dgvNuevoDoc.Rows
            ...
            If i.Cells(27).Value = "S" Then
                ...
                If i.Cells(39).Value = "Elegir almacén" Then
                    MessageBox.Show("Debe seleccionar un almacen valido!", "Atención", Nothing, MessageBoxIcon.Error)
                    Exit Sub
                End If
            End If
            ...
            Select Case i.Cells(21).Value()
                Case "GS"
                    ...
                    objDocumentoCompraDet.importeME = CDec(i.Cells(13).Value())
                ...
                Case Else
                    Select Case "01" ' strTipoNota
                        ...
                        Case Notas_Credito.ERR_CANTIDAD, Notas_Credito.BOF_REDUC_COSTO_IGUAL_COMPRA
                            ...
                            objDocumentoCompraDet.monto1 = CDec(i.Cells(7).Value())
                            objDocumentoCompraDet.importeMN = 0
                    ...
                    End Select
                    objDocumentoCompraDet.salidaCostoMN = CDec(i.Cells(7).Value()) * CDec(i.Cells(41).Value())
                    objDocumentoCompraDet.salidaCostoME = CDec(i.Cells(7).Value()) * CDec(i.Cells(42).Value())
        ...
        Select Case cboOperacion.Text
            ...
            Case "DEVOLUCION DE EXISTENCIAS"
                ...
                ndocumento.documentoCaja = ListaTotalesDePago()
            Case "DISMINUIR IMPORTE"
                ListaTotales = ListaTotalesAlmacen()
                AsientoNotaCredito()
                ndocumento.asiento = ListaAsientonTransito
        ...
        Dim xcod As Integer = CompraSA.SaveCompraNotaCreditoVenta(ndocumento, ListaTotales, DocCaja)
...
#End Region
    ...
    Private Sub frmCreditoVenta_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Dispose()
    End Sub

    Private Sub frmCreditoVenta_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If dgvNuevoDoc.Rows.Count > 0 Then
            ...
            Select Case e.ColumnIndex
                ...
                Case 8, 7
                    ...
                    Select Case TipoCompra
                        Case TIPO_VENTA.VENTA_AL_TICKET, TIPO_VENTA.VENTA_AL_TICKET_DIRECTA
...
AndAlso (e.Value IsNot Nothing) Then
    ...
    Private Sub Timer2_Tick(sender As System.Object, e As System.EventArgs) Handles Timer2.Tick
        ...
        End If
        If CDec(lblBonificaMN.Text) > 0 Then
            For Each item In lsvPagos.Items
                If (item.subitems(4).text = "12.1" Or item.subitems(4).text = "12.2") Then
                    chDeposito.Enabled = True
                End If
            Next
        Else
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            With frmBuscarVentas
                ...
                If datos.Count > 0 Then
                    ...
                    If (cboOperacion.Text = "DEVOLUCION DE EXISTENCIAS") Then
                        limpiarPagos(True)
                        ObtenerDetallePagos(IdCompraOrigen)
                    Else
                        limpiarPagos(False)
                    End If
    ...
    Private Sub limpiarPagos(tipo As Boolean)
        pnPagos.Visible = tipo
        lsvPagos.Items.Clear()
        lsvPagos.Columns.Clear()
    End Sub
                ...
                Case "GS"
                    If CDec(Me.dgvMov.Table.CurrentRecord.GetValue("importeMN")) > 0 Then
                        Try
                            ...
                            If Not IsNothing(objInsumo.descripcionItem) Then
                                If ExisteDatoEnGrid(objInsumo.IdInsumo) = False Then
                                    dgvNuevoDoc.Rows.Add(objInsumo.Secuencia,
                                                         objInsumo.origenProducto,
                                                   objInsumo.IdInsumo,
                                                   objInsumo.descripcionItem,
                                                   objInsumo.presentacion,
                                                   objInsumo.Nombrepresentacion,
                                                   ...
                                                   "0.00", 0,
                                                    ...
                                                    objInsumo.tipoExistencia, objInsumo.cuenta, objInsumo.IdActividadRecurso, Nothing,
                                                     ...
                                                     Me.dgvMov.Table.CurrentRecord.GetValue("cantidad"),
                                                     Me.dgvMov.Table.CurrentRecord.GetValue("importeMN"),
                                                     Me.dgvMov.Table.CurrentRecord.GetValue("importeME"), Nothing, Nothing, Nothing, "Elegir almacén", Nothing,
                                                     Me.dgvMov.Table.CurrentRecord.GetValue("PMmn"), Me.dgvMov.Table.CurrentRecord.GetValue("PMme"))
                                End If
                            End If
                            If dgvNuevoDoc.Rows.Count > 0 Then
                                CellEndEditRefresh()
                ...
                Case Else
                    If CDec(Me.dgvMov.Table.CurrentRecord.GetValue("cantidad")) > 0 Then
                        Try
                            ...
                            objInsumo.PU = 0 ' lsvCanasta.SelectedItems(0).SubItems(6).Text
                            objInsumo.Total = Me.dgvMov.Table.CurrentRecord.GetValue("importeMN")
                            objInsumo.TotalUS = Me.dgvMov.Table.CurrentRecord.GetValue("importeME")

                            objInsumo.saldoMN = Me.dgvMov.Table.CurrentRecord.GetValue("montokardex")
                            objInsumo.saldoME = Me.dgvMov.Table.CurrentRecord.GetValue("montokardexus")

                            objInsumo.IgvMN = Me.dgvMov.Table.CurrentRecord.GetValue("montoIgv")
                            objInsumo.IgvME = Me.dgvMov.Table.CurrentRecord.GetValue("montoIgvUS")
                                                          ...
                                                          objInsumo.IgvMN,
                                                           ...
                                                           objInsumo.IgvME,
                                                            ...
                                                            objInsumo.tipoExistencia, objInsumo.cuenta, objInsumo.IdActividadRecurso, Nothing,
                                                             ...
                                                             Me.dgvMov.Table.CurrentRecord.GetValue("cantidad"),
                                                             Me.dgvMov.Table.CurrentRecord.GetValue("importeMN"),
                                                             Me.dgvMov.Table.CurrentRecord.GetValue("importeME"), Nothing, Nothing, Nothing, Nothing, Nothing,
                                                                     Me.dgvMov.Table.CurrentRecord.GetValue("PMmn"), Me.dgvMov.Table.CurrentRecord.GetValue("PMme"))
                                            If (cboOperacion.SelectedItem = "DEVOLUCION DE EXISTENCIAS") Then
                                                Select Case TipoCompra
                                                    Case TIPO_VENTA.VENTA_AL_TICKET, TIPO_VENTA.VENTA_AL_TICKET_DIRECTA
                                                        dgvNuevoDoc.Item(37, dgvNuevoDoc.CurrentRow.Index).Value = Me.dgvMov.Table.CurrentRecord.GetValue("importeMN")
                                                        dgvNuevoDoc.Item(38, dgvNuevoDoc.CurrentRow.Index).Value = Me.dgvMov.Table.CurrentRecord.GetValue("importeME")
                                                End Select
                                                totales_xx()
                                                TotalesCabeceras()
                                            End If
                    ...
                    If Not IsNothing(objInsumo.descripcionItem) Then
                        dgvNuevoDoc.Rows.Add(objInsumo.Secuencia, objInsumo.origenProducto,
                                       ...
                                       "0.00", 0,
                                        ...
                                        objInsumo.tipoExistencia, objInsumo.cuenta, objInsumo.IdActividadRecurso, Nothing,
                                         Nothing, Nothing, "S", Nothing, Nothing, objInsumo.IdAlmacen,
                                         Me.dgvMov.Table.CurrentRecord.GetValue("cantCompra"),
                                         Me.dgvMov.Table.CurrentRecord.GetValue("compraMN"),
                                         Me.dgvMov.Table.CurrentRecord.GetValue("cantidad"),
                                         Me.dgvMov.Table.CurrentRecord.GetValue("importeMN"),
                                         Me.dgvMov.Table.CurrentRecord.GetValue("importeME"))
        ...
        If dgvNuevoDoc.Rows.Count > 0 Then
            ...
            If Not IsNothing(dgvNuevoDoc.CurrentRow) Then
                ...
                Else
                    ...
                    txtTotalBImn.Text = "0.00"
                    txtTotalBIme.Text = "0.00"
                    txtMontoIGVme.Text = "0.00"
                    txtMontoIGVmn.Text = "0.00"
                End If
            End If
        End If
    End Sub
    ...
    Private Sub btnPagos_Click(sender As Object, e As EventArgs) Handles btnPagos.Click
        If (pnPagos.Visible = False) Then
            If (cboOperacion.Text = "DEVOLUCION DE EXISTENCIAS" And TextBoxExt1.Text.Length > 0) Then
                limpiarPagos(True)
                ObtenerDetallePagos(IdCompraOrigen)
            End If
        Else
