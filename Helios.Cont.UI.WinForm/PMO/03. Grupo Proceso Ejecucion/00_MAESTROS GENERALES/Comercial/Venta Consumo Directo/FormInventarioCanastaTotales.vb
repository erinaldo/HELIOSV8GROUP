...
Public Class FormInventarioCanastaTotales
    Implements IBusquedaAvanzadaProductos, IConfirmarCompraRapida
...
#Region "Constructors"
    Public Sub New(id As Integer)
        ...
        FormatoGridAvanzado(GridSinAlmacen, False, False, 11.5F)
...
#Region "Methods"
    ...
    Private Sub setDataSource(ByVal table As DataTable)
        ...
        Else
            GridSinAlmacen.DataSource = table
            GridSinAlmacen.TableDescriptor.Columns("descripcion").Appearance.AnyCell.VerticalAlignment = GridVerticalAlignment.Middle
            GridSinAlmacen.TableDescriptor.Columns("descripcion").Appearance.AnyCell.HorizontalAlignment = GridHorizontalAlignment.Left

            TabControlAdv1.TabPages(1).Text = "Productos sin almacén: " & GridSinAlmacen.Table.Records.Count
        End If
    End Sub
    ...
    Private Sub ObtenerCanastaVentaFiltro(strTipoExistencia As String, strProducto As String)
        ...
        Try
            ...
            For Each i As totalesAlmacen In CanastaSA.GetInventarioParaVentaAcumulado(
                New totalesAlmacen With
                {
                .Moneda = monedaVenta,
                .tipoExistencia = strTipoExistencia,
                ...
                If CheckSinInventario.Checked = False Then
                    If i.cantidad > 0 Then
                        'Dim valPrecUnitario As Decimal = CDec(i.importeSoles) / (i.cantidad)
                        'Dim valPrecUnitarioUS As Decimal = CDec(i.importeDolares) / (i.cantidad)
    ...
    Private Sub ObtenerCanastaVentaFiltroForma3(strTipoExistencia As String, strProducto As String)
        ...
        Try
            ...
            For Each i As usp_GetProductsByEstable_Result In CanastaSA.GetProductsShopingOrOthers(objAlmacen)

                cprecioVentaFinalMenorMN = i.menor.GetValueOrDefault
                cprecioVentaFinalMayorMN = i.mayor.GetValueOrDefault
                cprecioVentaFinalGMayorMN = i.granMayor.GetValueOrDefault
...
#Region "Events"
    Private Sub dgvKardexVal_TableControlDrawCell(sender As Object, e As GridTableControlDrawCellEventArgs) Handles GridTotales.TableControlDrawCell
        If e.Inner.Style.CellType = "PushButton" Then
            ...
            ElseIf e.Inner.ColIndex = 8 Then ' menor dolares
                e.Inner.Style.Description = e.Inner.Style.CellValue
                e.Inner.Style.TextColor = Color.Black
                e.Inner.Renderer.Draw(e.Inner.Graphics, e.Inner.Bounds, e.Inner.RowIndex, e.Inner.ColIndex, e.Inner.Style)
                Dim irect As New Rectangle(New Point(e.Inner.Bounds.X + 3, e.Inner.Bounds.Y + 3), New Size(e.Inner.Bounds.Size.Width - 6, e.Inner.Bounds.Size.Height - 6))
            ...
            ElseIf e.Inner.ColIndex = 10 Then 'mayor dolares
                ...
                e.Inner.Renderer.Draw(e.Inner.Graphics, e.Inner.Bounds, e.Inner.RowIndex, e.Inner.ColIndex, e.Inner.Style)
                Dim irect As New Rectangle(New Point(e.Inner.Bounds.X + 3, e.Inner.Bounds.Y + 3), New Size(e.Inner.Bounds.Size.Width - 6, e.Inner.Bounds.Size.Height - 6))
            ElseIf e.Inner.ColIndex = 11 Then
                e.Inner.Style.Description = e.Inner.Style.CellValue
                e.Inner.Style.TextColor = Color.Black
                e.Inner.Renderer.Draw(e.Inner.Graphics, e.Inner.Bounds, e.Inner.RowIndex, e.Inner.ColIndex, e.Inner.Style)
                Dim irect As New Rectangle(New Point(e.Inner.Bounds.X + 3, e.Inner.Bounds.Y + 3), New Size(e.Inner.Bounds.Size.Width - 6, e.Inner.Bounds.Size.Height - 6))
            ElseIf e.Inner.ColIndex = 12 Then
                e.Inner.Style.Description = e.Inner.Style.CellValue
                e.Inner.Style.TextColor = Color.Black
                e.Inner.Renderer.Draw(e.Inner.Graphics, e.Inner.Bounds, e.Inner.RowIndex, e.Inner.ColIndex, e.Inner.Style)
                Dim irect As New Rectangle(New Point(e.Inner.Bounds.X + 3, e.Inner.Bounds.Y + 3), New Size(e.Inner.Bounds.Size.Width - 6, e.Inner.Bounds.Size.Height - 6))
    ...
    Private Sub dgvKardexVal_TableControlPushButtonClick(sender As Object, e As GridTableControlCellPushButtonClickEventArgs) Handles GridTotales.TableControlPushButtonClick
        Me.Cursor = Cursors.WaitCursor
        Try
            If chCantidadPrevia.Checked = True Then
                If e.Inner.ColIndex = 7 Then 'menor soles
                    Dim precioMenor = Decimal.Parse(GridTotales.TableModel(e.Inner.RowIndex, 7).CellValue)
                    'If validaStocks = True Then
                ...
                ElseIf e.Inner.ColIndex = 8 Then 'menor dolares
                    Dim precioMenorME = Decimal.Parse(GridTotales.TableModel(e.Inner.RowIndex, 8).CellValue)
                    'If validaStocks = True Then
                    '          GetproductoSelect(1, "Precio por Menor", precioMenor)
                    'GetproductoUnico(1, "Precio por Menor", precioMenor)
...
#End Region
    ...
    Private Sub chCantidadPrevia_Click(sender As Object, e As EventArgs) Handles chCantidadPrevia.Click
    ...
    Private Sub GridSinAlmacen_TableControlCellClick(sender As Object, e As GridTableControlCellClickEventArgs) Handles GridSinAlmacen.TableControlCellClick

    End Sub

    Private Sub GridSinAlmacen_QueryCellStyleInfo(sender As Object, e As GridTableCellStyleInfoEventArgs) Handles GridSinAlmacen.QueryCellStyleInfo
        'If (e.TableCellIdentity.TableCellType = GridTableCellType.RecordFieldCell OrElse e.TableCellIdentity.TableCellType = GridTableCellType.AlternateRecordFieldCell) Then
        '    If e.TableCellIdentity.Column.MappingName = "pvmenor" Then
        '        Dim el As Element = e.Style.TableCellIdentity.DisplayElement
        '        If el IsNot Nothing AndAlso el.Kind = DisplayElementKind.Record Then
    ...
    Public Sub ConfirmaTransaccion(Confirmado As Boolean) Implements IConfirmarCompraRapida.ConfirmaTransaccion
        If Confirmado = True Then
            If GridSinAlmacen.Table.CurrentRecord IsNot Nothing Then
                GridSinAlmacen.Table.CurrentRecord.Delete()
                GridSinAlmacen.Refresh()
            End If
        End If
    End Sub
