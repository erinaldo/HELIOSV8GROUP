...
Public Class frmNotaCreditoEspecial
    Inherits frmMaster
    ...
    Public Sub New(intIdDocumento As Integer)
        ...
        UbicarDetalleFast(intIdDocumento)
    End Sub
...
#Region "Metodos"
    ...
    Private Sub MostrarColumnsByMoneda(strMoneda As String)
        ...
        Select Case strMoneda
            Case "1"
                ...
                dgvMov.TableDescriptor.Columns("importeMN").Width = 65
            ...
            Case "2"
                ...
                dgvMov.TableDescriptor.Columns("importeME").Width = 65
    ...
    Public Sub UbicarDetalleFast(intIddocumento As Integer)
        Dim detalleSA As New DocumentoCompraDetalleSA
        Dim objLista As New DocumentoCajaDetalleSA
        Dim detalle As New documentocompradetalle
        Dim compraSA As New DocumentoCompraSA
        ...
        Try
            ...
            For Each det As documentoCajaDetalle In objLista.ObtenerCuentasPorPagarPorDetails(intIddocumento)
                ...
                i = objLista.ObtenerCuentasPorPagarBySecuencia(det.secuencia)

                detalle = detalleSA.SumaNotasXidPadreItemOpcionDefault(det.secuencia)
                ...
                Select Case i.TipoExistencia
                    ...
                    Case Else
                        If IsNothing(detalle) Then
                            Me.dgvMov.Table.CurrentRecord.SetValue("cantidad", 0)
                        Else
                            Me.dgvMov.Table.CurrentRecord.SetValue("cantidad", i.CantidadCompra - detalle.monto1.GetValueOrDefault)
                        End If
                End Select
                ...
                Me.dgvMov.Table.CurrentRecord.SetValue("canDev", i.CantidadCompra - detalle.monto1.GetValueOrDefault)
                Me.dgvMov.Table.CurrentRecord.SetValue("canSaldo", 0)

                Me.dgvMov.Table.CurrentRecord.SetValue("vcmn", i.montokardex)
                Me.dgvMov.Table.CurrentRecord.SetValue("vcme", i.montokardexus)
                Me.dgvMov.Table.CurrentRecord.SetValue("ivamn", i.montoIgv)
                Me.dgvMov.Table.CurrentRecord.SetValue("ivame", i.montoIgvUS)
                Me.dgvMov.Table.CurrentRecord.SetValue("totalmn", i.MontoDeudaSoles)
                Me.dgvMov.Table.CurrentRecord.SetValue("totalme", i.MontoDeudaUSD)
...
#End Region
    ...
    Private Sub frmNotaCreditoEspecial_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Dispose()
    End Sub

    Private Sub frmNotaCreditoEspecial_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub dgvMov_QueryCellStyleInfo(sender As Object, e As GridTableCellStyleInfoEventArgs) Handles dgvMov.QueryCellStyleInfo
        If Not IsNothing(e.TableCellIdentity.Column) Then
            ...
            ElseIf (el.Kind = Syncfusion.Grouping.DisplayElementKind.Record AndAlso e.TableCellIdentity.Column IsNot Nothing AndAlso (e.TableCellIdentity.Column.Name = "cboMov")) Then
                Dim str = Me.dgvMov.TableModel(e.TableCellIdentity.RowIndex, 25).CellValue
                If Not IsNothing(str) Then
                    If str = "GS" Then
                        e.Style.ReadOnly = True
                    Else
                    ...
                    Case 12
                        ...
                        If dgvMov.Table.CurrentRecord.GetValue("tipoEx") <> TipoRecurso.SERVICIO Then
                            If CDec(dgvMov.Table.CurrentRecord.GetValue("canDev")) > 0 Then
                                ...
                                With PanelStock
                                    .Top = (.Parent.ClientSize.Height - PanelStock.Height) \ 2
                                    .Left = (.Parent.ClientSize.Width - PanelStock.Width) \ 2
                                End With
    ...
    Private Sub btGrabar_Click(sender As Object, e As EventArgs) Handles btGrabar.Click
        Try
            ...
            If Not txtNumeroGuia.Text.Trim.Length > 0 Then
                lblEstado.Text = "Ingresar un número de guía de comprobante válido"
                Timer1.Enabled = True
                PanelError.Visible = True
                TiempoEjecutar(10)
