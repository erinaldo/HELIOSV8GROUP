...
#Region "variables"
    ...
    Public lineaEspacio As Integer = 83
...
#End Region
    ...
    Public Sub ImprimeTicket(ByVal impresora As String, conteoImpresion As Integer)
        ...
        Try
            If (Not IsNothing(headerImageQR.Width)) Then
                If (headerImageQR.Width <> 0) Then
                    Dim destRect As New Rectangle(8, 122, 18, 18)
...
#Region "DATOS COMPLEMEMTARIOS"
    ...
    Private Sub DibujaDatosComplemnetarios()
        ...
        For Each Cabecera As String In LineasDeDatosComplementarios
            '//SE CREA UN RECTANGULO
            Dim R1 As New RectangleF(6, 68, 35, 3)
        ...
        gfx.DrawString("                     N° Pedido                             Orden de Compra                           Guía de Remisión                             Forma de Pago                                Tipo de Venta", FuenteImpresaComplemeneto, ColorDeLaFuente, 5, 61, New StringFormat())
        gfx.DrawString("                       Fecha                                             Fecha                                               Fecha", FuenteImpresaComplemeneto, ColorDeLaFuente, 5, 64, New StringFormat())

        '//SE DIBUJA TODA LAS LINEAS DE LOS DATOS COMPLEMENTARIOS
        gfx.DrawRectangle(blackPen, 5, 61, 197, 6)
        gfx.DrawRectangle(blackPen, 5, 61, 39, 14)
        gfx.DrawRectangle(blackPen, 44, 61, 39, 14)
        gfx.DrawRectangle(blackPen, 83, 61, 39, 14)
        gfx.DrawRectangle(blackPen, 122, 61, 39, 14)
        gfx.DrawRectangle(blackPen, 161, 61, 41, 14)
...
#Region "DATOS DETALLE DE VENTA"
    ...
    Private Sub DibujaElementosFactura(i As ArrayList)
        ...
        gfx.DrawString("    Numero                                                 Descripción                                               UM           Cant.           Precio Unit.        Valor Total", FuenteImpresaEncabezado, ColorDeLaFuente, 6, 62, New StringFormat())

        '//SE CREA EL CUADRO O TABLAS DE LOS DETALLES
        gfx.DrawRectangle(blackPen, 5, 61, 197, 7)
        'crear la fila codigo
        gfx.DrawRectangle(blackPen, 5, 61, 20, 50)
        'crear la fila descrpcion
        gfx.DrawRectangle(blackPen, 25, 61, 98, 50)
        'crear la fila um
        gfx.DrawRectangle(blackPen, 123, 61, 10, 50)
        'crear la fila cantidad
        gfx.DrawRectangle(blackPen, 133, 61, 20, 50)
        ''crear la fila PRECIO UNITARIO
        gfx.DrawRectangle(blackPen, 153, 61, 24, 50)
        ''crear la fila Venta Total
        gfx.DrawRectangle(blackPen, 177, 61, 25, 50)
        ...
        For Each Item As String In i
            ...
            '//CODIGO DEL CANTIDAD
            'se crea un rectangulo para manipular texto dentro del rectangulo
            Dim R2 As New RectangleF(133, lineaEspacio, 20, 130)
            Cantidad = 0.0
...
#Region "DATOS DETALLE TOTALES"
    ...
    Private Sub DibujaTotalFactura()
        ...
        gfx.DrawRectangle(blackPen, 139, 112, 63, 29)
        ...
        For Each Elemento As String In LineasTotalFactura
            ...
            CadenaPorEscribirEnLinea = AlineaTextoaLaDerecha(CadenaPorEscribirEnLinea.Length) + CadenaPorEscribirEnLinea
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezadoTexto, ColorDeLaFuente, 155, contadorGeneral + 112, New StringFormat())

            CadenaPorEscribirEnLinea = "0.00"
            CadenaPorEscribirEnLinea = AlineaTextoaLaDerecha(CadenaPorEscribirEnLinea.Length) + CadenaPorEscribirEnLinea
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, ColorDeLaFuente, 174, contadorGeneral + 112, New StringFormat())
            contadorGeneral += 3
...
#Region "DATOS FINALES DE LA FACTURA"
    ...
    Private Sub DibujaDatosGenerales(Paginacion As String)
        ...
        'crear la fila codigo
        gfx.DrawRectangle(blackPen, 5, 112, 133, 5)
        'crear la fila cantidad
        gfx.DrawRectangle(blackPen, 5, 117, 133, 24)

        'gfx.DrawLine(blackPen, 104, 123, 104, 143)
        ...
        For Each Elemento As String In LineasDatosComplementaria
            ...
            gfx.DrawString("SON: " & conver & " SOLES", FuenteImpresaEncabezado, ColorDeLaFuente, 11, contadorDatos + 112, New StringFormat())
        Next
        ...
        'gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, ColorDeLaFuente, 116, contadorDatos + 140, New StringFormat())
        ...
        If (tipoComprobante = "2") Then
            ...
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, ColorDeLaFuente, 35, contadorDatos + 123, New StringFormat())

            CadenaPorEscribirEnLinea = "N° 034-005-0010982. Puede consultar su comprobante electrónico en"
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, ColorDeLaFuente, 35, contadorDatos + 126, New StringFormat())

            CadenaPorEscribirEnLinea = "http://www.spk.com.pe/"
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, ColorDeLaFuente, 35, contadorDatos + 129, New StringFormat())

            CadenaPorEscribirEnLinea = "Conserve su comprobante por regulación de SUNAT es indispensable presentarlo para"
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, ColorDeLaFuente, 35, contadorDatos + 132, New StringFormat())

            CadenaPorEscribirEnLinea = "cambios o devoluciones. Todo cambio o devolución es personal o dentro de los 7 días"
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, ColorDeLaFuente, 35, contadorDatos + 135, New StringFormat())

            CadenaPorEscribirEnLinea = "posteriores a la compra."
            gfx.DrawString(CadenaPorEscribirEnLinea, FuenteImpresaEncabezado, ColorDeLaFuente, 35, contadorDatos + 138, New StringFormat())
