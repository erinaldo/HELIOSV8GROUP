'------------------------------------------------------------------------------
' <auto-generated>
'     This code was generated from a template.
'
'     Manual changes to this file may cause unexpected behavior in your application.
'     Manual changes to this file will be overwritten if the code is regenerated.
' </auto-generated>
'------------------------------------------------------------------------------

Imports System
Imports System.Collections.Generic

Partial Public Class DerechoHabientes
    Public Property IDHabiente As Integer
    Public Property IDPersonal As Integer
    Public Property Nombre As String
    Public Property Sexo As String
    Public Property fechanacimiento As Nullable(Of Date)
    Public Property Vinculo As String
    Public Property Situacion As String
    Public Property Incapacidad As Nullable(Of Integer)
    Public Property CertificadoMedico As String
    Public Property Domicilio As Nullable(Of Integer)
    Public Property FechaModificacion As Nullable(Of Date)
    Public Property UsuarioModificacion As String

    Public Overridable Property Personal As Personal

End Class
